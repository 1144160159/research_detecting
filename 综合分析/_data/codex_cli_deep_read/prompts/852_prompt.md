...
元数据：
编号：852
题名：On Calibration of Modern Neural Networks
年份：2017
...
正文包信息：
- 正文来源：综合分析\_data\full_text_cache_plain\852.txt
- 原始字符数：91098
- 本次发送字符数：91098
- 是否截断：False
...
but does not match its confidence.
                                                                                             1 X
Our goal is not only to understand why neural networks                         acc(Bm ) =              1(ŷi = yi ),
                                                                                           |Bm |
                                                                                                   i∈Bm
have become miscalibrated, but also to identify what meth-
ods can alleviate this problem. In this paper, we demon-           where ŷi and yi are the predicted and true class labels for
strate on several computer vision and NLP tasks that neu-          sample i. Basic probability tells us that acc(Bm ) is an un-
ral networks produce confidences that do not represent true        biased and consistent estimator of P(Ŷ = Y | P̂ ∈ Im ).
...
(Ŷ , P̂ ), where Ŷ is a class prediction and P̂ is its associ-                   h                          i
ated confidence, i.e. probability of correctness. We would                       E P Ŷ = Y | P̂ = p − p                    (2)
                                                                                 P̂
...
Error/ECE
 ...
 deviation between confidence and accuracy:                                    and hundreds of convolutional filters per layer (Zagoruyko
              ...
              max P Ŷ = Y | P̂ = p − p .             (4)                      or wide models are able to generalize better than smaller
                        p∈[0,1]
...
where M is the number of intervals; a1 , . . . , aM +1 are the        outputs a class prediction ŷi and confidence score p̂i for
interval boundaries; and θ1 , . . . , θM are the function val-        each input xi . In this case, the network logits zi are vectors,
                                                                                                (k)
ues. Under this parameterization, isotonic regression is a            where ŷi = argmaxk zi , and p̂i is typically derived using
...
2015) is a extension of histogram binning using Bayesian              The goal is to produce a calibrated confidence q̂i and (pos-
                                                                      sibly new) class prediction ŷi0 based on yi , ŷi , p̂i , and zi .
  1
...
test time, we obtain an unnormalized probability vector                 not change the maximum of the softmax function, the class
   (1)           (K)          (k)
[q̂i , . . . , q̂i ], where q̂i is the calibrated probability for       prediction ŷi0 remains unchanged. In other words, temper-
class k. The new class prediction ŷi0 is the argmax of the             ature scaling does not affect the model’s accuracy.
...
plies a linear transformation Wzi + b to the logits:
                ...
                ŷi0 = argmax (Wzi + b)(k) .                            ious contexts in recent years. Kuleshov & Ermon (2016)
                             k
...
Collobert, Ronan, Kavukcuoglu, Koray, and Farabet,             Iyyer, Mohit, Manjunatha, Varun, Boyd-Graber, Jordan,
  Clément. Torch7: A matlab-like environment for ma-            and Daumé III, Hal. Deep unordered composition rivals
...
Kuleshov, Volodymyr and Liang, Percy. Calibrated struc-       Srivastava, Rupesh Kumar, Greff, Klaus, and Schmid-
  tured prediction. In NIPS, pp. 3474–3482, 2015.               huber, Jürgen. Highway networks. arXiv preprint
...
LeCun, Yann, Bottou, Léon, Bengio, Yoshua, and Haffner,      Vapnik, Vladimir N. Statistical Learning Theory. Wiley-
...
tion definition, which is restated here:
                 h                     i
               E P Ŷ = Y | P̂ = p − p                             Proof. We solve this constrained optimization problem us-
                 P̂
...
Stieltjes integral we have                                                              K
                                                                                      n X
                                                                                      X
         h                          i                                      L=−                     q(zi )(k) log q(zi )(k)
       E P Ŷ = Y | P̂ = p − p                                                        i=1 k=1
      P̂                                                                               "K                    #
           Z 1                                                                     n  X (k)          (yi )
                                                                                     X
                                                                                                   (k)
      =          P Ŷ = Y | P̂ = p − p dFP̂ (p)                                   +λ      zi q(zi ) − zi
           0                                                                             i=1     k=1
           M
           X                                                                          n          K
...
EP̂ P Ŷ = Y | P̂ = p − p .                                        ing gives
                                                                                                            (k)
                                                                                                                  +βi −nK
                                                                                      q(zi )(k) = eλzi                         .
S2. Further Information on Temperature                                     PK            (k)
    Scaling                                                        Since    k=1 q(zi )         = 1 for all i, we must have
                                                                                                                   (k)

Here we derive the temperature scaling model using the en-                                     (k)           eλzi
                                                                                      q(zi )         = PK                (j)
                                                                                                                               ,
tropy maximization principle with an appropriate balanced                                                           λzi
                                                                                                            j=1 e
...
solution q to the following entropy maximization problem:
       ...
       subject to         q(zi )(k) ≥ 0         ∀i, k              confidence. Temperature scaling not only lowers the NLL
                          ...
                          n        n X
                             ...
                             (y )       (k)
                          X        X
                            zi i =     zi q(zi )(k) .              Tables S1, S2, and S3 display the MCE, test error, and NLL
