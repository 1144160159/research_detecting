...
def get_pgd_adv_features(
    ...
    for _ in range(total_repeat_time):
        ...
        added_features[:, dataset_fields["duration"]] = np.clip(added_features[:, dataset_fields["duration"]], perturbed_feature_[0, dataset_fields["duration"]], 1.2 * perturbed_feature_[0, dataset_fields["duration"]])
        added_features[:, dataset_fields["total_fiat"]] = np.clip(added_features[:, dataset_fields["total_fiat"]], perturbed_feature_[0, dataset_fields["total_fiat"]], 1.2 * perturbed_feature_[0, dataset_fields["total_fiat"]])
    ...
    for _ in range(num_restarts):
        ...
        for result in final_results:
            ...
            if smt_success:
                ...
                if (
                    attack_success == False
                    and torch.round(torch.squeeze(adv_pred)) != labels[0]
                ):
                    attack_success = True
...
for i, testdata in enumerate(loader):
    ...
    if torch.round(torch.squeeze(pred)) == labels[0]:
        (
            adv_loss,
            orig_loss,
            attack_success,
