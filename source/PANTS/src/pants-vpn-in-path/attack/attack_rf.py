...
def get_pgd_adv_features(
    ...
    for i in range(total_repeat_time):
        ...
        perturbed_feature[:, dataset_fields["duration"]] = np.clip(perturbed_feature[:, dataset_fields["duration"]], perturbed_feature_[0, dataset_fields["duration"]], 1.2 * perturbed_feature_[0, dataset_fields["duration"]])
        perturbed_feature[:, dataset_fields["total_fiat"]] = np.clip(perturbed_feature[:, dataset_fields["total_fiat"]], perturbed_feature_[0, dataset_fields["total_fiat"]], 1.2 * perturbed_feature_[0, dataset_fields["total_fiat"]])
...
asset_model_dir = os.path.join("../../../", "asset", "pants-vpn-in-path", args.asset_name)
asset_common_dir = os.path.join("../../../", "asset", "pants-vpn-in-path", "common")
...
clf = RandomForestClassifier(max_depth=50, n_estimators=10, random_state=0, max_features=None)
