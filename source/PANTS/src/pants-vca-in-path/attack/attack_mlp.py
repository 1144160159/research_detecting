...
def get_pgd_adv_features(orig_feature, model, loss, total_repeat_time, iters, alpha):
    ...
    for _ in range(total_repeat_time):
        ...
        for _ in range(iters):
            ...
            perturbed_feature[:, dataset_fields["l_max"]] = orig_feature[:, dataset_fields["l_max"]]
            perturbed_feature[:, dataset_fields["l_min"]] = orig_feature[:, dataset_fields["l_min"]]
            perturbed_feature[:, dataset_fields["l_mean"]] = orig_feature[:, dataset_fields["l_mean"]]
            perturbed_feature[:, dataset_fields["l_std"]] = orig_feature[:, dataset_fields["l_std"]]
            perturbed_feature[:, dataset_fields["l_q2"]] = orig_feature[:, dataset_fields["l_q2"]]

            perturbed_feature[:, dataset_fields["l_num_bytes"]] = orig_feature[:, dataset_fields["l_num_bytes"]]
            perturbed_feature[:, dataset_fields["l_num_pkts"]] = orig_feature[:, dataset_fields["l_num_pkts"]]
            perturbed_feature[:, dataset_fields["l_num_unique"]] = orig_feature[:, dataset_fields["l_num_unique"]]
