...
def get_pgd_adv_features(orig_feature, model, loss, total_repeat_time, iters, alpha):
    ...
    for _ in range(total_repeat_time):
        ...
        added_features[:, dataset_fields["l_max"]] = orig_feature[:, dataset_fields["l_max"]]
        added_features[:, dataset_fields["l_min"]] = orig_feature[:, dataset_fields["l_min"]]
        added_features[:, dataset_fields["l_mean"]] = orig_feature[:, dataset_fields["l_mean"]]
        added_features[:, dataset_fields["l_std"]] = orig_feature[:, dataset_fields["l_std"]]
        added_features[:, dataset_fields["l_q2"]] = orig_feature[:, dataset_fields["l_q2"]]

        added_features[:, dataset_fields["l_num_bytes"]] = orig_feature[:, dataset_fields["l_num_bytes"]]
        added_features[:, dataset_fields["l_num_pkts"]] = orig_feature[:, dataset_fields["l_num_pkts"]]
        added_features[:, dataset_fields["l_num_unique"]] = orig_feature[:, dataset_fields["l_num_unique"]]
...
def get_feature_and_solve(
    ...
    for num_important_features in range(1, max_num_important_features):
        ...
        smt_results += results
        if len(considerred_important_features) == 2:
            break
