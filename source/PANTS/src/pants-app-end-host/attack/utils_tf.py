...
def pgd_func(
    ...
    perturbable_pkt_size_idx = [
        i
        for i in range(prepared_feature.size(1))
        if prepared_feature[0, i, 0] > normalized_direction_value
    ]
    ...
    for iteration in range(total_repeat_time * iters):
        ...
        if (iteration + 1) % iters == 0:
            ...
            diff = (
                added_features[0, existing_fwd_pkt_idx, 0]
                - prepared_feature[:, existing_fwd_pkt_idx, 0]
            ).tolist()[0]
            ...
            added_features[0, not_allowd_to_change_payload_idx, 0] = prepared_feature[
                0, not_allowd_to_change_payload_idx, 0
            ]
            tmp_queue.append(
...
def get_pgd_adv_features(
    ...
    added_pkt_num_list = list(
        np.random.choice(added_pkt_num_candidate_1, int(sample_num / 2), replace=True)
    ) + list(
        np.random.choice(added_pkt_num_candidate_2, int(sample_num / 2), replace=True)
    )
    large_candidate_list = []
    for added_pkt_num in added_pkt_num_list:
        position_list = np.random.choice(
            range(1, flow_length + 1), added_pkt_num, replace=True
        )
        position_list.sort()
        ...
        appended_pkt_len = (
            np.ones((1, first_n_pkts - pkt_len.shape[1], 1))
            * normalized_direction_value
        )
        ...
        large_candidate_list.append(
            (new_flow, orig_flow_len + added_pkt_num, injected_pkt_idx_list)
        )
...
def denormalize(orig_feat, max_iats, min_iats, max_pkt_lens, min_pkt_lens):
    ...
    orig_feat[:, :, 0] = (
        orig_feat[:, :, 0] * (max_pkt_lens - min_pkt_lens) + min_pkt_lens
    )
    orig_feat[:, :, 1] = np.expm1(orig_feat[:, :, 1])
    orig_feat[:, :, 0] = np.expm1(np.abs(orig_feat[:, :, 0])) * np.sign(
        orig_feat[:, :, 0]
    )
    return orig_feat
...
def normalize(orig_denorm_feat, max_iats, min_iats, max_pkt_lens, min_pkt_lens):
    ...
    orig_denorm_feat[:, :, 0] = np.log1p(np.abs(orig_denorm_feat[:, :, 0])) * np.sign(
        orig_denorm_feat[:, :, 0]
    )
    orig_denorm_feat[:, :, 1] = (orig_denorm_feat[:, :, 1] - min_iats) / (
        max_iats - min_iats
    )
    orig_denorm_feat[:, :, 0] = (orig_denorm_feat[:, :, 0] - min_pkt_lens) / (
