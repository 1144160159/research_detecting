...
normalized_min_iat = (np.log1p(1) - min_iats) / (max_iats - min_iats)
...
slice_data_volumn = np.ceil(len(correct_X) / 9)
success = 0
total = 0
...
for idx in range(len(correct_X)):
    ...
    for n, adv_inputs_set in enumerate(pgd_adv_features_list):
        ...
        for i in range(denorm_adv_inputs.size(1)):
            ...
            elif denorm_adv_inputs[0, i, 0] < 40 and denorm_adv_inputs[0, i, 0] > 0:
                denorm_adv_inputs[0, i, 0] = 40
        ...
        df_denorm_adv_inputs = pd.DataFrame(
            denorm_adv_inputs[0].numpy(), columns=["pkt_len", "iat"]
        )
        ...
        denorm_inputs = denorm_inputs * perturbable_mask
        df_denorm_inputs = pd.DataFrame(
            denorm_inputs[0].numpy(), columns=["pkt_len", "iat"]
        )
        ...
        if overhead > max_overhead:
            sorted_idx = get_important_features(
                df_denorm_adv_inputs, df_denorm_ref_inputs
            )
            smt_process_list.append(
                (
                    df_denorm_adv_inputs,
                    df_denorm_ref_inputs,
                    max_overhead,
                    sorted_idx,
                    remaining_sum_iats,
                )
    ...
    if len(smt_process_list) > 0:
        # print("smt_len", len(smt_process_list), "no_smt_len", len(final_pgd_adv_features_list))
        df_smt_out = call_smt(smt_process_list)
        # print("smt_out_len", len(df_smt_out))
        final_pgd_adv_features_list += df_smt_out

    # print("final_list_len", len(final_pgd_adv_features_list))
    for adv_feat in final_pgd_adv_features_list:
        ...
        smt_cross_entropy_results.append(
            nn.CrossEntropyLoss()(adv_outputs, labels).item()
        )
        if (
    ...
    # print("success", success_indicator)
