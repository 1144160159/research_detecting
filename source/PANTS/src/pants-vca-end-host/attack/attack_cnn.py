...
model = DF(input_shape=1, num_classes=11)
...
for idx in range(len(correct_X)):
    ...
    for n, adv_inputs_set in enumerate(pgd_adv_features_list):
        ...
        for i in range(first_n_pkts):
            ...
            elif denorm_adv_inputs[0, 0, i] < 307 and denorm_adv_inputs[0, 0, i] > 0:
                denorm_adv_inputs[0, 0, i] = 307

        perturbable_mask = get_perturbable_mask(inputs, adv_inputs_len)
