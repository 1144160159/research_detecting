...
def call_smt(
    ...
    if "l_num_pkts" in considered_important_feature_list:
        ...
        num_added_pkts_list = [i for i in range(6)]
        # num_added_pkts_list = [0]
...
def try_smt_weak(
    ...
    pkt_appendable_idx = [i for i in range(len(orig_pkt_len_list))]
    random.shuffle(pkt_appendable_idx)
    pkt_appendable_idx = pkt_appendable_idx[:int(PKT_PAYLOAD_APPEND_LIMIT * len(orig_pkt_len_list))]

    X_pkt_len = []
    for i in range(num_existing_pkts):
        s.add(X_existing_flow_delta_ts[i] >= 0)
        if i in pkt_appendable_idx:
