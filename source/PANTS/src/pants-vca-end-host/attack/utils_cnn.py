...
def pgd_func(prepared_feature, prepared_feature_len, injected_pkt_idx_list, existing_fwd_pkt_idx, normalized_direction_value, total_repeat_time, iters, model, loss, labels, alpha, tmp_queue):
    ...
    fwd_pkt_idx = []
    # pkt_len mask
    perturbable_pkt_size_idx = [i for i in range(first_n_pkts) if prepared_feature[0, 0, i] > normalized_direction_value]
    ...
    for iteration in range(total_repeat_time*iters):
        ...
        if (iteration + 1) % iters == 0:
            ...
            diff = added_features[0, 0, existing_fwd_pkt_idx] - prepared_feature[0, 0, existing_fwd_pkt_idx]
            ...
            added_features[0, 0, not_allowd_to_change_payload_idx] = prepared_feature[0, 0, not_allowd_to_change_payload_idx]
...
def try_smt(df_denorm_adv_inputs, df_denorm_inputs, max_overhead, sorted_idx, remaining_sum_iats, tmp_queue):
    ...
    s.add(sum(X_existing_flow_delta_ts[1:]) <= maximum_iats - sum(orig_iats[1:]) - remaining_sum_iats)
    s.add(sum(X_existing_flow_delta_ts[1:]) >= 0.98 * (maximum_iats - sum(orig_iats[1:]) - remaining_sum_iats))
