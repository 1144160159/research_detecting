...
def call_smt(
    ...
    num_splitted_pkts_percent_list = [0, 0.5 * PKT_SPLIT_LIMIT, PKT_SPLIT_LIMIT]  # [0.1 * i for i in range(0, 2)]
...
def try_smt_strong(
    ...
    num_split_pkts = np.ceil(num_splitted_pkts_percent * orig_num_perturable_pkts)
    random.shuffle(perturbable_pkt_indices)
    shuffle_idx = perturbable_pkt_indices[:int(num_split_pkts)]
    shuffle_idx = sorted(shuffle_idx)
    split_pkt_idx = []
    
    for i, idx in enumerate(shuffle_idx):
        inserted = df_benign_bi_flow.iloc[idx + i].copy()
        inserted["iat"] = 1
        inserted["pkt_len"] = 40
        inserted_df = pd.DataFrame([inserted])
        df_benign_bi_flow = pd.concat(
            [
                df_benign_bi_flow.iloc[: idx + i + 1],
               inserted_df,
                df_benign_bi_flow.iloc[idx + i + 1:],
            ], axis=0, ignore_index=True
        )
        
        split_pkt_idx.append(idx + i)
        
    # Update num_existing_pkts
    num_existing_pkts = len(df_benign_bi_flow)
    orig_pkt_len_list = df_benign_bi_flow["pkt_len"]
    orig_interarrival_list = df_benign_bi_flow["iat"]
