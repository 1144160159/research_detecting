...
def get_important_features_v2(inputs, refs):
    ...
    # tmp = [x for x in tmp if "total_fpkt" not in x]
    # tmp = [x for x in tmp if "total_bpkt" not in x]
    return tmp, indice_sort
...
def self_correct(perturbed_features, orig_features, scaler):
    torch_denorm_perturbed_features = torch_data_denormalization(
        perturbed_features, scaler
    )

    denorm_orig_features = data_denormalization(orig_features, scaler)
    denorm_orig_features = pd.DataFrame(
        denorm_orig_features, columns=list(dataset_fields.keys())
    )
    if (
        torch_denorm_perturbed_features[0, dataset_fields["total_fiat"]]
        < 1.2 * denorm_orig_features["total_fiat"]
    ):
        torch_denorm_perturbed_features[0, dataset_fields["total_fiat"]] = (
            1.2 * denorm_orig_features["total_fiat"]
        )
    ...
    if (
        torch_denorm_perturbed_features[0, dataset_fields["mean_fiat"]]
        < 1.2 * denorm_orig_features["mean_fiat"]
    ):
        torch_denorm_perturbed_features[0, dataset_fields["mean_fiat"]] = (
            1.2 * denorm_orig_features["mean_fiat"]
        )
