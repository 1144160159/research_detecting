...
def get_pgd_adv_features(orig_feature, model, loss, total_repeat_time, iters, alpha):
    ...
    for _ in range(total_repeat_time):
        ...
        for _ in range(iters):
            ...
            perturbed_feature[:, dataset_fields["total_fpkt"]] = orig_feature[
                :, dataset_fields["total_fpkt"]
            ]
            perturbed_feature[:, dataset_fields["total_bpkt"]] = orig_feature[
                :, dataset_fields["total_bpkt"]
            ]
            perturbed_feature[:, dataset_fields["total_fbyt"]] = orig_feature[
                :, dataset_fields["total_fbyt"]
            ]
            perturbed_feature[:, dataset_fields["total_bbyt"]] = orig_feature[
                :, dataset_fields["total_bbyt"]
            ]
            perturbed_feature[:, dataset_fields["min_fbyt"]] = orig_feature[
                :, dataset_fields["min_fbyt"]
            ]
            perturbed_feature[:, dataset_fields["min_bbyt"]] = orig_feature[
                :, dataset_fields["min_bbyt"]
            ]
            perturbed_feature[:, dataset_fields["max_fbyt"]] = orig_feature[
                :, dataset_fields["max_fbyt"]
            ]
            perturbed_feature[:, dataset_fields["max_bbyt"]] = orig_feature[
                :, dataset_fields["max_bbyt"]
            ]
            perturbed_feature[:, dataset_fields["mean_fbyt"]] = orig_feature[
                :, dataset_fields["mean_fbyt"]
            ]
            perturbed_feature[:, dataset_fields["mean_bbyt"]] = orig_feature[
                :, dataset_fields["mean_bbyt"]
            ]
            perturbed_feature[:, dataset_fields["std_fbyt"]] = orig_feature[
                :, dataset_fields["std_fbyt"]
            ]
            perturbed_feature[:, dataset_fields["std_bbyt"]] = orig_feature[
        ...
        added_features[:, dataset_fields["total_fiat"]] = torch.clamp(
            added_features[:, dataset_fields["total_fiat"]],
            min=orig_feature[:, dataset_fields["total_fiat"]],
            max=1.2 * orig_feature[:, dataset_fields["total_fiat"]],
        )
        added_features[:, dataset_fields["total_biat"]] = torch.clamp(
            added_features[:, dataset_fields["total_biat"]],
            min=orig_feature[:, dataset_fields["total_biat"]],
            max=1.2 * orig_feature[:, dataset_fields["total_biat"]],
        )
        added_features[:, dataset_fields["mean_fiat"]] = torch.clamp(
            added_features[:, dataset_fields["mean_fiat"]],
            min=orig_feature[:, dataset_fields["mean_fiat"]],
            max=1.2 * orig_feature[:, dataset_fields["mean_fiat"]],
        )
        added_features[:, dataset_fields["mean_biat"]] = torch.clamp(
            added_features[:, dataset_fields["mean_biat"]],
            min=orig_feature[:, dataset_fields["mean_biat"]],
            max=1.2 * orig_feature[:, dataset_fields["mean_biat"]],
        )

        added_features[:, dataset_fields["total_fpkt"]] = orig_feature[
            :, dataset_fields["total_fpkt"]
        ]
        added_features[:, dataset_fields["total_bpkt"]] = orig_feature[
            :, dataset_fields["total_bpkt"]
        ]
        added_features[:, dataset_fields["total_fbyt"]] = orig_feature[
            :, dataset_fields["total_fbyt"]
        ]
        added_features[:, dataset_fields["total_bbyt"]] = orig_feature[
            :, dataset_fields["total_bbyt"]
        ]
        added_features[:, dataset_fields["min_fbyt"]] = orig_feature[
            :, dataset_fields["min_fbyt"]
        ]
        added_features[:, dataset_fields["min_bbyt"]] = orig_feature[
            :, dataset_fields["min_bbyt"]
        ]
        added_features[:, dataset_fields["max_fbyt"]] = orig_feature[
            :, dataset_fields["max_fbyt"]
        ]
        added_features[:, dataset_fields["max_bbyt"]] = orig_feature[
            :, dataset_fields["max_bbyt"]
        ]
        added_features[:, dataset_fields["mean_fbyt"]] = orig_feature[
            :, dataset_fields["mean_fbyt"]
        ]
        added_features[:, dataset_fields["mean_bbyt"]] = orig_feature[
            :, dataset_fields["mean_bbyt"]
        ]
        added_features[:, dataset_fields["std_fbyt"]] = orig_feature[
            :, dataset_fields["std_fbyt"]
        ]
        added_features[:, dataset_fields["std_bbyt"]] = orig_feature[
            :, dataset_fields["std_bbyt"]
        ]
...
def pgd_attack_RANDOM_INIT_v3(
    ...
    for _ in range(num_restarts):
        ...
        for pgd_perturbed_feature in pgd_adv_features_list:
            ...
            # pgd_perturbed_feature = self_correct(pgd_adv_feature, orig_feature, scaler)
            (
                important_feature_list,
                important_feature_indices_list,
