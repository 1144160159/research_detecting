...
def get_pgd_adv_features(orig_feature, model, label, total_repeat_time, iters, alpha):
    ...
    labels = [i for i in range(model.n_classes_) if i != label]
    ohe_labels = np.zeros((len(labels), model.n_classes_), dtype=int)
    ohe_labels[np.arange(len(labels)), labels] = 1
    np.random.shuffle(ohe_labels)
    ...
    for i in range(total_repeat_time):
        ...
        y[0, label] = 1
        perturbed_feature = zoo.generate(x=perturbed_feature, y=y)

        # perturbed_feature = np.clip(perturbed_feature, 0.8 * orig_feature.clone().detach().numpy(), 1.2 * orig_feature.clone().detach().numpy())
        perturbed_feature[:, dataset_fields["total_fiat"]] = np.clip(
            perturbed_feature[:, dataset_fields["total_fiat"]],
            0.75 * perturbed_feature_[0, dataset_fields["total_fiat"]],
            1.25 * perturbed_feature_[0, dataset_fields["total_fiat"]],
        )
        perturbed_feature[:, dataset_fields["total_biat"]] = np.clip(
            perturbed_feature[:, dataset_fields["total_biat"]],
            0.75 * perturbed_feature_[0, dataset_fields["total_biat"]],
            1.25 * perturbed_feature_[0, dataset_fields["total_biat"]],
        )
        perturbed_feature[:, dataset_fields["mean_fiat"]] = np.clip(
            perturbed_feature[:, dataset_fields["mean_fiat"]],
            0.75 * perturbed_feature_[0, dataset_fields["mean_fiat"]],
            1.25 * perturbed_feature_[0, dataset_fields["mean_fiat"]],
        )
        perturbed_feature[:, dataset_fields["mean_biat"]] = np.clip(
            perturbed_feature[:, dataset_fields["mean_biat"]],
            0.75 * perturbed_feature_[0, dataset_fields["mean_biat"]],
            1.25 * perturbed_feature_[0, dataset_fields["mean_biat"]],
        )
        perturbed_feature[:, dataset_fields["total_fpkt"]] = perturbed_feature_[
            :, dataset_fields["total_fpkt"]
        ]
        perturbed_feature[:, dataset_fields["total_bpkt"]] = perturbed_feature_[
            :, dataset_fields["total_bpkt"]
        ]
        perturbed_feature[:, dataset_fields["total_fbyt"]] = perturbed_feature_[
            :, dataset_fields["total_fbyt"]
        ]
        perturbed_feature[:, dataset_fields["total_bbyt"]] = perturbed_feature_[
            :, dataset_fields["total_bbyt"]
        ]
        perturbed_feature[:, dataset_fields["min_fbyt"]] = perturbed_feature_[
            :, dataset_fields["min_fbyt"]
        ]
        perturbed_feature[:, dataset_fields["min_bbyt"]] = perturbed_feature_[
            :, dataset_fields["min_bbyt"]
        ]
        perturbed_feature[:, dataset_fields["max_fbyt"]] = perturbed_feature_[
            :, dataset_fields["max_fbyt"]
        ]
        perturbed_feature[:, dataset_fields["max_bbyt"]] = perturbed_feature_[
            :, dataset_fields["max_bbyt"]
        ]
        perturbed_feature[:, dataset_fields["mean_fbyt"]] = perturbed_feature_[
            :, dataset_fields["mean_fbyt"]
        ]
        perturbed_feature[:, dataset_fields["mean_bbyt"]] = perturbed_feature_[
            :, dataset_fields["mean_bbyt"]
        ]
        perturbed_feature[:, dataset_fields["std_fbyt"]] = perturbed_feature_[
            :, dataset_fields["std_fbyt"]
        ]
        perturbed_feature[:, dataset_fields["std_bbyt"]] = perturbed_feature_[
            :, dataset_fields["std_bbyt"]
        ]
...
def get_feature_and_solve(
    ...
    considerred_important_feature_indices = []
    if mode == "single":
        max_num_important_features = min(num_important_features, len(important_feature_list))
    else:
        max_num_important_features = min(num_important_features, len(important_feature_list))
    for num_important_features in range(1, max_num_important_features):
        ...
        smt_results += results
        # if len(considerred_important_features) == 5:
        #     break
...
def pgd_attack_RANDOM_INIT_v3(
    ...
    orig_fwd_pkts = (
        int(
            np.round(
                denorm_orig_features["total_fiat"] / denorm_orig_features["mean_fiat"]
            )
        )
        + 1
    )
    orig_bwd_pkts = (
        int(
            np.round(
                denorm_orig_features["total_biat"] / denorm_orig_features["mean_biat"]
            )
        )
        + 1
    )
    ...
    for _ in range(num_restarts):
        ...
        orig_label = model.predict(orig_feature)
        total_repeat_time = 5
        ...
        for pgd_perturbed_feature in pgd_adv_features_list:
            ...
            # pgd_perturbed_feature = self_correct(pgd_perturbed_feature, orig_feature, scaler)
            (
                important_feature_list,
                important_feature_indices_list,
            ...
            if only_src:
                important_feature_list = [
                    x for x in important_feature_list if "bpkt" not in x
                ]
                important_feature_list = [
                    x for x in important_feature_list if "bbyt" not in x
                ]
                important_feature_list = [
                    x for x in important_feature_list if "fpkt" not in x
                ]
                important_feature_list = [
                    x for x in important_feature_list if "fbyt" not in x
                ]
            else:
                important_feature_list = [
                    x for x in important_feature_list if "bpkt" not in x
...
parser.add_argument("-k", "--k_feat", type=int, help="number of important samples")

# Parse the arguments
args = parser.parse_args()

num_important_features = args.k_feat
...
class Data(Dataset):
    def __init__(self, X_train, y_train):
        ...
        self.y = torch.from_numpy(np.array(y_train).astype(np.long))
...
clf = RandomForestClassifier(
    max_depth=50, n_estimators=10, random_state=0, max_features=None
)
...
prediction_results = np.concatenate(prediction, axis=0)
ground_truth_results = np.concatenate(ground_truth, axis=0)
