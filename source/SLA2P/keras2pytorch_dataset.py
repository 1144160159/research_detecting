...
class trainset_pytorch_IMG(data.Dataset):
...
class trainset_pytorch_double(data.Dataset):
    def __init__(self, n_trans, train_data_original, train_data_transformed, train_labels, transform=None, target_transform=None):
        self.transform = transform
        self.target_transform = target_transform

        self.train_data_original = train_data_original  # ndarray
        self.train_data = train_data_transformed
        self.train_labels = train_labels
    ...
    def __getitem__(self, index):
        ...
        Returns:
            tuple: (image, target) where target is index of the target class.
        """
        img_original, img_transformed, target = self.train_data_original[index//self.n_trans], \
        ...
        if self.target_transform is not None:
            target = torch.from_numpy(np.array(target, np.float32, copy=False)).view(8, 1, 1)
            target = self.target_transform(target)

        return img_original, img_transformed, target

    def __len__(self):
        return len(self.train_data)

class testset_pytorch_double(data.Dataset):
    def __init__(self, test_data_original, test_data, transform=None):
        self.transform = transform
        self.test_data_original = test_data_original
    ...
    def __getitem__(self, index):
        ...
        Returns:
            tuple: (image, target) where target is index of the target class.
        """
        img_original = self.test_data_original[index]
        img_transformed = self.test_data[index]
