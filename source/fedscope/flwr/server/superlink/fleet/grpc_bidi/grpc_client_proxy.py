...
class GrpcClientProxy(ClientProxy):
    ...
    def evaluate(
        ...
        message = ServerMessage(evaluate_ins=evaluate_msg)
        logger = logging.getLogger("server")
        start = time.time()
        logger.info(f"[{start}] Sending evaluate_ins to [{self.cid}] - {message.ByteSize()} bytes")
        res_wrapper: ResWrapper = self.bridge.request(
            ins_wrapper=InsWrapper(
        ...
        client_msg: ClientMessage = res_wrapper.client_message
        logger.info(f"[{time.time()}] Received evaluate_res from [{self.cid}] - {client_msg.ByteSize()} bytes - Duration {time.time()-start}")
    ...
    def proposal_req(
        ...
        prop_ins_msg = serde.prop_ins_to_proto(ins)
        message = ServerMessage(prop_ins=prop_ins_msg)
        logger = logging.getLogger("server")
        start= time.time()
        logger.info(f"[{start}] Sending prop_ins to [{self.cid}] - {message.ByteSize()} bytes")
        ...
        client_msg: ClientMessage = res_wrapper.client_message
        logger.info(f"[{time.time()}] Received prop_res from [{self.cid}] - {client_msg.ByteSize()} bytes - Duration {time.time()-start}")
        prop_res = serde.prop_res_from_proto(client_msg.prop_res)
    
        return prop_res

    def architecture_fit(
        self,
        ins: common.ArchitectureFitIns,
        timeout: Optional[float],
        group_id: Optional[int],
    ) -> common.FitRes:
        
        architecture_ins_msg = serde.architecture_fit_ins_to_proto(ins)
        message = ServerMessage(architecture_fit_ins=architecture_ins_msg)
      
        logger = logging.getLogger("server")
        start= time.time()
        logger.info(f"[{start}] Sending architecture_ins to [{self.cid}] - {message.ByteSize()} bytes")
        ...
        client_msg: ClientMessage = res_wrapper.client_message
        logger.info(f"[{time.time()}] Received fit_res from [{self.cid}] - {client_msg.ByteSize()} bytes - Duration {time.time()-start}")
        fit_res = serde.fit_res_from_proto(client_msg.fit_res)
