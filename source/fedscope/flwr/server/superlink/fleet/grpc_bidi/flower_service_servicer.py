...
class FlowerServiceServicer(transport_pb2_grpc.FlowerServiceServicer):
    ...
    def Join(  # pylint: disable=invalid-name
      ...
      # cid: str = uuid.uuid4().hex
        #cid: str = context.peer()

        cid = (context.invocation_metadata()[1])[1]
        logger = logging.getLogger(f"server_high_level")
        logger.debug(f"Client [{context.peer()}] has ID [{cid}] {context.invocation_metadata()}")
