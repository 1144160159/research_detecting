...
class History:
    ...
    def add_data(self, server_round: int, time: float,t_loss:float,t_accuracy:float,
                 f_loss:float,f_accuracy:float, feature:List) -> None:
        """Add one loss entry (from distributed evaluation)."""
        self.data.append((server_round, time,t_loss,t_accuracy,f_loss,f_accuracy,feature))
