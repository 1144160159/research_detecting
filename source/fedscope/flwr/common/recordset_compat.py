...
def _recordset_to_fit_or_architecture_fit_or_evaluate_ins_components(
    recordset: RecordSet,
    ins_str: str,
    keep_input: bool,
) -> Tuple[Parameters, Dict[str, Scalar]]:
    """Derive Fit/Architecture Fit/Evaluate Ins from a RecordSet."""
...
def _fit_or_architecture_fit_or_evaluate_ins_to_recordset(
    ins: Union[FitIns, EvaluateIns,ArchitectureFitIns], keep_input: bool
) -> RecordSet:
    recordset = RecordSet()
    ...
    if isinstance(ins, FitIns):
        ...
        ins_str ="evaluateins"
    elif  isinstance(ins, ArchitectureFitIns):
        ins_str ="architecturefitins"

    else:
       ("Error: _fit_or_architecture_fit_or_evaluate_ins_to_recordset has received an invalid 'ins' object.")
...
def recordset_to_fitins(recordset: RecordSet, keep_input: bool) -> FitIns:
    ...
    parameters, config = _recordset_to_fit_or_architecture_fit_or_evaluate_ins_components(
        recordset,
        ins_str="fitins",
...
def fitins_to_recordset(fitins: FitIns, keep_input: bool) -> RecordSet:
    ...
    return _fit_or_architecture_fit_or_evaluate_ins_to_recordset(fitins, keep_input)
...
def recordset_to_architecture_fit_ins(recordset: RecordSet, keep_input: bool) -> ArchitectureFitIns:
        """Derive FitRes from a RecordSet object."""
        ins_str = "architecturefitins"
        parameters = parametersrecord_to_parameters(
            recordset.parameters_records[f"{ins_str}.parameters"], keep_input=keep_input
        )
        config =  recordset.configs_records[f"{ins_str}.config"]
        model_architecture = recordset.configs_records[f"{ins_str}.model_architecture"]["model_architecture"]       #workaround for pass a scalar as a dict
        features_mapping = recordset.configs_records[f"{ins_str}.features_mapping"]
        # pylint: disable-next=protected-access
        features_mapping = _check_mapping_from_recordscalartype_to_scalar(features_mapping._data)
    
      #  log(INFO,ArchitectureFitIns(
...
def architecture_fit_ins_to_recordset(architecture_ifit_ins: ArchitectureFitIns, keep_input: bool) -> RecordSet:
    """Construct a RecordSet from a ArchitectureFitIns object."""
    return _architecture_fit_ins_to_recordset(architecture_ifit_ins, keep_input)


def _architecture_fit_ins_to_recordset(
    ins:ArchitectureFitIns, keep_input: bool
) -> RecordSet:
    recordset = RecordSet()
    ins_str = "architecturefitins"
    ...
    parametersrecord = parameters_to_parametersrecord(ins.parameters, keep_input)
    recordset.parameters_records[f"{ins_str}.parameters"] = parametersrecord
    recordset.configs_records[f"{ins_str}.features_mapping"] =ConfigsRecord( ins.features_mapping)
   
    recordset.configs_records[f"{ins_str}.config"] = ConfigsRecord( ins.config)
    recordset.configs_records[f"{ins_str}.model_architecture"] = ConfigsRecord({"model_architecture":ins.model_architecture})#workaround for pass a scalar as a dict
    return recordset
...
def propins_to_recordset(propins: PropIns, keep_input: bool) -> RecordSet:
    """Construct a RecordSet from a PropIns object."""
    return _prop_ins_to_recordset(propins, keep_input)


def recordset_to_propins(recordset: RecordSet, keep_input: bool) -> PropIns :
    """Derive FitRes from a RecordSet object."""
    ins_str = "propins"

    configs_record = recordset.configs_records[f"{ins_str}.config"]
    # pylint: disable-next=protected-access
    configs_record = _check_mapping_from_recordscalartype_to_scalar(configs_record._data)

    return PropIns(
        config=configs_record
    )

def _prop_ins_to_recordset(
    ins:PropIns, keep_input: bool
) -> RecordSet:
    recordset = RecordSet()
...
def recordset_to_propres(recordset: RecordSet, keep_input: bool) -> PropRes:
    """Derive PropRes from a RecordSet object."""
    ins_str = "propres"

    configs_record = recordset.configs_records[f"{ins_str}.proposal"]
    # pylint: disable-next=protected-access
    proposal = _check_mapping_from_recordscalartype_to_scalar(configs_record._data)
...
def propres_to_recordset(propres: PropRes, keep_input: bool) -> RecordSet:
    """Construct a RecordSet from a FitRes object."""
    recordset = RecordSet()

    res_str = "propres"

    recordset.configs_records[f"{res_str}.proposal"] = ConfigsRecord(
        propres.proposal  # type: ignore
    )


    # status
    recordset = _embed_status_into_recordset(res_str, propres.status, recordset)
