...
class NumPyClient(ABC):
    ...
    def evaluate(
        ...
        _ = (self, parameters, config)
        return 0.0, 0, {}
    def proposals_req(self, config:Dict[str, Scalar])-> Dict[str,Scalar] :    
        _ = (self,config)
        return {}
    

    def architecture_fit(self,model_architecture, parameters: NDArrays, features_mapping: Dict[str, Scalar],config:Dict[str, Scalar])  -> Tuple[float, int, Dict[str, Scalar]]:

        _ = (self,model_architecture, parameters,features_mapping, config)
        return 0.0, 0, {}
...
def has_proposals_req(client: NumPyClient) -> bool:
    """Check if NumPyClient implements proposals_req(."""
    return type(client).proposals_req != NumPyClient.proposals_req


def has_architecture_fit(client: NumPyClient) -> bool:
    """Check if NumPyClient implements architecture_fit."""
    return type(client).architecture_fit != NumPyClient.architecture_fit
...
def _proposals_req(self: Client, ins: PropIns) -> PropRes:

    results = self.numpy_client.proposals_req(ins.config)  # type: ignore
...
def _architecture_fit(self: Client, ins: ArchitectureFitIns) -> FitRes:
    """Refine the provided parameters using the locally held dataset."""
    ...
    # Train
    results = self.numpy_client.architecture_fit(ins.model_architecture, parameters,ins.features_mapping,ins.config)  # type: ignore
...
def _wrap_numpy_client(client: NumPyClient) -> Client:
    ...
    if has_proposals_req(client=client):
        member_dict["proposals_req"] = _proposals_req
        
    if has_architecture_fit(client=client):
        member_dict["architecture_fit"] = _architecture_fit
