...
from flwr.client.client import (
    ...
    maybe_call_proposals_req,
    maybe_call_architecture_fit,
)
...
from flwr.common.recordset_compat import (
    ...
    recordset_to_fitins,
    recordset_to_propins,
    propres_to_recordset,
    recordset_to_getparametersins,
    recordset_to_getpropertiesins,
    recordset_to_architecture_fit_ins,
)
...
def handle_legacy_message_from_msgtype(
    ...
    # Handle PropIns 
        ...
        prop_res = maybe_call_proposals_req(
            client=client,
            prop_ins = recordset_to_propins(message.content,keep_input=True),
        )
        out_recordset = propres_to_recordset(prop_res,keep_input=False)
    # Handle ArchictureFitIns
    elif message_type == MessageType.ARCHITECTURE_TRAIN:
        
        fit_res = maybe_call_architecture_fit(
            client=client,
            architecture_ins= recordset_to_architecture_fit_ins(message.content,keep_input=True),
        )
        out_recordset = fitres_to_recordset(fit_res,keep_input=False)
