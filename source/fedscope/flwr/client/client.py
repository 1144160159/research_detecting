...
class Client(ABC):
    ...
    def proposals_req(self, ins: PropIns) -> PropRes:

        _ = (self, ins)
        return PropRes(
            status=Status(
                code=Code.PROP_NOT_IMPLEMENTED,
                message="Client does not implement `proposals_req`",
            ),
            proposal={},
        )
    
    def architecture_fit(self,architecture,parameters,features_mapping,config) -> FitRes:

       
        _ = (self,architecture, parameters,features_mapping,config)
             
        return FitRes(
            status=Status(
                code=Code.ARCHITECTURE_FIT_NOT_IMPLEMENTED,
                message="Client does not implement `architecture_fit`",
            ),
            parameters=Parameters(tensor_type="", tensors=[]),
            num_examples=0,
            metrics={},
...
def has_proposals_req(client: Client) -> bool:
    """Check if Client implements evaluate."""
    return type(client).proposals_req != Client.proposals_req


def has_architecture_fit(client: Client) -> bool:
    """Check if Client implements evaluate."""
    return type(client).architecture_fit != Client.architecture_fit
...
def maybe_call_proposals_req(client: Client, prop_ins: PropIns) -> PropRes:
    """Call `proposals_req` if the client overrides it."""
    # Check if client overrides `proposals_req`
    if not has_proposals_req(client=client):
        # If client does not override `proposals_req`, don't call it
        status = Status(
            code=Code.PROP_NOT_IMPLEMENTED,
            message="Client does not implement `proposals_req`",
        )
    ...
    # If the client implements `evaluate`, call it
    return client.proposals_req(prop_ins)


def maybe_call_architecture_fit(client: Client, architecture_ins: ArchitectureFitIns) -> FitRes:
    """Call `architecture_fit` if the client overrides it."""
    # Check if client overrides `architecture_fit`
    if not has_architecture_fit(client=client):
        # If client does not override `architecture_fit`, don't call it
        status = Status(
            code=Code.ARCHITECTURE_FIT_NOT_IMPLEMENTED,
            message="Client does not implement `architecture_fit`",
        )
    ...
    # If the client implements `architecture_fit`, call it
    return client.architecture_fit(architecture_ins)
