...
def grpc_connection(  # pylint: disable=R0915
    ...
    server_message_iterator: Iterator[ServerMessage] = stub.Join(iter(queue.get, None),metadata=[('id',f"{client_id}")])

    def receive(client_id: Optional[str] =None) -> Message:
        # Receive ServerMessage proto
        #print("message",server_message_iterator)
        ...
        if client_id is not None:
            loggerFile = logging.getLogger(f"client_{client_id}")
            loggerFile.info(f"[{time.time()}] Received {field} - {proto.ByteSize()} bytes")
        ...
        if field == "get_properties_ins":
            ...
            recordset = compat.propins_to_recordset(
                serde.prop_ins_from_proto(proto.prop_ins), False
            )
            message_type = MessageType.PROPOSALS
        elif field == "architecture_fit_ins":
            
            recordset = compat.architecture_fit_ins_to_recordset(
                serde.architecture_fit_ins_from_proto(proto.architecture_fit_ins), False
            )
            
            message_type = MessageType.ARCHITECTURE_TRAIN
        else:
    ...
    def send(message: Message,
             client_id: Optional[str] =None     #Used for logging
             ) -> None:
        ...
        elif message_type== MessageType.ARCHITECTURE_TRAIN:
            fitres = compat.recordset_to_fitres(recordset, False)
            msg_proto = ClientMessage(fit_res=serde.fit_res_to_proto(fitres))
        ...
        elif message_type == MessageType.PROPOSALS:
            propres = compat.recordset_to_propres(recordset, False)
            msg_proto = ClientMessage(prop_res=serde.prop_res_to_proto(propres))
        ...
        if client_id is not None:
            loggerFile = logging.getLogger(f"client_{client_id}")
            loggerFile.info(f"[{time.time()}] Sending {message_type} - {msg_proto.ByteSize()} bytes ")
