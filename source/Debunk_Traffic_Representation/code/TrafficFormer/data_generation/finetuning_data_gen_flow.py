...
def process_one_label(session_pcap_path,key,payload_length,payload_packet,samples,label_id,start_index=76):
    ...
    os.makedirs("/home/dauin_user/yzhao/temp/",exist_ok=True)
    with open("/home/dauin_user/yzhao/temp/"+key,'wb') as f:
...
def generation_multiP(pcap_path, samples, dataset_save_path, payload_length = 64, payload_packet = 5, start_index=76):
    ...
    for key in os.listdir("/home/dauin_user/yzhao/temp/"):
        with open("/home/dauin_user/yzhao/temp/"+key,'rb') as f:
            result = pickle.load(f)
        dataset[label_id[key]] = result
        os.system(f'rm -r {"/home/dauin_user/yzhao/temp/"+key}')
...
def dataset_extract(dataset_save_path, to_path, features, category):
    
    print("read dataset from json file.")
    with open(dataset_save_path + "dataset.json","r") as f:
        dataset = json.load(f)
    
    dataset_statistic = [0] * category

    data_all = []
    for app_label in dataset.keys():
        for index_sample in range(len(dataset[app_label]["datagram"])):
            x = []
    ...
    # save bytes to tsv
    write_dataset_tsv(data_train['datagram'], data_train['label'], to_path, "train")

    print("finish generating pre-train's datagram dataset.\nPlease check in %s" % to_path)
