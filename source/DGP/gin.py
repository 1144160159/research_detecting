...
class Encoder(torch.nn.Module):
    ...
    def forward(self, x, edge_index, batch, notfinal=False, edge_weight=None):
        if x is None:
        ...
        for i in range(self.num_gc_layers):
            ...
            x = self.bns[i](x)
            xs.append(x) # [(N * M, F), (N * M, F), (N * M, F), ...]

        xpool = [global_add_pool(x, batch) for x in xs] # [(N, F), (N, F), (N, F), ...]
        x = torch.cat(xpool, 1) # (N, F * num_gc_layers)

        return x, torch.cat(xs, 1) # (M * N, F * num_gc_layers)

    def get_embeddings(self, loader, device=None, is_batch=False, edge_weight=None):
        ...
        with torch.no_grad():
            for data in loader:
                ...
                if x is None:
                    x = torch.ones((batch.shape[0],1)).to(device)
                x, node_x = self.forward(x, edge_index, batch, notfinal=True, edge_weight=edge_weight)

                ret.append(x.cpu().numpy())
                y.append(data.y.cpu().numpy())
                node_emb.append(node_x)
        if is_batch:
            return node_emb, ret, y
