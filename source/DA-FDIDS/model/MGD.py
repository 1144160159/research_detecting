...
class ICI(object):
    ...
    def logit(self, support_set, X_hat, y_hat, way, num_support, pseudo_y):
        clf = LogisticRegression(penalty='l1', solver='saga', C=self.logit_penalty)
        clf.fit(X_hat[:num_support], np.argmax(y_hat[:num_support], axis=1))
        
        # 获取未标记样本的决策分数
        scores = clf.decision_function(X_hat[num_support:])
        confidence = np.max(scores, axis=1)  # 最高类别得分
        
        selected = np.zeros(way)
        sorted_indices = np.argsort(-confidence)
        for i in sorted_indices:
            if (i + num_support not in support_set) and \
