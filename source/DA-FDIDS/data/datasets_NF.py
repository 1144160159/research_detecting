...
def count_attacks_and_filter(csv_file, output_file):
    ...
    mask = ~((df[columns_to_check] == '0.0.0.0').any(axis=1) | (df[columns_to_check] == 0).any(axis=1) | (df[columns_to_check] == 0.0).any(axis=1) | (df[columns_to_check].isna()).any(axis=1))
    df_filtered = df[mask]
    attack_counts = df_filtered['Attack'].value_counts()
    df_filtered.to_csv(output_file, index=False)

    return attack_counts
#input_csv = "./data/NF-CSE-CIC-IDS2018-v2.csv"
#output_csv = "./data/NF-CSE-CIC-IDS2018-v2_dealed.csv"
...
class My_Dataset(Dataset, ABC):
    ...
    def raw_file_names(self):
        #return 'NF-CSE-CIC-IDS2018-v2.csv'
        return 'NF-UNSW-NB15-v2_dealed.csv'

    @property
    def processed_file_names(self):
        #return 'data/NF-CSE-CIC-IDS2018-v2.pt'
    ...
    def process(self):
        ...
        if os.path.exists(f'./data/{self.processed_file_names}'):
            ...
            #time
            num_records = len(df)
            random_seconds = np.arange(num_records).tolist()                 # 改动1: 用行序号替换随机数
            random_time_stamps = [datetime.datetime(1970, 1, 1) + datetime.timedelta(seconds=i) for i in random_seconds]
            random_time_stamps_str = [dt.strftime('%d/%m/%Y %I:%M:%S %p') for dt in random_time_stamps]
            ...
            for name in opt:
                ...
                s = pd.to_numeric(df[name], errors='coerce')          # 先赋值
                s = s.replace([np.inf, -np.inf], np.nan).fillna(0)    # 再清理
            ...
            df.insert(5, 'layer_i', 0)
            df.loc[src_matches, 'layer_i'] = 1
            df.insert(6, 'layer_j', 0)
            ...
            src_layer = torch.tensor(df['layer_i'].values.tolist())
            dst_layer = torch.tensor(df['layer_j'].values.tolist())
            ...
            #torch.save(events, './data/NF-CSE-CIC-IDS2018-v2.pt')
