...
class My_Dataset(Dataset, ABC):
    ...
    def process(self):
        if os.path.exists(f'./data/{self.processed_file_names}'):
            ...
            # origin_nodelist 已经在你前面构好了：origin_nodelist[i] = [src_i, dst_i]
            node_to_eids = defaultdict(list)
            for eid, (s_id, d_id) in enumerate(origin_nodelist):
                node_to_eids[s_id].append(eid)
                node_to_eids[d_id].append(eid)

            # 用集合去重，避免同一对事件因共享src和dst被重复加入
            pair_set = set()
