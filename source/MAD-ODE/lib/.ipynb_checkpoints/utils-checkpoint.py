...
class StandardScaler:
    ...
    def inverse_transform(self, data):  #去归一化
...
def load_dataset(dataset_dir, batch_size, test_batch_size=None, **kwargs):
    ...
    for category in ['train', 'val', 'test']:
        print(category)
        a = os.path.join(dataset_dir, category + '.npz')
...
def load_dataset_other(dataset_dir, batch_size, test_batch_size=None, **kwargs):
    data = {}
    for category in ['train', 'val', 'test']:
        a = '{}/{}.npz'.format(dataset_dir,category)
        cat_data = np.load('{}/{}.npz'.format(dataset_dir,category),allow_pickle=True)
        data['x_' + category] = cat_data['x']
        data['y_' + category] = cat_data['y']
    ...
    for category in ['train', 'val', 'test']:
        data['x_' + category][..., 0] = scaler.transform(data['x_' + category][..., 0])     #归一化
        data['y_' + category][..., 0] = scaler.transform(data['y_' + category][..., 0])
    data['train_loader'] = DataLoader(data['x_train'], data['y_train'],batch_size, shuffle=True,labels=data['labels_train'] )
    data['val_loader'] = DataLoader(data['x_val'], data['y_val'],test_batch_size, shuffle=False,labels=data['labels_val'] )
    data['test_loader'] = DataLoader(data['x_test'], data['y_test'] ,test_batch_size, shuffle=False,labels=data['labels_test'])
    data['scaler'] = scaler
