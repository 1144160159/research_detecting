...
def search_best_score(score: np.array, label: np.array, divide_num: int = 1000):
    ...
    pre = np.array(pre)
    rec = np.array(rec)
    return pre[fscore.index(max(fscore))], rec[fscore.index(max(fscore))], max(fscore), fscore.index(
        max(fscore)) / divide_num
...
#     from sklearn.metrics import precision_score, recall_score, f1_score
#
#     max_sco = np.max(proba)
#     min_sco = np.min(proba)
#
...
#     for item in range(round(min_sco * 100), round(max_sco * 100), 1):
#         thr = item / 100
...
def evaluation(true_label, proba, threshold=0):
    ...
    # 直接基于分数计算 AUC
    try:
        roc_auc = roc_auc_score(true_label, new_proba)
    except ValueError:
        roc_auc = np.nan  # 若 y 只有单一类别会报错

    try:
        ap = average_precision_score(true_label, new_proba)
    except ValueError:
        ap = np.nan

    # 可选：也能显式计算 PR 曲线并用 auc 积分（与 AP 略有差异）
    precision_arr, recall_arr, _ = precision_recall_curve(true_label, new_proba)
    pr_auc = auc(recall_arr, precision_arr)
    
    plot_roc=True
    save_path=r"./tupian/wadi.jpg"
    title_prefix="WADI"
    if plot_roc:
        try:
            fpr, tpr, _ = roc_curve(true_label, new_proba)
        except ValueError:
            fpr, tpr = np.array([0, 1]), np.array([0, 1])

        ncols = 2
        fig, axes = plt.subplots(1, ncols, figsize=(12, 5), dpi=120)

        # ROC
        ax = axes[0]
        ax.plot(fpr, tpr, color="#1f77b4", lw=2, label=f"AUC = {roc_auc:.4f}" if not np.isnan(roc_auc) else "AUC = n/a")
        ...
        # PR
        ax2 = axes[1]
        ax2.plot(recall_arr, precision_arr, color="#d62728", lw=2, label=f"PR AUC = {pr_auc:.4f} | AP = {ap:.4f}")
    ...
    # 若需要沿阈值搜索最佳F1，保留你的逻辑
    min_sco = float(np.min(proba))
    pre, rec, fsc = [], [], []

    if threshold:
        predict_label = (new_proba >= threshold).astype(np.int32)
        precision = precision_score(true_label, predict_label, zero_division=0) * 100
        recall = recall_score(true_label, predict_label, zero_division=0) * 100
        ...
        return precision, recall, f_score, threshold, roc_auc, ap, pr_auc

    # 阈值搜索范围与你原代码一致（单位：0.01）
    for item in range(round(min_sco * 100), 10000, 1):
        thr = item / 100.0
        predict_label = (new_proba >= thr).astype(np.int32)
        precision = precision_score(true_label, predict_label, zero_division=0) * 100
        recall = recall_score(true_label, predict_label, zero_division=0) * 100
        f_score = 0 if precision + recall == 0 else 2 * precision * recall / (precision + recall)
        pre.append(precision); rec.append(recall); fsc.append(f_score)

    # best_idx = int(np.argmax(fsc))
    # best_score = fsc[best_idx]
    # best_pre = pre[best_idx]
    # best_rec = rec[best_idx]
    # best_thr = (round(min_sco * 100) + best_idx) / 100.0
    ...
    return best_pre, best_rec, best_score, best_thr, roc_auc, ap, pr_auc
