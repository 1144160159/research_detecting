...
def generate_graph_seq2seq_io_data(
    ...
    for t in range(min_t, max_t):
        ...
        labels_t = labels_t.reshape(min_t+1)
        x.append(x_t)
        y.append(y_t)
...
def generate_train_val_test(args):
    ...
    x_offsets = np.sort(
        ...
        np.concatenate((np.arange(-14, 1, 1),))
    )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--output_dir", type=str, default="../data/smap", help="Output directory."
    )
    parser.add_argument(
        "--train_df_filename",
        type=str,
        default="../data/smap/train.csv",
        help="Raw traffic readings.",
    )
    parser.add_argument(
        "--test_df_filename",
        type=str,
        default="../data/smap/test.csv",
        help="Raw traffic readings.",
    )
