...
METHODS = ("grood", "mlp_energy", "opendetect")
...
def analyze(pilot_root, source_root, opendetect_root, gate):
    ...
    if gate.get("schema_version") != "strict_v4_mlp_grood_expansion_gate_v1":
        raise ValueError("unexpected GROOD gate schema")
    if gate.get("manifest_sha256") != canonical_hash(gate):
        raise ValueError("GROOD gate SHA mismatch")
    if gate.get("pilot_protocol_manifest_sha256") != protocol.get("manifest_sha256"):
        raise ValueError("GROOD gate protocol binding mismatch")
    blocks = []
            ...
            blocks.append(
                {
                    ...
                    "scenario": scenario,
                    "reports": {
                        "grood": _report(payloads["new"], "grood", paths["new"]),
    ...
    for metric in UNKNOWN_METRICS:
        values = []
        for block in blocks:
            candidate = block["reports"]["grood"][metric]
    ...
    suite_gains = {suite: sum(v) / len(v) for suite, v in sorted(by_suite.items())}
    candidate_row = next(row for row in overall if row["method"] == "grood")
    known_differences = [
        block["reports"]["grood"]["known_macro_f1"]
        - block["reports"]["mlp_energy"]["known_macro_f1"]
        for block in blocks
    ]
    known_mean = sum(known_differences) / len(known_differences)
    known_worst = min(known_differences)
    checks = {
        "pilot_runs_complete": len(blocks) == 14 and not failures,
        "split_integrity": True,
        "known_f1_tolerance": known_mean >= -0.01 and known_worst >= -0.05,
    ...
    return {
        "schema_version": "strict_v4_mlp_grood_pilot_analysis_v1",
        ...
        "overall": overall,
        "grood_vs_mlp_energy_oriented_mean_gains": gains,
        "grood_vs_mlp_energy_four_metric_mean_gain": sum(gains.values()) / 4.0,
        "grood_vs_mlp_energy_suite_mean_gains": suite_gains,
        "known_f1_mean_difference": known_mean,
        "known_f1_worst_scenario_difference": known_worst,
        "expansion_checks": checks,
        "decision": {"expand_grood_to_full102": all(checks.values())},
    }


def render(result):
    lines = [
        "# Strict-v4 GROOD pilot analysis",
        "",
        "Expand GROOD to full 102: `%s`."
        % ("YES" if result["decision"]["expand_grood_to_full102"] else "NO"),
        "",
