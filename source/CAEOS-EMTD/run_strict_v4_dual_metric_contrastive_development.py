...
def task_command(
    ...
    arguments = {
        ...
        "attack_loss_weight": "attack-loss-weight",
        "knownness_loss_weight": "knownness-loss-weight",
        "family_contrastive_loss_weight": (
            "family-contrastive-loss-weight"
        ),
        "attack_contrastive_loss_weight": (
            "attack-contrastive-loss-weight"
        ),
        "pseudo_mix_loss_weight": "pseudo-mix-loss-weight",
        "episodic_margin_loss_weight": "episodic-margin-loss-weight",
        "contrastive_temperature": "contrastive-temperature",
        "pseudo_mix_lambda": "pseudo-mix-lambda",
        "cosine_scale": "cosine-scale",
        "known_similarity_margin": "known-similarity-margin",
        "pseudo_unknown_similarity_margin": (
            "pseudo-unknown-similarity-margin"
        ),
        "early_stopping_patience": "early-stopping-patience",
        "minimum_improvement": "minimum-improvement",
    }
    for key, flag in arguments.items():
        values.extend((f"--{flag}", str(training[key])))
    values.extend(
        (
...
def verify_completed_task(
    ...
    metrics = load_canonical(
        output_dir / "metrics.json", "DMC-CAEOS task metrics"
    )
    gpu = load_canonical(
        output_dir / "gpu_execution.json", "DMC-CAEOS GPU evidence"
    )
    if (
        ...
        raise ValueError(f"DMC-CAEOS evidence failed for {unknown_family}")
    artifact_sha256 = {
...
def run(args: argparse.Namespace) -> dict[str, Any]:
    ...
    protocol = load_canonical(protocol_path, "DMC-CAEOS protocol")
    if protocol.get("state") != "frozen_development_protocol":
        raise ValueError("DMC-CAEOS protocol is not frozen")
    ...
    dataset = Path(protocol["sequence_dataset"]["path"])
    if file_hash(dataset) != protocol["sequence_dataset"]["sha256"]:
        raise ValueError("DMC-CAEOS dataset drifted after freeze")
        ...
        with concurrent.futures.ThreadPoolExecutor(
            ...
            for future in concurrent.futures.as_completed(futures):
                ...
                progress: dict[str, Any] = {
                    "schema_version": (
                        "strict_v4_dual_metric_contrastive_progress_v1"
                    ),
                    "state": "running",
    ...
    completion: dict[str, Any] = {
        "schema_version": (
            "strict_v4_dual_metric_contrastive_completion_v1"
        ),
    ...
    if not passed:
        raise RuntimeError(f"DMC-CAEOS development failed: {failures}")
    return completion
