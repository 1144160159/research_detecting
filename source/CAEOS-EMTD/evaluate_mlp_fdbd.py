...
def parse_arguments() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Evaluate official fDBD from a frozen strict MLP run")
...
def main() -> None:
    ...
    with np.load(source_paths["scores.npz"], allow_pickle=False) as source_scores:
        ...
        if arguments.get("dataset") != "tabular":
            raise ValueError("strict fDBD matrix only accepts tabular runs")
    ...
    calibrator.fit(train["embedding"], weights)
    ...
    result: dict[str, Any] = {
        "schema_version": "strict_v4_mlp_fdbd_v1", "model": "mlp_fdbd", "method": METHOD,
        ...
        "implementation": "official hyperparameter-free fDBD on frozen tabular MLP",
        "selection_evidence": {
            "protocol": "strict_frozen_checkpoint_official_fdbd",
        ...
        "diagnostics": {
            ...
            "validation_feature_deviation_mean": float(validation_output["feature_deviation"].mean()),
            "test_feature_deviation_mean": float(test_output["feature_deviation"].mean()),
        },
    ...
    np.savez_compressed(
        ...
        validation_fdbd=validation_output["risk"], test_fdbd=test_output["risk"],
    )
    provenance = {
        "schema_version": "strict_v4_mlp_fdbd_provenance_v1", "source_run": str(source_run),
        "source_artifact_sha256": source_sha256, "implementation_sha256": sha256_file(Path(__file__).resolve()),
        "scorer_module_sha256": sha256_file(Path(__file__).resolve().parent / "caeos" / "fdbd_posthoc.py"),
