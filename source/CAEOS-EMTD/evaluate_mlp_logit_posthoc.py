...
def parse_arguments() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Evaluate GEN and Shannon entropy from a frozen strict MLP run"
    )
...
def main() -> None:
    ...
    with np.load(source_paths["scores.npz"], allow_pickle=False) as source_scores:
        ...
        if arguments.get("dataset") != "tabular":
            raise ValueError("strict logit post-hoc matrix only accepts tabular runs")
    ...
    validation_risks = {
        "gen": generalized_entropy_risk(validation["logits"], args.gamma, args.top_m),
        "shannon_entropy": shannon_entropy_risk(validation["logits"]),
    }
    test_risks = {
        "gen": generalized_entropy_risk(test["logits"], args.gamma, args.top_m),
    ...
    result = {
        "schema_version": "strict_v4_mlp_logit_posthoc_v1",
        "model": "mlp_logit_posthoc",
        "method": "mlp_logit_posthoc_suite",
        ...
        "reports": reports,
        "implementation": (
            "Official GEN generalized-entropy formula and a Shannon-entropy "
            "diagnostic applied to frozen shared-MLP logits"
        ),
        "selection_evidence": {
            "protocol": "strict_frozen_checkpoint_logit_posthoc",
            "source_run": str(source_run),
            "source_artifact_sha256": source_sha256,
            "postprocessors": evidence(args.gamma, args.top_m),
    ...
    for name in METHODS:
        archive[f"validation_{name}"] = validation_risks[name]
        archive[f"test_{name}"] = test_risks[name]
    np.savez_compressed(args.output_dir / "scores.npz", **archive)
    provenance = {
        "schema_version": "strict_v4_mlp_logit_posthoc_provenance_v1",
        "source_run": str(source_run),
        "source_artifact_sha256": source_sha256,
        "implementation_sha256": sha256_file(Path(__file__).resolve()),
        "scorer_module_sha256": sha256_file(
            Path(__file__).resolve().parent / "caeos" / "logit_posthoc.py"
        ),
