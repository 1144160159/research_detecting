...
def task_command(
    ...
    return [
        str(python.resolve()),
        str(project_root / "train_strict_v4_flow_statistic_xgboost_task_cuda.py"),
        "--sequence-dataset",
...
def run_one(
    *,
    ...
    if completed.returncode != 0:
        raise RuntimeError(
            f"XGBoost task {unknown_family} exited {completed.returncode}"
        )
    metrics = load_canonical(output_dir / "metrics.json", "XGBoost task metrics")
    gpu = load_canonical(output_dir / "gpu_execution.json", "XGBoost GPU evidence")
    if (
        ...
        raise ValueError(f"XGBoost CUDA evidence failed for {unknown_family}")
    artifact_sha256 = {
...
def run(args: argparse.Namespace) -> dict[str, Any]:
    ...
    protocol = load_canonical(protocol_path, "FSX-CAEOS protocol")
        ...
        with concurrent.futures.ThreadPoolExecutor(
            ...
            for future in concurrent.futures.as_completed(futures):
                ...
                progress: dict[str, Any] = {
                    "schema_version": "strict_v4_fsx_caeos_progress_v1",
                    "state": "running",
    ...
    completion: dict[str, Any] = {
        "schema_version": "strict_v4_flow_statistic_xgboost_completion_v1",
    ...
    if not passed:
        raise RuntimeError(f"FSX-CAEOS development failed: {failures}")
    return completion
