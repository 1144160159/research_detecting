...
def run_task(
    *,
    ...
    if missing:
        raise FileNotFoundError(f"missing binary artifacts: {missing}")
    evidence = load_canonical(
        output_dir / "gpu_execution.json", "binary GPU evidence"
    )
    if (
        ...
        raise ValueError("binary task lacks valid CUDA evidence")
    return {
...
def run_protocol(
    *,
    ...
    completion: dict[str, Any] = {
        "schema_version": (
            "strict_v4_fine_binary_warning_development_completion_v1"
        ),
        "state": (
