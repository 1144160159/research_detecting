...
from caeos.npos import ClassFeatureQueues, NPOSClassifier
from train_multiclass import choose_device, move_batch, set_seed, weighted_sampler


def parse_arguments() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Strict known-only NPOS baseline")
    ...
    parser.add_argument("--minimum-queue", type=int, default=20)
    parser.add_argument("--synthesis-neighbors", type=int, default=20)
    parser.add_argument("--boundary-count", type=int, default=20)
    parser.add_argument("--noise-count", type=int, default=64)
    parser.add_argument("--outliers-per-class", type=int, default=2)
    parser.add_argument("--covariance-scale", type=float, default=0.1)
    parser.add_argument("--outlier-loss-weight", type=float, default=0.1)
    parser.add_argument("--evaluation-neighbors", type=int, default=100)
...
@torch.no_grad()
def collect(model: NPOSClassifier, loader: DataLoader, device: torch.device) -> dict[str, np.ndarray]:
    model.eval()
    ...
    for batch in loader:
        ...
        for name in ("logits", "embedding", "id_logit"):
            values[name].append(output[name].cpu().numpy())
    return {name: np.concatenate(parts) for name, parts in values.items()}


def train(
    model: NPOSClassifier,
    train_loader: DataLoader,
    validation_loader: DataLoader,
    class_count: int,
    device: torch.device,
    args: argparse.Namespace,
) -> tuple[list[dict[str, object]], ClassFeatureQueues]:
    queues = ClassFeatureQueues(class_count, args.queue_size)
    ...
    for epoch in range(args.epochs):
        ...
        for batch in train_loader:
            ...
            queues.update(output["embedding"].float(), labels)
            outlier_loss = torch.zeros((), device=device)
            if epoch + 1 >= args.start_epoch and queues.ready(args.minimum_queue):
                with torch.no_grad():
                    synthetic = queues.synthesize(
                        minimum=args.minimum_queue,
                        neighbors=args.synthesis_neighbors,
                        boundary_count=args.boundary_count,
                        noise_count=args.noise_count,
                        outlier_count=args.outliers_per_class,
                        covariance_scale=args.covariance_scale,
                    ).detach()
                with torch.cuda.amp.autocast(enabled=amp):
                    id_logits = torch.cat(
                        [output["id_logit"], model.id_logits(synthetic)], dim=0
                    )
                    targets = torch.cat(
                        [
                            torch.ones(len(output["id_logit"]), device=device),
                            torch.zeros(len(synthetic), device=device),
                        ]
                    )
                    outlier_loss = torch.nn.functional.binary_cross_entropy_with_logits(
                        id_logits, targets
                    )
                synthesis_batches += 1
            ...
            total_classification += float(classification.detach().cpu())
            total_outlier += float(outlier_loss.detach().cpu())
            batches += 1
        ...
        record = {
            ...
            "outlier_loss": total_outlier / max(1, synthesis_batches),
        ...
        print(
            "npos_epoch=%d loss=%.6f validation_macro_f1=%.6f synthesis_batches=%d"
...
def main() -> None:
    ...
    if args.start_epoch < 1 or args.start_epoch > args.epochs:
        raise ValueError("NPOS start epoch must be inside the training budget")
    if args.minimum_queue > args.queue_size:
        raise ValueError("NPOS minimum queue cannot exceed queue size")
    set_seed(args.seed)
    ...
    if args.dataset == "synthetic":
        ...
        if not args.csv or not args.config:
            raise ValueError("NPOS tabular training requires --csv and --config")
    ...
    knn = KnownKnnDistance(args.evaluation_neighbors)
    knn.fit(normalize_numpy(train_values["embedding"]))
    validation_knn = knn.score(normalize_numpy(validation["embedding"]))
    test_knn = knn.score(normalize_numpy(test["embedding"]))
    knn_threshold, knn_report = open_set_report(
        test["labels"],
        unknown,
        prediction,
        validation_knn,
    ...
    validation_head = -validation["id_logit"]
    test_head = -test["id_logit"]
    ...
    result = {
        "schema_version": "strict_v4_npos_metrics_v1",
        "model": "npos",
        "method": "npos_knn",
        ...
        "validation_thresholds": {
            "npos_knn": knn_threshold,
            "npos_auxiliary_head": head_threshold,
        },
        "reports": {"npos_knn": knn_report},
        "auxiliary_reports": {"npos_auxiliary_head": head_report},
        "training_history": history,
        "training_seconds": training_seconds,
        "trainable_parameters": count_trainable_parameters(model),
        "implementation": (
            "Official NPOS class queues, non-parametric KNN boundary sampling, "
            "Gaussian candidate perturbation, auxiliary ID/OOD loss, and normalized "
            "KNN evaluation adapted to shared tabular side-channel views"
        ),
        "npos_evidence": {
            "paper": "https://openreview.net/forum?id=JHklpEZqduQ",
            "official_code": "https://github.com/deeplearning-wisc/npos",
            "official_commit": "583c06db0876c3d1c4e5a9a5371cc3a5cb916255",
            "fit_split": "known_only_train",
            "checkpoint_selection": "fixed_prefrozen_training_budget",
            "primary_score": "normalized_penultimate_feature_kth_neighbor_distance",
    ...
    np.savez_compressed(
        ...
        validation_npos_knn=validation_knn,
        test_labels=test["labels"],
        test_unknown=unknown,
        test_npos_knn=test_knn,
        prediction_npos_knn=prediction,
        validation_npos_auxiliary_head=validation_head,
        test_npos_auxiliary_head=test_head,
    )
    torch.save(
        {
    ...
    print(json.dumps({"npos_knn": knn_report}, sort_keys=True), flush=True)
