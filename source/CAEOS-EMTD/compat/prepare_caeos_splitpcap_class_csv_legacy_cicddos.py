...
def capture_processing_policy(
    policy: dict[str, Any], repair: dict[str, Any] | None
) -> dict[str, Any]:
    if repair is None:
        return policy
    ...
    effective["capture_specific_pcap_repair"] = {
        ...
        "full_tshark_scan_passed": bool(repair.get("full_tshark_scan_passed", False)),
    }
    return effective
...
def process_original_capture(
    ...
    if marker_path.exists():
        ...
        verify_reusable_capture(
            marker, canonical_json_hash(schema), canonical_json_hash(effective_policy)
        )
        marker["reused"] = True
        return marker
