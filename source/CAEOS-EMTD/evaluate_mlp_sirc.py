...
from caeos.sirc_posthoc import METHODS, SIRCMSPFixedCalibrator
...
def parse_arguments() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Evaluate fixed SIRC-MSP variants from a frozen strict MLP run")
...
def main() -> None:
    ...
    with np.load(source_paths["scores.npz"], allow_pickle=False) as source_scores:
        ...
        if arguments.get("dataset") != "tabular":
            raise ValueError("strict SIRC matrix only accepts tabular runs")
    ...
    for method in METHODS:
        thresholds[method], reports[method] = report_for(
            ...
            validation_output[method], test_output[method], acceptance,
        )
    ...
    result: dict[str, Any] = {
        "schema_version": "strict_v4_mlp_sirc_msp_fixed_v1", "model": "mlp_sirc_msp_fixed",
        "method": "mlp_sirc_msp_fixed_suite", "unknown_classes": unknown_classes,
        ...
        "implementation": "Official SIRC equation with MSP and predeclared L1/residual auxiliaries fitted on known training data",
        "selection_evidence": {
            "protocol": "strict_frozen_checkpoint_sirc_msp_fixed_no_ood_selection",
        ...
        "diagnostics": {
            method: {
                "validation_risk_std": float(np.std(validation_output[method])),
                "test_risk_std": float(np.std(test_output[method])),
                "validation_auxiliary_std": float(np.std(validation_output[method + "_auxiliary"])),
                "test_auxiliary_std": float(np.std(test_output[method + "_auxiliary"])),
            }
            for method in METHODS
        },
    ...
    for method in METHODS:
        score_archive["validation_" + method] = validation_output[method]
        score_archive["test_" + method] = test_output[method]
    np.savez_compressed(args.output_dir / "scores.npz", **score_archive)
    provenance = {
        "schema_version": "strict_v4_mlp_sirc_msp_fixed_provenance_v1", "source_run": str(source_run),
        "source_artifact_sha256": source_sha256, "implementation_sha256": sha256_file(Path(__file__).resolve()),
        "scorer_module_sha256": sha256_file(Path(__file__).resolve().parent / "caeos" / "sirc_posthoc.py"),
