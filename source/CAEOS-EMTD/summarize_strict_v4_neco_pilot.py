...
METHODS = ("neco_id90", "mlp_energy", "opendetect")
...
def analyze(pilot_root: Path, source_root: Path, opendetect_root: Path, gate: dict[str, Any]) -> dict[str, Any]:
    ...
    if gate.get("schema_version") != "strict_v4_mlp_neco_id90_expansion_gate_v1":
        raise ValueError("unexpected NECO-ID90 expansion gate schema")
    if gate.get("manifest_sha256") != canonical_hash(gate):
        raise ValueError("NECO-ID90 expansion gate SHA mismatch")
    if gate.get("pilot_protocol_manifest_sha256") != protocol.get("manifest_sha256"):
        raise ValueError("NECO-ID90 expansion gate protocol binding mismatch")
    blocks = []
    ...
    for suite, scenarios in protocol["selected_scenarios"].items():
        for scenario in scenarios:
            ...
            dimensions["%s/%s" % (suite, scenario)] = {
                "selected": int(evidence["selected_dimension"]),
                "embedding": int(evidence["embedding_dimension"]),
                "cumulative_explained_variance": float(evidence["selected_cumulative_explained_variance"]),
            }
            blocks.append({
                "suite": suite, "scenario": scenario,
                "reports": {
                    "neco_id90": _report(candidate, "neco_id90", candidate_path),
    ...
    for metric in UNKNOWN_METRICS:
        gains = []
        for block in blocks:
            candidate = block["reports"]["neco_id90"][metric]
    ...
    candidate_row = next(row for row in overall if row["method"] == "neco_id90")
    known_f1_max_difference = max(
        abs(block["reports"]["neco_id90"]["known_macro_f1"] - block["reports"]["mlp_energy"]["known_macro_f1"])
        for block in blocks
    )
    checks = {
        ...
        "nondegenerate_dimension": all(item["selected"] < item["embedding"] for item in dimensions.values()),
    ...
    return {
        "schema_version": "strict_v4_mlp_neco_id90_pilot_analysis_v1",
        ...
        "neco_id90_vs_mlp_energy_oriented_mean_gains": oriented_gains,
        "neco_id90_vs_mlp_energy_four_metric_mean_gain": sum(oriented_gains.values()) / len(oriented_gains),
        "neco_id90_vs_mlp_energy_suite_mean_gains": suite_gains,
        "known_f1_max_absolute_difference": known_f1_max_difference,
        "expansion_checks": checks, "decision": {"expand_neco_id90_to_full102": all(checks.values())},
    }
...
def render(result: dict[str, Any]) -> str:
    lines = [
        ...
        "Expand NECO-ID90 to full 102: `%s`." % ("YES" if result["decision"]["expand_neco_id90_to_full102"] else "NO"), "",
