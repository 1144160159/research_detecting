...
def evaluate(
    ...
    *,
    ...
    if (
        ...
        raise ValueError("protocol-bound canonical CSR v4 design required")
    if condition not in ("clean", *FAMILIES):
        raise ValueError("unsupported CSR exact-replay condition")
    capture_path = capture_dir / "repair_capture_manifest.json"
    ...
    if (
        ...
        or capture.get("runtime_revision")
        != "exact_clean_probability_replay_v2"
        or capture.get("suite") != suite
        or capture.get("scenario") != scenario
        or capture.get("repair_protocol_manifest_sha256")
        != protocol["manifest_sha256"]
        or capture.get("source_capture_manifest_file_sha256")
        != protocol["source_capture_manifest_file_sha256"][identity]
    ):
        raise ValueError("valid protocol-bound exact-replay capture required")
    ...
    if (
        ...
        raise ValueError("exact-replay capture artifact hash mismatch")
    runtime = joblib.load(artifact)
    runtime_evidence = runtime.evidence()
    if (
        runtime_evidence.get("algorithm") != "csr_caeos_v1"
        or runtime_evidence.get("runtime_revision")
        != "exact_clean_probability_replay_v2"
        or runtime_evidence.get("contains_test_ground_truth") is not False
    ):
        raise ValueError("invalid exact-replay runtime identity")
    ...
    value: Dict[str, Any] = {
        ...
        "capture": {
            "repair_manifest_file_sha256": file_hash(capture_path),
            "repair_manifest_sha256": capture["manifest_sha256"],
            "source_capture_manifest_file_sha256": capture[
