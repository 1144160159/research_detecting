...
def create_gate(protocol: dict[str, Any], observed_metrics: int) -> dict[str, Any]:
    if protocol.get("schema_version") != "strict_v4_mlp_klm_protocol_v1":
        raise ValueError("unexpected KLM protocol schema")
    if protocol.get("mode") != "pilot" or protocol.get("expected_runs") != 14:
        raise ValueError("KLM expansion gate requires the 14-scenario pilot protocol")
    if protocol.get("manifest_sha256") != canonical_hash(protocol):
        raise ValueError("KLM pilot protocol SHA mismatch")
    if observed_metrics != 0:
        raise ValueError("KLM expansion gate must be frozen before every pilot result")
    result = {
        "schema_version": "strict_v4_mlp_klm_expansion_gate_v1",
        ...
        "all_required_checks": {
            ...
            "split_integrity": "KLM, source MLP and OpenDetect split fingerprints identical",
            "known_f1_nonregression": "absolute KLM minus source-energy Known F1 <= 1e-12 in every scenario",
            "top_two_rank": "KLM four-unknown-metric mean rank among three pilot methods <= 2.0",
            "metric_breadth": "KLM has positive mean oriented gain over Energy on at least 2 of 4 metrics",
            "overall_gain": "mean of four KLM-versus-Energy oriented mean gains is strictly positive",
