...
def combined_fingerprint(metrics: dict[str, Any]) -> str:
    ...
    value = fingerprint.get("combined")
...
def select_scenarios(coverage: dict[str, Any]) -> dict[str, list[str]]:
    ...
    if len(registry) != 7:
        raise ValueError("Mahalanobis++ pilot requires the seven-suite coverage registry")
    ...
    for suite, entry in sorted(registry.items()):
        ...
        selected[suite] = sorted(
            ...
            key=lambda scenario: hashlib.sha256(
                f"{manifest_sha}:{suite}:{scenario}:mahalanobis_pp_v1".encode("utf-8")
            ).hexdigest(),
        )[:2]
    return selected
...
def create_protocol(
    ...
    if sum(len(values) for values in selected.values()) != 14 or len(selected) != 7:
        raise ValueError("Mahalanobis++ pilot requires two scenarios from each of seven suites")

    sources: dict[str, Any] = {}
    for suite, scenarios in sorted(selected.items()):
        if len(scenarios) != 2:
            raise ValueError(f"suite {suite} does not contain two frozen pilot scenarios")
        for scenario in sorted(scenarios):
            ...
            if missing:
                raise FileNotFoundError("missing frozen pilot sources: " + ", ".join(missing))
            ...
            mlp_fingerprint = combined_fingerprint(mlp_metrics)
            if mlp_fingerprint != combined_fingerprint(comparator_metrics):
    ...
    if missing_implementations:
        raise FileNotFoundError(
            "missing Mahalanobis++ implementations: " + ", ".join(missing_implementations)
        )
    protocol = {
        "schema_version": "strict_v4_mahalanobis_pp_pilot_protocol_v1",
        ...
        "selection_rule": (
            ...
            "after excluding the isolated implementation smoke; selection is independent "
            "of all formal Mahalanobis++ results"
        ),
        ...
        "method_definition": {
            ...
            "distance": "minimum_class_shared_covariance_mahalanobis",
            "covariance_estimator": "ledoit_wolf_on_known_train_class_residuals",
            "epsilon": 1e-12,
        },
        "expansion_gate": {
...
def main() -> None:
    ...
    implementations = {
        ...
        "runner": root / "run_strict_v4_mahalanobis_pp_pilot.py",
    ...
    if args.output.is_file():
        ...
        if existing != protocol:
            raise ValueError("existing Mahalanobis++ pilot protocol differs from frozen inputs")
    else:
