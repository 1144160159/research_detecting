...
def select_pilot_scenarios(coverage: dict[str, Any]) -> dict[str, list[str]]:
    ...
    for suite in sorted(registry):
        ...
        attempt = 0
        while len(indices) < 2:
            digest = hashlib.sha256(("%s:neco_id90:%s:%d" % (coverage_sha, suite, attempt)).encode("utf-8")).hexdigest()
...
def run_one(evaluator: Path, source: Path, output: Path, device: str, expected_source: dict[str, str]) -> dict[str, str]:
    ...
    if completed.returncode:
        raise RuntimeError("NECO-ID90 run failed for %s: %s" % (source, completed.stderr[-3000:]))
    if not valid_output(output, expected_source):
        raise RuntimeError("NECO-ID90 output validation failed for %s" % source)
...
def main() -> None:
    ...
    evaluator = base / "evaluate_mlp_neco.py"
    scorer = base / "caeos" / "neco_posthoc.py"
    source_hashes = {str(path.relative_to(source_root)): source_artifacts(path) for path in sources}
    protocol = {
        "schema_version": "strict_v4_mlp_neco_id90_protocol_v1",
        "status": "frozen_before_neco_results", "mode": args.mode,
        "coverage_manifest_sha256": coverage["manifest_sha256"],
        "selection_rule": "all frozen scenarios" if args.mode == "full" else "two NECO-ID90-SHA-indexed scenarios per suite independent of metric values",
        "selected_scenarios": {
        ...
        "paper": "https://proceedings.iclr.cc/paper_files/paper/2024/file/04b84142b99dae8560b517401e6e5275-Paper-Conference.pdf",
        "official_implementation": "https://gitlab.com/drti/neco",
        "official_implementation_commit": "6a55640669f0aad3e23f45ce2f6a8e6400c929ba",
        "formula": "NECO Algorithm-1 PCA projection norm ratio",
        "parameter_policy": "paper-recommended minimum PCs explaining at least 90% known-training variance",
        "main_table_audit": "official main results tune dimension separately per ID/OOD pair; that OOD-tuned policy is excluded",
        "fit_data": "known_training_features_only", "threshold_data": "known_validation_only",
        ...
        "architecture_policy": "Algorithm-1 ratio only without transformer max-logit multiplier",
        "nonredundancy": "principal-subspace ratio differs from NCI classifier-weight alignment and ViM null-space residual plus Energy",
    ...
    if protocol_path.is_file():
        if json.loads(protocol_path.read_text(encoding="utf-8")) != protocol:
            raise ValueError("existing NECO-ID90 protocol differs from frozen inputs")
    else:
    ...
    if completed != len(sources):
        raise RuntimeError("NECO-ID90 coverage incomplete: %d/%d" % (completed, len(sources)))
    summary = {
        "schema_version": "strict_v4_mlp_neco_id90_matrix_v1", "status": "complete", "mode": args.mode,
    ...
    (output_root / "neco_id90_complete").write_text(protocol["manifest_sha256"] + "\n", encoding="ascii")
