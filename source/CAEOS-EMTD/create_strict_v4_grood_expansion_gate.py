...
def create_gate(protocol: dict[str, Any], observed_metrics: int) -> dict[str, Any]:
    if protocol.get("schema_version") != "strict_v4_mlp_grood_protocol_v1":
        raise ValueError("unexpected GROOD protocol schema")
    if protocol.get("mode") != "pilot" or protocol.get("expected_runs") != 14:
        raise ValueError("GROOD gate requires the 14-scenario pilot protocol")
    if protocol.get("manifest_sha256") != canonical_hash(protocol):
        raise ValueError("GROOD pilot protocol SHA mismatch")
    if observed_metrics != 0:
        raise ValueError("GROOD gate must be frozen before every pilot result")
    result = {
        "schema_version": "strict_v4_mlp_grood_expansion_gate_v1",
        ...
        "all_required_checks": {
            ...
            "split_integrity": "GROOD, source MLP and OpenDetect splits identical",
            "known_f1_tolerance": "mean GROOD minus Energy Known F1 >= -0.01 and worst >= -0.05",
            "top_two_rank": "GROOD mean rank among three methods <= 2.0",
        ...
        "failure_action": "retain the 14-scenario negative pilot and stop expansion",
