...
from caeos.fisher_rao_posthoc import ICLR_URL, PAPER_URL
    ...
    for suite in sorted(registry):
        ...
        while len(indices) < 2:
            digest = hashlib.sha256(
                f"{coverage_sha}:fisher-rao-iclr2026:{suite}:{attempt}".encode()
            ).hexdigest()
...
def valid_output(output: Path, expected_source: dict[str, str]) -> bool:
    ...
    return (
        metrics.get("schema_version") == "strict_v4_mlp_fisher_rao_family_v1"
        and set(metrics.get("reports", {})) == set(METHODS)
        and provenance.get("source_artifact_sha256") == expected_source
        and provenance.get("methods") == METHODS
        and postprocessor.get("paper") == PAPER_URL
        and postprocessor.get("coefficient_policy")
        == "Eq.14-15 ID-only analytic variance balancing"
        and postprocessor.get("fit_split")
...
def run_one(evaluator, source, output, device, expected_source):
    ...
    completed = subprocess.run(
        [sys.executable, str(evaluator), "--source-run", str(source),
    ...
    if completed.returncode:
        raise RuntimeError(f"Fisher-Rao run failed for {source}: {completed.stderr[-2000:]}")
    if not valid_output(output, expected_source):
        raise RuntimeError(f"Fisher-Rao output validation failed for {source}")
...
def main() -> None:
    ...
    identities = (
        ...
        if args.mode == "full"
        else sorted((suite, scenario) for suite, items in pilot.items() for scenario in items)
    )
    sources = [source_by_identity[identity] for identity in identities]
    base = Path(__file__).resolve().parent
    implementation = {
        "evaluator": base / "evaluate_mlp_fisher_rao.py",
        "scorer": base / "caeos" / "fisher_rao_posthoc.py",
        "runner": Path(__file__).resolve(),
        "gate_creator": base / "create_strict_v4_fisher_rao_expansion_gate.py",
        "summarizer": base / "summarize_strict_v4_fisher_rao.py",
        "orchestration_script": base / "scripts" / "run_strict_v4_fisher_rao_pilot.sh",
    }
    source_hashes = {
    ...
    protocol = {
        ...
        "status": "frozen_before_fisher_rao_results",
        "mode": args.mode,
        "coverage_manifest_sha256": coverage["manifest_sha256"],
        "selection_rule": (
            "all frozen scenarios" if args.mode == "full"
            else "two Fisher-Rao-SHA-indexed scenarios per suite independent of metrics"
        ),
        "selected_scenarios": {
            suite: [scenario for selected_suite, scenario in identities if selected_suite == suite]
        ...
        "venue": ICLR_URL,
        "adapter": {
            "standard_formula": "Eq.7",
            "tensor_formula": "Eq.9",
            "additive_formula": "Eq.13",
            "coefficient_policy": "Eq.14-15 ID-only analytic variance balancing",
            "coefficient_signs": "lambda_M negative; lambda_y positive",
            "prediction_source": "unmodified frozen MLP",
        },
        "fit_data": "known_training_embeddings_logits_and_labels_only",
        ...
        "implementation_sha256": {name: sha256_file(path) for name, path in implementation.items()},
    }
    ...
    if protocol_path.is_file():
        if json.loads(protocol_path.read_text(encoding="utf-8")) != protocol:
            raise ValueError("existing Fisher-Rao protocol differs from frozen inputs")
    else:
        if observed:
            raise ValueError("refusing to freeze Fisher-Rao protocol after results exist")
    ...
    with ThreadPoolExecutor(max_workers=args.workers) as executor:
        ...
        for source in sources:
            ...
            future = executor.submit(
                run_one, implementation["evaluator"], source, output, args.device,
    ...
    completed = sum(
        valid_output(
            output_root / source.relative_to(source_root).parent
    ...
    if completed != len(sources):
        raise RuntimeError(f"Fisher-Rao coverage incomplete: {completed}/{len(sources)}")
    summary = {
        "schema_version": "strict_v4_mlp_fisher_rao_family_matrix_v1",
        "status": "complete",
        "mode": args.mode,
        "expected_runs": len(sources),
        "completed_runs": completed,
        "failures": 0,
        "methods_per_run": METHODS,
    ...
    (output_root / "fisher_rao_complete").write_text(
