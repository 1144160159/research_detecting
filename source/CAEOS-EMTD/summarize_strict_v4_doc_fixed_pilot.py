...
METHODS = ("doc_fixed", "mlp_msp", "mlp_energy", "opendetect")
...
def analyze(pilot_root: Path, source_root: Path, opendetect_root: Path, gate: dict[str, Any]) -> dict[str, Any]:
    ...
    if gate.get("schema_version") != "strict_v4_mlp_doc_fixed_expansion_gate_v1":
        raise ValueError("unexpected DOC-Fixed expansion gate schema")
    if gate.get("manifest_sha256") != canonical_hash(gate):
        raise ValueError("DOC-Fixed expansion gate SHA mismatch")
    if gate.get("pilot_protocol_manifest_sha256") != protocol.get("manifest_sha256"):
        raise ValueError("DOC-Fixed expansion gate protocol binding mismatch")
    blocks = []
    ...
    for suite, scenarios in protocol["selected_scenarios"].items():
        for scenario in scenarios:
            ...
            if candidate.get("selection_evidence", {}).get("unknown_or_test_labels_used_for_fitting_or_selection") is not False:
                raise ValueError("DOC-Fixed leakage invariant failed for %s/%s" % (suite, scenario))
            ...
            evidence = candidate["selection_evidence"]["doc"]
            diagnostics["%s/%s" % (suite, scenario)] = {
                "validation_risk_std": float(candidate["diagnostics"]["validation_risk_std"]),
                "test_risk_std": float(candidate["diagnostics"]["test_risk_std"]),
                "initial_bce": float(evidence["initial_bce"]),
                "final_bce": float(evidence["final_bce"]),
                "threshold_min": float(evidence["threshold_min"]),
                "threshold_max": float(evidence["threshold_max"]),
                "class_count": len(evidence["class_counts"]),
            }
            blocks.append({
                "suite": suite, "scenario": scenario,
                "reports": {
                    "doc_fixed": _report(candidate, "doc_fixed", candidate_path),
    ...
    for metric in UNKNOWN_METRICS:
        gains = []
        for block in blocks:
            candidate = block["reports"]["doc_fixed"][metric]
    ...
    known_f1_differences = [
        block["reports"]["doc_fixed"]["known_macro_f1"] - block["reports"]["mlp_msp"]["known_macro_f1"]
        for block in blocks
    ]
    ...
    candidate_row = next(row for row in overall if row["method"] == "doc_fixed")
    checks = {
        "pilot_runs_complete": len(blocks) == 14 and not failures,
        "split_integrity": True,
        "known_f1_tolerance": known_f1_mean_difference >= -0.01 and known_f1_worst_difference >= -0.05,
        "optimization_integrity": all(
            item["final_bce"] <= item["initial_bce"] + 1e-6
            and 0.5 <= item["threshold_min"] <= item["threshold_max"] <= 1.0
            and item["class_count"] >= 2
            for item in diagnostics.values()
        ),
        "nondegenerate_score": all(item["validation_risk_std"] > 1e-12 and item["test_risk_std"] > 1e-12 for item in diagnostics.values()),
    ...
    return {
        ...
        "doc_fixed_vs_mlp_msp_oriented_mean_gains": oriented_gains,
        "doc_fixed_vs_mlp_msp_four_metric_mean_gain": sum(oriented_gains.values()) / len(oriented_gains),
        "doc_fixed_vs_mlp_msp_suite_mean_gains": suite_gains,
        "known_f1_mean_difference": known_f1_mean_difference,
        "known_f1_worst_scenario_difference": known_f1_worst_difference,
        "validation": validation, "expansion_checks": checks,
        "decision": {"expand_doc_fixed_to_full102": all(checks.values())},
    }
...
def render(result: dict[str, Any]) -> str:
    lines = [
        ...
        "Expand DOC-Fixed to full 102: `%s`." % ("YES" if result["decision"]["expand_doc_fixed_to_full102"] else "NO"), "",
