...
def without_suffix(value: str, suffix: str) -> str:
    if not value.endswith(suffix):
        raise ValueError(f"{value!r} does not end with {suffix!r}")
...
def select_pilot_scenarios(coverage: dict[str, Any]) -> dict[str, list[str]]:
    ...
    for suite in sorted(registry):
        ...
        while len(indices) < 2:
            digest = hashlib.sha256(
                f"{coverage_sha}:logit-posthoc:{suite}:{attempt}".encode("utf-8")
            ).hexdigest()
...
def run_one(
    ...
    if completed.returncode:
        raise RuntimeError(
            f"logit post-hoc run failed for {source}: {completed.stderr[-2000:]}"
        )
    if not valid_output(output, expected_source):
        raise RuntimeError(f"logit post-hoc output validation failed for {source}")
...
def main() -> None:
    ...
    evaluator = Path(__file__).resolve().parent / "evaluate_mlp_logit_posthoc.py"
    scorer = Path(__file__).resolve().parent / "caeos" / "logit_posthoc.py"
    source_hashes = {
    ...
    protocol = {
        ...
        "status": "frozen_before_logit_posthoc_results",
        "mode": args.mode,
        "coverage_manifest_sha256": coverage["manifest_sha256"],
        "selection_rule": (
            "all frozen scenarios"
            if args.mode == "full"
            else "two SHA-indexed scenarios per suite, independent of metric values"
        ),
        "selected_scenarios": {
        ...
        "gen_gamma": 0.1,
        "gen_top_m": 100,
        "fit_data": "none_frozen_logits_only",
    ...
    if protocol_path.is_file():
        ...
        if existing != protocol:
            raise ValueError("existing logit post-hoc protocol differs from frozen inputs")
    else:
    ...
    completed = sum(
        valid_output(
            output_root / source.relative_to(source_root).parent / without_suffix(source.name, "_mlp"),
            source_hashes[str(source.relative_to(source_root))],
        )
        for source in sources
    )
    if completed != len(sources):
        raise RuntimeError(f"logit post-hoc coverage incomplete: {completed}/{len(sources)}")
    summary = {
        "schema_version": "strict_v4_mlp_logit_posthoc_matrix_v1",
        "status": "complete",
        "mode": args.mode,
        "expected_runs": len(sources),
    ...
    (output_root / "logit_posthoc_complete").write_text(
