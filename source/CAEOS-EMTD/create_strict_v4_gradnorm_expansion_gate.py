...
def create_gate(protocol: dict[str, Any], observed_metrics: int) -> dict[str, Any]:
    if protocol.get("schema_version") != "strict_v4_mlp_gradnorm_protocol_v1":
        raise ValueError("unexpected GradNorm protocol schema")
    if protocol.get("mode") != "pilot" or protocol.get("expected_runs") != 14:
        raise ValueError("GradNorm gate requires the 14-scenario pilot protocol")
    if protocol.get("manifest_sha256") != canonical_hash(protocol):
        raise ValueError("GradNorm pilot protocol SHA mismatch")
    if observed_metrics != 0:
        raise ValueError("GradNorm gate must be frozen before every pilot result")
    result = {
        "schema_version": "strict_v4_mlp_gradnorm_expansion_gate_v1",
        ...
        "all_required_checks": {
            ...
            "split_integrity": "GradNorm, source MLP and OpenDetect splits identical",
            "known_f1_nonregression": "absolute GradNorm minus Energy Known F1 <= 1e-12",
            "top_two_rank": "GradNorm mean rank among three methods <= 2.0",
