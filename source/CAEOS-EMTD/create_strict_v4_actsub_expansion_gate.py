...
def create_gate(protocol: dict[str, Any], observed_metrics: int) -> dict[str, Any]:
    if protocol.get("schema_version") != "strict_v4_mlp_actsub_scale_fixed_protocol_v1":
        raise ValueError("unexpected ActSub protocol schema")
    if protocol.get("mode") != "pilot" or protocol.get("expected_runs") != 14:
        raise ValueError("ActSub gate requires the 14-scenario pilot protocol")
    if protocol.get("manifest_sha256") != canonical_hash(protocol):
        raise ValueError("ActSub pilot protocol SHA mismatch")
    if observed_metrics != 0:
        raise ValueError("ActSub gate must be frozen before every pilot result")
    result = {
        "schema_version": "strict_v4_mlp_actsub_scale_fixed_expansion_gate_v1",
        ...
        "expansion_candidate": "actsub_scale_fixed",
        ...
        "all_required_checks": {
            ...
            "split_integrity": "ActSub, source MLP and OpenDetect splits identical",
            "known_f1_nonregression": "absolute ActSub minus source MLP Known F1 <= 1e-12",
            "formula_integrity": "official SVD/Eq.4/Eq.9/Eq.10 with fixed defaults verified",
            "score_nonconstant": "validation and test ActSub score standard deviation > 1e-12 in all 14 runs",
            "top_two_rank": "ActSub mean rank among four methods <= 2.0",
