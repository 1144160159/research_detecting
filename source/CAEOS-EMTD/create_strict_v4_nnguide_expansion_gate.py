...
def create_gate(protocol: dict[str, Any], observed_metrics: int) -> dict[str, Any]:
    if protocol.get("schema_version") != "strict_v4_mlp_nnguide_protocol_v1":
        raise ValueError("unexpected NNGuide protocol schema")
    if protocol.get("mode") != "pilot" or protocol.get("expected_runs") != 14:
        raise ValueError("NNGuide expansion gate requires the 14-scenario pilot protocol")
    if protocol.get("manifest_sha256") != canonical_hash(protocol):
        raise ValueError("NNGuide pilot protocol SHA mismatch")
    if protocol.get("fit_data") != "known_training_features_and_logits_only":
        raise ValueError("NNGuide fitting must use known training data only")
    if protocol.get("ood_parameter_sweep") is not False:
        raise ValueError("NNGuide expansion gate forbids OOD parameter sweeping")
    if observed_metrics != 0:
        raise ValueError("NNGuide expansion gate must be frozen before every pilot result")
    result = {
        "schema_version": "strict_v4_mlp_nnguide_expansion_gate_v1",
        ...
        "expansion_candidate": "nnguide_energy", "reference_methods": ["mlp_energy", "opendetect"],
        ...
        "all_required_checks": {
            ...
            "split_integrity": "NNGuide, source MLP and OpenDetect split fingerprints identical",
            "known_f1_nonregression": "absolute NNGuide minus source-energy Known F1 <= 1e-12 in every scenario",
            "top_two_rank": "NNGuide four-unknown-metric mean rank among three pilot methods <= 2.0",
            "metric_breadth": "NNGuide has positive mean oriented gain over Energy on at least 2 of 4 metrics",
            "overall_gain": "mean of four NNGuide-versus-Energy oriented mean gains is strictly positive",
