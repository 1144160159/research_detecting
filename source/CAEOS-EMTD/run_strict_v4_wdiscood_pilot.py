...
def validate_protocol(protocol: dict[str, Any], root: Path) -> None:
    if protocol.get("schema_version") != "strict_v4_wdiscood_pilot_protocol_v1":
        raise ValueError("unexpected WDiscOOD pilot protocol schema")
    if protocol.get("manifest_sha256") != canonical_hash(protocol):
        raise ValueError("WDiscOOD pilot protocol SHA mismatch")
    paths = {
        "calibrator": root / "caeos" / "wdiscood.py",
        "evaluator": root / "evaluate_mlp_wdiscood.py",
        "protocol_creator": root / "create_strict_v4_wdiscood_pilot_protocol.py",
        "runner": Path(__file__).resolve(),
        "summarizer": root / "summarize_strict_v4_wdiscood_pilot.py",
    }
    if {name: file_hash(path) for name, path in paths.items()} != protocol.get(
        "implementation_sha256", {}
    ):
        raise ValueError("WDiscOOD implementation changed after protocol freeze")
...
def valid_output(output: Path, source_sha256: dict[str, str]) -> bool:
    ...
    try:
        ...
        return False
    return (
        metrics.get("schema_version") == "strict_v4_mlp_wdiscood_v1"
...
def run_one(
    ...
    if completed.returncode:
        raise RuntimeError(f"WDiscOOD run failed for {source}: {completed.stderr[-2000:]}")
    if not valid_output(output, source_sha256):
        raise RuntimeError(f"WDiscOOD output validation failed for {source}")
    return {"source": str(source), "output": str(output), "state": "completed"}


def main() -> None:
    parser = argparse.ArgumentParser(description="Run the frozen WDiscOOD pilot")
    ...
    if completed != expected:
        raise RuntimeError(f"WDiscOOD pilot coverage incomplete: {completed}/{expected}")
    summary = {
        "schema_version": "strict_v4_wdiscood_pilot_matrix_v1",
        "status": "complete",
