...
METHODS = ("ash_s_90", "mlp_energy", "opendetect")
...
def analyze(
    ...
    if protocol.get("percentile") != 90.0 or protocol.get("temperature") != 1.0:
        raise ValueError("ASH pilot parameters differ from frozen values")
    if gate.get("schema_version") != "strict_v4_mlp_ash_expansion_gate_v1":
        raise ValueError("unexpected ASH expansion gate schema")
    if gate.get("manifest_sha256") != canonical_hash(gate):
        raise ValueError("ASH expansion gate SHA mismatch")
    if gate.get("pilot_protocol_manifest_sha256") != protocol.get("manifest_sha256"):
        raise ValueError("ASH expansion gate protocol binding mismatch")
    ...
    for suite, scenarios in protocol["selected_scenarios"].items():
        for scenario in scenarios:
            ...
            if (
                new.get("arguments", {}).get("percentile") != 90.0
                or new.get("arguments", {}).get("temperature") != 1.0
                or new.get("selection_evidence", {}).get(
                    "unknown_or_test_labels_used_for_fitting_or_selection"
                )
                is not False
            ):
                raise ValueError("ASH parameter or leakage invariant failed for %s/%s" % (suite, scenario))
            ...
            blocks.append(
                {
                    ...
                    "scenario": scenario,
                    "reports": {
                        "ash_s_90": _report(new, "ash_s_90", new_path),
    ...
    for metric in UNKNOWN_METRICS:
        values = []
        for block in blocks:
            candidate = block["reports"]["ash_s_90"][metric]
    ...
    ash_row = next(row for row in overall if row["method"] == "ash_s_90")
    known_f1_differences = [
        block["reports"]["ash_s_90"]["known_macro_f1"]
    ...
    checks = {
        ...
        "top_two_rank": ash_row["mean_unknown_metric_rank"] <= 2.0,
    ...
    return {
        "schema_version": "strict_v4_mlp_ash_pilot_analysis_v1",
        ...
        "overall": overall,
        "ash_vs_mlp_energy_oriented_mean_gains": oriented_gains,
        "ash_vs_mlp_energy_four_metric_mean_gain": sum(oriented_gains.values())
        / len(oriented_gains),
        "ash_vs_mlp_energy_suite_mean_gains": suite_gains,
        ...
        "decision": {"expand_ash_to_full102": all(checks.values())},
    }
...
def render(result: dict[str, Any]) -> str:
    lines = [
        ...
        "",
        "Expand ASH to full 102: `%s`."
        % ("YES" if result["decision"]["expand_ash_to_full102"] else "NO"),
        "",
