...
def create_gate(protocol: dict[str, Any], observed_metrics: int) -> dict[str, Any]:
    if protocol.get("schema_version") != "strict_v4_mlp_fisher_rao_family_protocol_v1":
        raise ValueError("unexpected Fisher-Rao protocol schema")
    if protocol.get("mode") != "pilot" or protocol.get("expected_runs") != 14:
        raise ValueError("Fisher-Rao gate requires the 14-scenario pilot protocol")
    if protocol.get("manifest_sha256") != canonical_hash(protocol):
        raise ValueError("Fisher-Rao pilot protocol SHA mismatch")
    if observed_metrics != 0:
        raise ValueError("Fisher-Rao gate must be frozen before every pilot result")
    result = {
        "schema_version": "strict_v4_mlp_fisher_rao_family_expansion_gate_v1",
        ...
        "expansion_candidates": ["fim_standard", "fim_tensor", "fim_additive"],
        ...
        "per_candidate_required_checks": {
            ...
            "formula_integrity": "ICLR 2026 Eq.7/Eq.9/Eq.13/Eq.14-15 verified",
            "score_nonconstant": "validation and test score standard deviation > 1e-12",
            "top_two_rank": "candidate mean rank among five methods <= 2.0",
        ...
        "full_matrix_action": "run all shared-pass methods on all 102 scenarios if any passes",
