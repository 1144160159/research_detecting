...
def select_pilot_scenarios(coverage: dict[str, Any]) -> dict[str, list[str]]:
    ...
    for suite in sorted(registry):
        ...
        while len(indices) < 2:
            digest = hashlib.sha256(
                ("%s:ash-posthoc:%s:%d" % (coverage_sha, suite, attempt)).encode(
                    "utf-8"
                )
            ).hexdigest()
...
def valid_output(output: Path, expected_source: dict[str, str]) -> bool:
    ...
    try:
        ...
        return False
    return (
        set(metrics.get("reports", {})) == {"ash_s_90"}
        and provenance.get("source_artifact_sha256") == expected_source
        and provenance.get("methods") == ["ash_s_90"]
        and metrics.get("arguments", {}).get("percentile") == ASH_PERCENTILE
        and metrics.get("arguments", {}).get("temperature") == ASH_TEMPERATURE
    )
...
def run_one(
    ...
    if completed.returncode:
        raise RuntimeError("ASH run failed for %s: %s" % (source, completed.stderr[-2000:]))
    if not valid_output(output, expected_source):
        raise RuntimeError("ASH output validation failed for %s" % source)
...
def main() -> None:
    ...
    evaluator = base / "evaluate_mlp_ash.py"
    scorer = base / "caeos" / "ash_posthoc.py"
    source_hashes = {
        str(path.relative_to(source_root)): source_artifacts(path) for path in sources
    }
    protocol = {
        "schema_version": "strict_v4_mlp_ash_protocol_v1",
        "status": "frozen_before_ash_results",
        "mode": args.mode,
        "coverage_manifest_sha256": coverage["manifest_sha256"],
        "selection_rule": (
            "all frozen scenarios"
            if args.mode == "full"
            else "two ASH-SHA-indexed scenarios per suite, independent of metric values"
        ),
        "selected_scenarios": {
            suite: [
                scenario
        ...
        "official_implementation": (
            "https://github.com/andrijazz/ash/blob/main/ash.py"
        ),
        "formula": "ASH-S top-k pruning and exponential mass-ratio sharpening",
        "percentile": ASH_PERCENTILE,
        "temperature": ASH_TEMPERATURE,
        "parameter_policy": "paper main penultimate setting fixed before results",
        "activation_adapter": "ReLU clamp of GELU penultimate embeddings",
        "fit_data": "none",
    ...
    if protocol_path.is_file():
        ...
        if existing != protocol:
            raise ValueError("existing ASH protocol differs from frozen inputs")
    else:
    ...
    if completed != len(sources):
        raise RuntimeError("ASH coverage incomplete: %d/%d" % (completed, len(sources)))
    summary = {
        "schema_version": "strict_v4_mlp_ash_matrix_v1",
        "status": "complete",
        "mode": args.mode,
        "expected_runs": len(sources),
        "completed_runs": completed,
        "failures": 0,
        "methods_per_run": ["ash_s_90"],
    ...
    (output_root / "ash_complete").write_text(
