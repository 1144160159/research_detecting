...
def select_pilot_scenarios(coverage: dict[str, Any]) -> dict[str, list[str]]:
    ...
    for suite in sorted(registry):
        ...
        while len(indices) < 2:
            digest = hashlib.sha256(
                ("%s:optfs-posthoc:%s:%d" % (coverage_sha, suite, attempt)).encode("utf-8")
            ).hexdigest()
...
def valid_output(output: Path, expected_source: dict[str, str]) -> bool:
    ...
    return (
        ...
        and arguments.get("quantile_epsilon") == QUANTILE_EPSILON
        and arguments.get("bin_count") == BIN_COUNT
        and arguments.get("theta_norm") == THETA_NORM
    )
...
def run_one(evaluator: Path, source: Path, output: Path, device: str, expected_source: dict[str, str]) -> dict[str, str]:
    ...
    command = [
        ...
        "--device", device, "--quantile-epsilon", str(QUANTILE_EPSILON),
        "--bin-count", str(BIN_COUNT), "--theta-norm", str(THETA_NORM),
    ]
    completed = subprocess.run(command, text=True, capture_output=True)
    (output / "run.log").write_text(completed.stdout + completed.stderr, encoding="utf-8")
    if completed.returncode:
        raise RuntimeError("OptFS run failed for %s: %s" % (source, completed.stderr[-3000:]))
    if not valid_output(output, expected_source):
        raise RuntimeError("OptFS output validation failed for %s" % source)
...
def main() -> None:
    ...
    evaluator = base / "evaluate_mlp_optfs.py"
    scorer = base / "caeos" / "optfs_posthoc.py"
    ...
    protocol = {
        ...
        "status": "frozen_before_optfs_results", "mode": args.mode,
        "coverage_manifest_sha256": coverage["manifest_sha256"],
        "selection_rule": "all frozen scenarios" if args.mode == "full" else "two OptFS-SHA-indexed scenarios per suite independent of metric values",
        "selected_scenarios": {
        ...
        "paper": "https://openreview.net/forum?id=dm8e7gsH0d",
        "official_implementation": "https://github.com/Qinyu-Allen-Zhao/OptFSOOD",
        "official_implementation_commit": "c09a85e1f15de5a1f45c89419be6a5fff03f88c5",
        "formula": "official fixed-weight OptFS Ours (V)",
        "quantile_epsilon": QUANTILE_EPSILON, "bin_count": BIN_COUNT, "theta_norm": THETA_NORM,
        "parameter_policy": "official source constants and default vanilla score; no OOD sweep",
        ...
        "prediction_policy": "unshaped_frozen_classifier_prediction",
        "signed_feature_adapter": "native global bins over signed GELU embedding",
        "official_diagnostic_adapter": "omit non-algorithmic forced 10000-sample changed-prediction print",
    ...
    if protocol_path.is_file():
        if json.loads(protocol_path.read_text(encoding="utf-8")) != protocol:
            raise ValueError("existing OptFS protocol differs from frozen inputs")
    else:
    ...
    if completed != len(sources):
        raise RuntimeError("OptFS coverage incomplete: %d/%d" % (completed, len(sources)))
    summary = {
        "schema_version": "strict_v4_mlp_optfs_matrix_v1", "status": "complete", "mode": args.mode,
    ...
    (output_root / "optfs_complete").write_text(protocol["manifest_sha256"] + "\n", encoding="ascii")
