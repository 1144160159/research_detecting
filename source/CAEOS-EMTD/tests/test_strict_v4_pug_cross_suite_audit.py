...
from audit_strict_v4_pug_cross_suite_confirmation import build_audit
from create_strict_v4_external_confirmation_protocol import canonical_hash
from test_strict_v4_pug_cross_suite_summary import (
    protocol,
    records,
    summarize,
)
...
def test_audit_independently_reconciles_positive_summary() -> None:
    ...
    assert result["selection"]["selected_algorithm"] == "caeos_pug"
...
def test_audit_accepts_scientifically_negative_complete_result() -> None:
    ...
    assert result["selection"]["selected_algorithm"] == "upstream_incumbent"


def test_audit_detects_recanonicalized_route_tampering() -> None:
    value = protocol()
    rows = records(value)
    summary = copy.deepcopy(summarize(value, rows))
    ...
    assert (
        result["integrity"]["checks"][
            "decision_and_route_independently_reconcile"
        ]
        is False
    )
