...
METHODS = ("odin", "mlp_energy", "opendetect")
...
def analyze(
    ...
    if protocol.get("temperature") != 1000.0 or protocol.get("noise") != 0.001:
        raise ValueError("ODIN pilot parameters differ from the frozen values")
    if gate.get("schema_version") != "strict_v4_mlp_odin_expansion_gate_v1":
        raise ValueError("unexpected ODIN expansion gate schema")
    if gate.get("manifest_sha256") != canonical_hash(gate):
        raise ValueError("ODIN expansion gate SHA mismatch")
    if gate.get("pilot_protocol_manifest_sha256") != protocol.get("manifest_sha256"):
        raise ValueError("ODIN expansion gate protocol binding mismatch")
    ...
    for suite, scenarios in protocol["selected_scenarios"].items():
        for scenario in scenarios:
            ...
            if (
                new.get("arguments", {}).get("temperature") != 1000.0
                or new.get("arguments", {}).get("noise") != 0.001
                or new.get("selection_evidence", {}).get(
                    "unknown_or_test_labels_used_for_fitting_or_selection"
                )
                is not False
            ):
                raise ValueError("ODIN parameter or leakage invariant failed for %s/%s" % (suite, scenario))
            ...
            blocks.append(
                {
                    ...
                    "scenario": scenario,
                    "reports": {
                        "odin": _report(new, "odin", new_path),
    ...
    for metric in UNKNOWN_METRICS:
        values = []
        for block in blocks:
            candidate = block["reports"]["odin"][metric]
    ...
    odin_row = next(row for row in overall if row["method"] == "odin")
    known_f1_max_difference = max(
        abs(
            block["reports"]["odin"]["known_macro_f1"]
    ...
    checks = {
        ...
        "top_two_rank": odin_row["mean_unknown_metric_rank"] <= 2.0,
    ...
    return {
        "schema_version": "strict_v4_mlp_odin_pilot_analysis_v1",
        ...
        "overall": overall,
        "odin_vs_mlp_energy_oriented_mean_gains": oriented_gains,
        "odin_vs_mlp_energy_four_metric_mean_gain": sum(oriented_gains.values())
        / len(oriented_gains),
        "odin_vs_mlp_energy_suite_mean_gains": suite_gains,
        "known_f1_max_absolute_difference": known_f1_max_difference,
        "expansion_checks": checks,
        "decision": {"expand_odin_to_full102": all(checks.values())},
    }
...
def render(result: dict[str, Any]) -> str:
    lines = [
        ...
        "",
        "Expand ODIN to full 102: `%s`."
        % ("YES" if result["decision"]["expand_odin_to_full102"] else "NO"),
        "",
