...
from caeos.gradnorm_posthoc import evidence, gradnorm_risk
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Evaluate official-formula GradNorm from a frozen strict MLP run"
    )
    ...
    with np.load(source_paths["scores.npz"], allow_pickle=False) as source_scores:
        ...
        if arguments.get("dataset") != "tabular":
            raise ValueError("strict GradNorm matrix only accepts tabular runs")
    ...
    validation_risk = gradnorm_risk(validation["embedding"], validation["logits"])
    test_risk = gradnorm_risk(test["embedding"], test["logits"])
    ...
    result = {
        "schema_version": "strict_v4_mlp_gradnorm_v1",
        "model": "mlp_gradnorm",
        "method": "gradnorm",
        ...
        "validation_thresholds": {"gradnorm": threshold},
        "reports": {"gradnorm": report},
        "implementation": "OpenOOD GradNorm final-layer gradient formula on frozen MLP",
        "selection_evidence": {
            "protocol": "strict_frozen_checkpoint_gradnorm",
    ...
    np.savez_compressed(
        ...
        validation_gradnorm=validation_risk,
        test_gradnorm=test_risk,
    )
    provenance = {
        "schema_version": "strict_v4_mlp_gradnorm_provenance_v1",
        "source_run": str(source_run),
        "source_artifact_sha256": source_sha256,
        "implementation_sha256": sha256_file(Path(__file__).resolve()),
        "scorer_module_sha256": sha256_file(
            Path(__file__).resolve().parent / "caeos" / "gradnorm_posthoc.py"
        ),
        "methods": ["gradnorm"],
    ...
    print(json.dumps({"gradnorm": report}, ensure_ascii=False, sort_keys=True), flush=True)
