...
def analyze(protocol: dict[str, Any], run_root: Path) -> dict[str, Any]:
    if protocol.get("schema_version") != "mal_tls_counterfactual_conflict_gate_protocol_v1":
        raise ValueError("unexpected counterfactual conflict-gate protocol schema")
    if protocol.get("manifest_sha256") != canonical_hash(protocol):
        raise ValueError("counterfactual conflict-gate protocol SHA mismatch")
    ...
    for scenario in protocol["dataset"]["scenarios"]:
        ...
        for name, expected in frozen_metric_values.items():
            if candidate.get(name) != expected:
                raise ValueError(f"frozen candidate setting mismatch: {scenario}/{name}")
        if diagnostic.get("unknown_or_test_labels_used") is not False:
            raise ValueError(f"counterfactual diagnostic leakage declaration failed: {scenario}")
        if int(diagnostic.get("valid_samples", 0)) <= 0:
            raise ValueError(f"counterfactual diagnostic is empty: {scenario}")
    ...
    if len(blocks) * 2 != int(protocol["training"]["expected_development_runs"]):
        raise ValueError("counterfactual conflict-gate pilot run count is incomplete")
    means = {
    ...
    return {
        ...
        "decision": (
            ...
            if passes
            else "retain_caeos_pairwise_and_reject_counterfactual_conflict_gate"
        ),
        "blocks": blocks,
    }
