...
from materialize_rrc_csr_runtime import load_json
...
def evaluate(
    ...
    *,
    ...
    if identity not in expected:
        raise ValueError("RRC evaluation task is outside frozen universe")
    if condition not in protocol["conditions"]:
        raise ValueError("unsupported RRC evaluation condition")
    ...
    if (
        ...
        raise ValueError("RRC artifact hash mismatch")

    runtime = joblib.load(artifact)
    evidence = runtime.evidence()
    if (
        evidence.get("schema_version") != "strict_v4_rrc_csr_runtime_v1"
        or evidence.get("algorithm") != "rrc_csr_caeos_v1"
        or evidence.get("scenario_certificate", {}).get("manifest_sha256")
        != capture["scenario_certificate_manifest_sha256"]
    ):
        raise ValueError("RRC runtime evidence mismatch")
    ...
    value: Dict[str, Any] = {
        "schema_version": "strict_v4_rrc_csr_evaluation_v1",
        "state": "complete",
        ...
        "condition": condition,
        "scenario_certificate_manifest_sha256": capture[
        ...
        "routing": {
            ...
            "prediction_exactly_pairwise_all_rows": bool(
                np.array_equal(inference["prediction"], pairwise_prediction)
            ),
