...
METHODS = ("nnguide_energy", "mlp_energy", "opendetect")
...
def analyze(pilot_root: Path, source_root: Path, opendetect_root: Path, gate: dict[str, Any]) -> dict[str, Any]:
    ...
    if gate.get("schema_version") != "strict_v4_mlp_nnguide_expansion_gate_v1":
        raise ValueError("unexpected NNGuide expansion gate schema")
    if gate.get("manifest_sha256") != canonical_hash(gate):
        raise ValueError("NNGuide expansion gate SHA mismatch")
    if gate.get("pilot_protocol_manifest_sha256") != protocol.get("manifest_sha256"):
        raise ValueError("NNGuide expansion gate protocol binding mismatch")
    blocks = []
    ...
    for suite, scenarios in protocol["selected_scenarios"].items():
        for scenario in scenarios:
            ...
            blocks.append({
                ...
                "reports": {
                    "nnguide_energy": _report(new, "nnguide_energy", new_path),
    ...
    for metric in UNKNOWN_METRICS:
        gains = []
        for block in blocks:
            candidate = block["reports"]["nnguide_energy"][metric]
    ...
    candidate_row = next(row for row in overall if row["method"] == "nnguide_energy")
    known_f1_max_difference = max(
        abs(block["reports"]["nnguide_energy"]["known_macro_f1"] - block["reports"]["mlp_energy"]["known_macro_f1"])
        for block in blocks
    )
    minimum_bank_adapter_scenarios = []
    for block in blocks:
        metrics_path = pilot_root / block["suite"] / (block["scenario"] + "_seed7") / "metrics.json"
        evidence = _read(metrics_path)["selection_evidence"]["postprocessor"]
        if evidence["adaptation"]["minimum_bank_adapter_used"]:
            minimum_bank_adapter_scenarios.append("%s/%s" % (block["suite"], block["scenario"]))
    checks = {
    ...
    return {
        "schema_version": "strict_v4_mlp_nnguide_pilot_analysis_v1",
        ...
        "nnguide_vs_mlp_energy_oriented_mean_gains": oriented_gains,
        "nnguide_vs_mlp_energy_four_metric_mean_gain": sum(oriented_gains.values()) / len(oriented_gains),
        "nnguide_vs_mlp_energy_suite_mean_gains": suite_gains,
        "known_f1_max_absolute_difference": known_f1_max_difference,
        "minimum_bank_adapter_scenarios": minimum_bank_adapter_scenarios,
        "expansion_checks": checks, "decision": {"expand_nnguide_to_full102": all(checks.values())},
    }
...
def render(result: dict[str, Any]) -> str:
    lines = [
        ...
        "Expand NNGuide to full 102: `%s`." % ("YES" if result["decision"]["expand_nnguide_to_full102"] else "NO"), "",
