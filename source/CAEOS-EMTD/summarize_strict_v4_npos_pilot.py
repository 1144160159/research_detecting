...
def main() -> None:
    ...
    if (
        protocol.get("schema_version") != "strict_v4_npos_pilot_protocol_v1"
        or protocol.get("manifest_sha256") != canonical_hash(protocol)
        or gate.get("schema_version") != "strict_v4_npos_pilot_expansion_gate_v1"
        or gate.get("manifest_sha256") != canonical_hash(gate)
        or gate.get("pilot_protocol_manifest_sha256") != protocol["manifest_sha256"]
    ):
        raise ValueError("NPOS pilot frozen artifacts validation failed")
    if protocol["implementation_sha256"]["summarize_strict_v4_npos_pilot.py"] != file_hash(Path(__file__)):
        raise ValueError("NPOS summarizer implementation SHA mismatch")
    if not (args.run_root / "execution_complete").is_file():
        raise ValueError("NPOS pilot execution is incomplete")
    ...
    for source in protocol["source_registry"]:
        ...
        if (
            ...
            or file_hash(Path(source["comparator_metrics"])) != source["comparator_metrics_sha256"]
            or candidate.get("schema_version") != "strict_v4_npos_metrics_v1"
            or candidate["split_metadata"]["split_fingerprint"]["combined"] != source["split_fingerprint"]
            or candidate.get("selection_evidence", {}).get(
                "unknown_or_test_labels_used_for_fitting_or_selection"
            ) is not False
        ):
            raise ValueError(f"NPOS pilot source validation failed: {candidate_path}")
        methods = {
            "npos_knn": candidate["reports"]["npos_knn"],
            "mlp_knn": mlp["reports"]["knn"],
    ...
    if len(records) != 14:
        raise ValueError("NPOS pilot record count mismatch")

    method_names = ("npos_knn", "mlp_knn", "opendetect")
    overall: dict[str, dict[str, Any]] = {}
    for method in method_names:
        overall[method] = {
            "method": method,
            **{
                metric: float(np.mean([r["methods"][method][metric] for r in records]))
    ...
    gains = {
        metric: [
            oriented(
                r["methods"]["npos_knn"][metric],
                r["methods"]["opendetect"][metric],
                metric,
            )
            for r in records
        ]
    ...
    known_deltas = [
        r["methods"]["npos_knn"]["known_macro_f1"]
        - r["methods"]["opendetect"]["known_macro_f1"]
        for r in records
    ]
    suite_gains = {}
    ...
    checks = {
        ...
        "known_f1_tolerance": float(np.mean(known_deltas)) >= -0.03 and min(known_deltas) >= -0.10,
        "top_two_rank": overall["npos_knn"]["mean_unknown_metric_rank"] <= 2.0,
    ...
    result = {
        "schema_version": "strict_v4_npos_pilot_analysis_v1",
        "status": "complete",
        ...
        "overall": sorted(overall.values(), key=lambda item: item["mean_unknown_metric_rank"]),
        "npos_vs_opendetect": {
    ...
    (args.output_dir / "expansion_decision.json").write_text(
        json.dumps(
            {
                "schema_version": "strict_v4_npos_expansion_decision_v1",
                "protocol_manifest_sha256": protocol["manifest_sha256"],
                "checks": checks,
                "expand_to_full102": passes,
            },
