...
def evaluate(args: argparse.Namespace) -> dict[str, Any]:
    ...
    if len(test_incumbent) != len(test_features):
        raise ValueError("test incumbent risk is not aligned")
    ...
    validation_candidate = blend_with_incumbent(
        validation_incumbent, validation_topology, args.alpha
    )
    test_candidate = blend_with_incumbent(test_incumbent, test_topology, args.alpha)
    ...
    result = {
        "schema_version": "strict_v4_conflict_topology_copula_metrics_v1",
        "protocol_manifest_sha256": args.protocol_manifest_sha256,
        "suite": args.suite,
        "scenario": args.scenario,
        "seed": args.seed,
        "reference_risk": selected_risk_name,
        "candidate_risk": "conflict_topology_copula_blend",
        "parameters": {
        ...
        "diagnostics": {
            ...
            "known_macro_f1_absolute_difference": abs(
                candidate_report["known_macro_f1"]
                - reference_report["known_macro_f1"]
            ),
            "validation_feature_count": int(len(validation_features)),
            "test_feature_count": int(len(test_features)),
            "copula": scorer.evidence(),
            "unknown_or_test_labels_used_for_fit_or_threshold": False,
