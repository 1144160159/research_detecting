...
def booster_uses_cuda(configuration: str) -> bool:
    return '"device":"cuda' in configuration.replace(" ", "")
...
def train_task(args: argparse.Namespace) -> dict[str, Any]:
    ...
    scaled, scaling_report = robust_scale_statistics(
        statistics, splits["train"]
    )
    x_train = scaled[splits["train"]]
    x_validation = scaled[splits["validation"]]
    x_test = scaled[splits["test"]]
    y_train = encoded_labels[splits["train"]]
    y_validation = encoded_labels[splits["validation"]]
    ...
    np.savez_compressed(
        ...
        validation_type_prediction=validation_family_probability.argmax(axis=1),
    ...
    report: dict[str, Any] = {
        "schema_version": "strict_v4_flow_statistic_xgboost_cuda_task_v1",
        "state": "complete",
        "task": {
            "unknown_family": args.unknown_family,
            "seed": args.seed,
        ...
        "split_counts": {
            split: dict(sorted(Counter(families[indices]).items()))
            for split, indices in splits.items()
        },
        "model": {
            "name": "FSX-CAEOS CUDA XGBoost evidence branch",
            "architecture": "flow_statistic_dual_xgboost_cuda_v1",
    ...
    if not gpu_passes:
        raise RuntimeError("XGBoost CUDA execution evidence did not pass")
