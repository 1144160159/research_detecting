...
METHODS = ("fdbd", "mlp_energy", "opendetect")
...
def analyze(pilot_root: Path, source_root: Path, opendetect_root: Path, gate: dict[str, Any]) -> dict[str, Any]:
    ...
    if gate.get("schema_version") != "strict_v4_mlp_fdbd_expansion_gate_v1":
        raise ValueError("unexpected fDBD expansion gate schema")
    if gate.get("manifest_sha256") != canonical_hash(gate):
        raise ValueError("fDBD expansion gate SHA mismatch")
    if gate.get("pilot_protocol_manifest_sha256") != protocol.get("manifest_sha256"):
        raise ValueError("fDBD expansion gate protocol binding mismatch")
    blocks = []
    failures = list(pilot_root.glob("**/failure.json"))
    zero_weight_scenarios = []
    for suite, scenarios in protocol["selected_scenarios"].items():
        for scenario in scenarios:
            ...
            if int(evidence["zero_nonself_weight_distance_count"]):
                zero_weight_scenarios.append("%s/%s" % (suite, scenario))
            blocks.append({
                "suite": suite, "scenario": scenario,
                "reports": {
                    "fdbd": _report(candidate, "fdbd", candidate_path),
    ...
    for metric in UNKNOWN_METRICS:
        gains = []
        for block in blocks:
            candidate = block["reports"]["fdbd"][metric]
    ...
    candidate_row = next(row for row in overall if row["method"] == "fdbd")
    known_f1_max_difference = max(
        abs(block["reports"]["fdbd"]["known_macro_f1"] - block["reports"]["mlp_energy"]["known_macro_f1"])
        for block in blocks
    )
    checks = {
    ...
    return {
        "schema_version": "strict_v4_mlp_fdbd_pilot_analysis_v1",
        ...
        "fdbd_vs_mlp_energy_oriented_mean_gains": oriented_gains,
        "fdbd_vs_mlp_energy_four_metric_mean_gain": sum(oriented_gains.values()) / len(oriented_gains),
        "fdbd_vs_mlp_energy_suite_mean_gains": suite_gains,
        "known_f1_max_absolute_difference": known_f1_max_difference,
        "zero_weight_distance_scenarios": zero_weight_scenarios,
        "expansion_checks": checks, "decision": {"expand_fdbd_to_full102": all(checks.values())},
    }
...
def render(result: dict[str, Any]) -> str:
    lines = [
        ...
        "Expand fDBD to full 102: `%s`." % ("YES" if result["decision"]["expand_fdbd_to_full102"] else "NO"), "",
