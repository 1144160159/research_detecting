...
from caeos.cadref_posthoc import OFFICIAL_CODE_URL, OFFICIAL_COMMIT, PAPER_URL
    ...
    for suite in sorted(registry):
        ...
        while len(indices) < 2:
            digest = hashlib.sha256(
                f"{coverage_sha}:cadref-cvpr2025:{suite}:{attempt}".encode("utf-8")
            ).hexdigest()
...
def valid_output(output: Path, expected_source: dict[str, str]) -> bool:
    ...
    return (
        metrics.get("schema_version") == "strict_v4_mlp_cadref_family_v1"
        ...
        and postprocessor.get("logit_method") == "Energy"
...
def run_one(
    ...
    if completed.returncode:
        raise RuntimeError(f"CADRef run failed for {source}: {completed.stderr[-2000:]}")
    if not valid_output(output, expected_source):
        raise RuntimeError(f"CADRef output validation failed for {source}")
...
def main() -> None:
    ...
    evaluator = base / "evaluate_mlp_cadref.py"
    scorer = base / "caeos" / "cadref_posthoc.py"
    gate_creator = base / "create_strict_v4_cadref_expansion_gate.py"
    pilot_summarizer = base / "summarize_strict_v4_cadref_pilot.py"
    full_summarizer = base / "summarize_strict_v4_cadref_full.py"
    orchestration_script = base / "scripts" / "run_strict_v4_cadref_pilot.sh"
    source_hashes = {
    ...
    protocol = {
        ...
        "status": "frozen_before_cadref_results",
        "mode": args.mode,
        "coverage_manifest_sha256": coverage["manifest_sha256"],
        "selection_rule": (
            "all frozen scenarios"
            if args.mode == "full"
            else "two CADRef-SHA-indexed scenarios per suite independent of metrics"
        ),
        "selected_scenarios": {
            suite: [
                scenario
        ...
        "adapter": {
            "caref_formula": "Eq.6 normalized L1 relative error",
            "cadref_formula": "Eq.10 Ep/Energy(x)+En/mean_train_Energy",
            "centroid_grouping": "known-training predicted class matching official code",
            "logit_method": "Energy",
            "hyperparameter_policy": "official default; no OOD sweep",
            "prediction_source": "unmodified frozen MLP",
        },
        "fit_data": "known_training_embeddings_and_logits_only",
        ...
        "implementation_sha256": {
            ...
            "gate_creator": sha256_file(gate_creator),
            "pilot_summarizer": sha256_file(pilot_summarizer),
            "full_summarizer": sha256_file(full_summarizer),
            "orchestration_script": sha256_file(orchestration_script),
        },
    }
    ...
    if protocol_path.is_file():
        if json.loads(protocol_path.read_text(encoding="utf-8")) != protocol:
            raise ValueError("existing CADRef protocol differs from frozen inputs")
    else:
        if observed:
            raise ValueError("refusing to freeze CADRef protocol after results exist")
    ...
    if completed != len(sources):
        raise RuntimeError(f"CADRef coverage incomplete: {completed}/{len(sources)}")
    summary = {
    ...
    (output_root / "cadref_complete").write_text(
