...
from caeos.odin_posthoc import collect_odin, evidence
...
def parse_arguments() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Evaluate fixed-parameter ODIN from a frozen strict MLP run"
    )
...
def main() -> None:
    ...
    with np.load(source_paths["scores.npz"], allow_pickle=False) as source_scores:
        ...
        if arguments.get("dataset") != "tabular":
            raise ValueError("strict ODIN matrix only accepts tabular runs")
    ...
    result: dict[str, Any] = {
        "schema_version": "strict_v4_mlp_odin_v1",
        "model": "mlp_odin",
        "method": "odin",
        ...
        "validation_thresholds": {"odin": threshold},
        "reports": {"odin": report},
        "implementation": "fixed-parameter OpenOOD ODIN formula on frozen tabular MLP",
        "selection_evidence": {
            "protocol": "strict_frozen_checkpoint_fixed_parameter_odin",
            "source_run": str(source_run),
            "source_artifact_sha256": source_sha256,
            "postprocessor": evidence(args.temperature, args.noise),
    ...
    np.savez_compressed(
        ...
        validation_odin=validation["risk"],
        test_odin=test["risk"],
    )
    provenance = {
        "schema_version": "strict_v4_mlp_odin_provenance_v1",
        "source_run": str(source_run),
        "source_artifact_sha256": source_sha256,
        "implementation_sha256": sha256_file(Path(__file__).resolve()),
        "scorer_module_sha256": sha256_file(
            Path(__file__).resolve().parent / "caeos" / "odin_posthoc.py"
        ),
        "methods": ["odin"],
    ...
    print(json.dumps({"odin": report}, ensure_ascii=False, sort_keys=True), flush=True)
