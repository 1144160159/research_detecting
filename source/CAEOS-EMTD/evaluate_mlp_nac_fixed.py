...
from caeos.nac_posthoc import NACUEFixedCalibrator
...
def parse_arguments() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Evaluate no-leak NAC-UE-Fixed from a frozen strict MLP run")
...
def main() -> None:
    ...
    with np.load(source_paths["scores.npz"], allow_pickle=False) as source_scores:
        ...
        if arguments.get("dataset") != "tabular":
            raise ValueError("strict NAC matrix only accepts tabular runs")
    ...
    calibrator = NACUEFixedCalibrator()
    calibrator.fit(train["embedding"], train["logits"], train["labels"], weights)
    ...
    result: dict[str, Any] = {
        "schema_version": "strict_v4_mlp_nac_ue_fixed_v1", "model": "mlp_nac_ue_fixed", "method": METHOD,
        ...
        "implementation": "NAC-UE formula with official CIFAR-10 avgpool defaults frozen before traffic results",
        "selection_evidence": {
            "protocol": "strict_frozen_checkpoint_nac_ue_fixed_no_ood_tuning",
        ...
        "diagnostics": {
            ...
            "validation_confidence_std": float(validation_output["confidence"].std()),
            "test_confidence_mean": float(test_output["confidence"].mean()),
            "test_confidence_std": float(test_output["confidence"].std()),
        },
    ...
    np.savez_compressed(
        ...
        validation_nac_ue_fixed=validation_output["risk"], test_nac_ue_fixed=test_output["risk"],
    )
    provenance = {
        "schema_version": "strict_v4_mlp_nac_ue_fixed_provenance_v1", "source_run": str(source_run),
        "source_artifact_sha256": source_sha256, "implementation_sha256": sha256_file(Path(__file__).resolve()),
        "scorer_module_sha256": sha256_file(Path(__file__).resolve().parent / "caeos" / "nac_posthoc.py"),
