...
METHODS = ("krc_csr_caeos_v1", "caeos_pairwise")
SOURCE_HASH_KEYS = (
...
def aggregate_benchmarks(
    ...
    for record in records:
        if (
            ...
            or record.get("roundtrip", {}).get("krc_capture", {}).get(
                "passes"
            )
            is not True
            ...
            raise ValueError("invalid KRC selected-system benchmark record")
        ...
        ) or float(source.get("total_capture_wall_seconds", -1.0)) != float(
            expected_source["total_capture_wall_seconds"]
        ):
            raise ValueError("benchmark source binding mismatch")
        seen.add(identity)
        batches = {}
        for batch in protocol["benchmark"]["batch_sizes"]:
            ...
            for metric in LATENCY_METRICS:
                ratios[metric] = positive_ratio(
                    block[METHODS[0]][metric],
                    block[METHODS[1]][metric],
                )
            ratios["samples_per_second"] = positive_ratio(
                block[METHODS[0]]["samples_per_second"],
                block[METHODS[1]]["samples_per_second"],
            )
        ...
        by_scenario[(identity[0], identity[1])].append(ratio)
    ...
    for index, key in enumerate(sorted(ratio_series)):
        ...
        if key.endswith("samples_per_second"):
            efficiency_checks[key] = summary["bootstrap_95ci"][0] >= 1.0
        else:
            efficiency_checks[key] = summary["bootstrap_95ci"][1] <= 1.0
    suite_equal = {
        suite: {
    ...
    return {
        ...
        "raw_per_capture_ratios": raw_ratios,
        ...
        "suite_equal_secondary_summary": {
            ...
            "equal_weight_mean": {
                key: float(
                    np.mean(
                        [
                            suite_equal[suite][key]
                            for suite in suite_equal
                        ]
                    )
                )
        ...
        "strict_efficiency_decision": {
            ...
            "passes": all(efficiency_checks.values()),
...
def summarize(
    ...
    for source in protocol["sources"]:
        ...
        if not path.is_file():
            raise FileNotFoundError(f"missing KRC system benchmark: {path}")
    ...
    value: Dict[str, Any] = {
        "schema_version": "strict_v4_krc_selected_system_summary_v1",
        "state": "complete",
