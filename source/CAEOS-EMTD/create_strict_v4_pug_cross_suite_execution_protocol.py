...
def create_execution_protocol(
    *,
    ...
    if (
        ...
        raise ValueError("activation does not bind the frozen PUG design")
    ...
    if (
        ...
        if (
            ...
            raise ValueError("invalid negative PUG activation")
        return None
    if (
        ...
        raise ValueError("canonical positive PUG activation required")
    ...
    if set(implementation_sha256) != REQUIRED_IMPLEMENTATION_KEYS:
        raise ValueError("complete frozen PUG execution implementation required")
    if any(
    ...
    if (
        ...
        raise ValueError("frozen PUG full102x3 universe required")
    ...
    protocol: dict[str, Any] = {
        ...
        "route_coverage_contract": {
            ...
            "definition_frozen_before_cross_suite_results": True,
        },
        "output_contract": {
            "run_root": "runs/strict_v4_pug_cross_suite_confirmation_v1",
            "result_root": (
                "results/strict_v4_pug_cross_suite_confirmation_v1"
            ),
            "pairwise_pug_task_count": 306,
        ...
        "claim_boundary": {
            ...
            "upstream_incumbent_remains_until_all_selection_gates_pass": True,
        },
    }
...
def main() -> None:
    ...
    if file_hash(design_path) != DESIGN_FILE_SHA256:
        raise ValueError("exact frozen PUG cross-suite design required")
    ...
    if (
        ...
        print("state=not_required_retain_upstream_incumbent")
        return
    ...
    if missing:
        raise ValueError(f"missing PUG execution implementation: {missing}")
    ...
    protocol = create_execution_protocol(
        ...
        input_file_sha256={
            design_path.relative_to(root).as_posix(): file_hash(design_path),
            activation_path.relative_to(root).as_posix(): file_hash(
                activation_path
            ),
        },
