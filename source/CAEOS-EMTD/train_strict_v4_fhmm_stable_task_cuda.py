...
def family_heldout_meta_loss(
    *,
    ...
    inner_gradient_clip_norm: float,
) -> tuple[Any, Any]:
    if inner_learning_rate <= 0.0:
        raise ValueError("meta inner learning rate must be positive")
    if inner_gradient_clip_norm <= 0.0:
        raise ValueError("meta inner gradient clip norm must be positive")
    ...
    with torch.cuda.amp.autocast(enabled=False):
        ...
        if not present_gradients or any(
            not bool(torch.isfinite(gradient).all())
            for gradient in present_gradients
        ):
            raise FloatingPointError("non-finite FP32 meta inner gradient")
        ...
        gradient_scale = torch.clamp(
            inner_gradient_clip_norm / (gradient_norm + 1e-12),
            max=1.0,
        ).detach()
        fast_parameters = {
            name: (
        ...
        episode_outputs = torch.func.functional_call(
            model,
            fast_parameters,
            (episode_features.float(), episode_statistics.float()),
        )
        outer_loss = torch.nn.functional.binary_cross_entropy_with_logits(
            episode_outputs[3].float(),
            episode_attack_targets.float(),
        )
    if not bool(torch.isfinite(inner_loss).all()) or not bool(
        torch.isfinite(outer_loss).all()
    ):
        raise FloatingPointError("non-finite FP32 meta loss")
...
def train_task(args: argparse.Namespace) -> dict[str, Any]:
    ...
    architecture_name = (
        "stable_fp32_first_order_family_heldout_meta_packet_statistic_v2"
        if meta_enabled
        else "dual_metric_contrastive_packet_statistic_v1"
    )
    algorithm_name = (
        "FHMM-S-CAEOS stable family-held-out malicious-boundary meta learner"
        if meta_enabled
    ...
    try:
        for epoch in range(args.epochs):
                ...
                with torch.cuda.amp.autocast():
                    ...
                    if meta_enabled:
                        ...
                        _, meta_outer = family_heldout_meta_loss(
                            ...
                            inner_gradient_clip_norm=(
                                args.meta_inner_gradient_clip_norm
                            ),
                        )
                        total_loss = (
                            total_loss
            ...
            if not epoch_values or not all(
                math.isfinite(float(value)) for value in epoch_values
            ):
                raise FloatingPointError(
                    f"non-finite epoch summary at epoch={epoch}"
                )
            history.append(
                {
                    "epoch": epoch,
        ...
        if any(
            ...
            raise FloatingPointError("best checkpoint contains non-finite values")
    ...
    finite_outputs = [
        validation_probability,
        test_probability,
        *validation_attack_variants.values(),
        *test_attack_variants.values(),
        validation_open_max,
        test_open_max,
        validation_open_noisy_or,
        test_open_noisy_or,
    ]
    if not all(np.isfinite(value).all() for value in finite_outputs):
        raise FloatingPointError("inference outputs contain non-finite values")
    ...
    score_arrays: dict[str, Any] = {
        "validation_family_probability": validation_probability,
        ...
        "test_family_probability": test_probability,
    ...
    gpu_evidence: dict[str, Any] = {
        "schema_version": (
            "strict_v4_fhmm_stable_cuda_task_evidence_v2"
            if meta_enabled
    ...
    report: dict[str, Any] = {
        ...
        "training": {
            ...
            "meta_inner_gradient_clip_norm": (
                args.meta_inner_gradient_clip_norm
            ),
            "outer_gradient_clip_norm": args.gradient_clip_norm,
            "meta_update_order": "first_order_maml_approximation",
            "meta_update_precision": "fp32_autocast_disabled",
            "non_finite_policy": "fail_fast_without_task_artifacts",
            "amp_scale_reduction_count": amp_scale_reduction_count,
            "amp_gradient_overflow_policy": (
                "grad_scaler_skip_step_reduce_scale_and_record"
            ),
            ...
            "pseudo_mix_lambda": args.pseudo_mix_lambda,
        ...
        "source": {
            ...
            "stable_training_core_sha256": file_hash(
        ...
        "claim_boundary": {
            ...
            "fp32_first_order_meta_update": meta_enabled,
            "second_order_meta_gradient_enabled": False,
            "non_finite_training_is_fatal": True,
            "transient_amp_gradient_overflow_is_recorded_not_fatal": True,
    ...
    if not gpu_passes:
        raise RuntimeError("FHMM-S CUDA execution evidence did not pass")
    return report
...
def main() -> None:
    ...
    if args.meta_heldout_loss_weight <= 0.0:
        raise ValueError("FHMM-S requires positive meta heldout loss weight")
