...
from caeos.dynamic_covariance_calibration import DynamicCovarianceCalibration
...
def parse_arguments() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Evaluate DCC from a frozen strict-v4 MLP run"
    )
    ...
    parser.add_argument("--residual-dimension", type=int, default=50)
    parser.add_argument("--ridge", type=float, default=1e-6)
    parser.add_argument("--eigenvalue-floor", type=float, default=1e-8)
...
def validate_source(
    ...
    if arguments.get("model") != "mlp" or metrics.get("model") != "mlp":
        raise ValueError("DCC evaluation requires a frozen MLP source run")
...
def main() -> None:
    ...
    with np.load(source_paths["scores.npz"], allow_pickle=False) as source_scores:
        ...
        if arguments.get("dataset") != "tabular":
            raise ValueError("strict DCC evaluation only accepts tabular runs")
    ...
    calibrator = DynamicCovarianceCalibration(
        residual_dimension=args.residual_dimension,
        ridge=args.ridge,
    ...
    result = {
        "schema_version": "strict_v4_mlp_dcc_v1",
        "model": "mlp_posthoc_dcc",
        "method": METHOD,
        ...
        "implementation": (
            "ICML 2025 DCC adapted to the frozen tabular MLP: row-wise L2-normalized "
            "pre-logit features, fixed residual dimension 50 capped at d-1, per-query "
            "dynamic covariance correction, and known-validation threshold"
        ),
        "selection_evidence": {
    ...
    np.savez_compressed(
        ...
        validation_dcc=validation_risk,
        test_dcc=test_risk,
        prediction_dcc=prediction,
    )
    provenance = {
        "schema_version": "strict_v4_mlp_dcc_provenance_v1",
        "source_run": str(source_run),
        "source_artifact_sha256": source_sha256,
        ...
        "calibrator_module_sha256": sha256_file(
            ...
            / "caeos"
            / "dynamic_covariance_calibration.py"
        ),
        "method": METHOD,
