...
METHODS = ("nac_ue_fixed", "mlp_energy", "opendetect")
...
def analyze(pilot_root: Path, source_root: Path, opendetect_root: Path, gate: dict[str, Any]) -> dict[str, Any]:
    ...
    if gate.get("schema_version") != "strict_v4_mlp_nac_ue_fixed_expansion_gate_v1":
        raise ValueError("unexpected NAC-UE-Fixed expansion gate schema")
    if gate.get("manifest_sha256") != canonical_hash(gate):
        raise ValueError("NAC-UE-Fixed expansion gate SHA mismatch")
    if gate.get("pilot_protocol_manifest_sha256") != protocol.get("manifest_sha256"):
        raise ValueError("NAC-UE-Fixed expansion gate protocol binding mismatch")
    blocks = []
    ...
    for suite, scenarios in protocol["selected_scenarios"].items():
        for scenario in scenarios:
            ...
            evidence = candidate["selection_evidence"]["postprocessor"]
            diagnostics["%s/%s" % (suite, scenario)] = {
                "validation_confidence_std": float(candidate["diagnostics"]["validation_confidence_std"]),
                "test_confidence_std": float(candidate["diagnostics"]["test_confidence_std"]),
                "coverage_nonzero_fraction": float(evidence["coverage_nonzero_fraction"]),
                "coverage_subset_count": int(evidence["coverage_subset_count"]),
            }
            blocks.append({
                "suite": suite, "scenario": scenario,
                "reports": {
                    "nac_ue_fixed": _report(candidate, "nac_ue_fixed", candidate_path),
    ...
    for metric in UNKNOWN_METRICS:
        gains = []
        for block in blocks:
            candidate = block["reports"]["nac_ue_fixed"][metric]
    ...
    candidate_row = next(row for row in overall if row["method"] == "nac_ue_fixed")
    known_f1_max_difference = max(
        abs(block["reports"]["nac_ue_fixed"]["known_macro_f1"] - block["reports"]["mlp_energy"]["known_macro_f1"])
        for block in blocks
    )
    checks = {
        ...
        "nondegenerate_score": all(item["validation_confidence_std"] > 1e-12 and item["test_confidence_std"] > 1e-12 for item in diagnostics.values()),
        "coverage_support": all(item["coverage_nonzero_fraction"] > 0.0 for item in diagnostics.values()),
    ...
    return {
        "schema_version": "strict_v4_mlp_nac_ue_fixed_pilot_analysis_v1",
        ...
        "nac_ue_fixed_vs_mlp_energy_oriented_mean_gains": oriented_gains,
        "nac_ue_fixed_vs_mlp_energy_four_metric_mean_gain": sum(oriented_gains.values()) / len(oriented_gains),
        "nac_ue_fixed_vs_mlp_energy_suite_mean_gains": suite_gains,
        "known_f1_max_absolute_difference": known_f1_max_difference,
        "expansion_checks": checks, "decision": {"expand_nac_ue_fixed_to_full102": all(checks.values())},
    }
...
def render(result: dict[str, Any]) -> str:
    lines = [
        ...
        "Expand NAC-UE-Fixed to full 102: `%s`." % ("YES" if result["decision"]["expand_nac_ue_fixed_to_full102"] else "NO"), "",
