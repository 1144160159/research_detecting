    ...
    for suite in sorted(registry):
        ...
        while len(indices) < 2:
            digest = hashlib.sha256(
                ("%s:gradnorm-posthoc:%s:%d" % (coverage_sha, suite, attempt)).encode(
                    "utf-8"
                )
            ).hexdigest()
...
def valid_output(output: Path, expected_source: dict[str, str]) -> bool:
    ...
    try:
        ...
        return False
    return (
        set(metrics.get("reports", {})) == {"gradnorm"}
        and provenance.get("source_artifact_sha256") == expected_source
        and provenance.get("methods") == ["gradnorm"]
    )
...
def run_one(
    ...
    if completed.returncode:
        raise RuntimeError(
            "GradNorm run failed for %s: %s" % (source, completed.stderr[-2000:])
        )
    if not valid_output(output, expected_source):
        raise RuntimeError("GradNorm output validation failed for %s" % source)
...
def main() -> None:
    ...
    evaluator = base / "evaluate_mlp_gradnorm.py"
    scorer = base / "caeos" / "gradnorm_posthoc.py"
    source_hashes = {
        str(path.relative_to(source_root)): source_artifacts(path) for path in sources
    }
    protocol = {
        "schema_version": "strict_v4_mlp_gradnorm_protocol_v1",
        "status": "frozen_before_gradnorm_results",
        "mode": args.mode,
        "coverage_manifest_sha256": coverage["manifest_sha256"],
        "selection_rule": (
            "all frozen scenarios"
            if args.mode == "full"
            else "two GradNorm-SHA-indexed scenarios per suite, independent of metric values"
        ),
        "selected_scenarios": {
        ...
        "fit_data": "none_frozen_embedding_and_logits_only",
    ...
    if protocol_path.is_file():
        if json.loads(protocol_path.read_text(encoding="utf-8")) != protocol:
            raise ValueError("existing GradNorm protocol differs from frozen inputs")
    else:
    ...
    if completed != len(sources):
        raise RuntimeError("GradNorm coverage incomplete: %d/%d" % (completed, len(sources)))
    summary = {
        "schema_version": "strict_v4_mlp_gradnorm_matrix_v1",
        "status": "complete",
        "mode": args.mode,
        "expected_runs": len(sources),
        "completed_runs": completed,
        "failures": 0,
        "methods_per_run": ["gradnorm"],
    ...
    (output_root / "gradnorm_complete").write_text(
