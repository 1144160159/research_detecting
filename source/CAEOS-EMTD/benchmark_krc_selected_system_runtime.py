...
def benchmark_once(runtime, views: Sequence[np.ndarray]) -> float:
...
def benchmark(
    *,
    ...
    if (
        ...
        raise ValueError("canonical KRC selected-system protocol required")
    identity = (
        str(source["suite"]),
        str(source["scenario"]),
    ...
    if len(matches) != 1:
        raise ValueError("KRC system source identity is not in protocol")
    expected = matches[0]
    ...
    inputs_path = capture_dir / str(manifest.get("evaluation_inputs", ""))
    if (
        ...
        or capture_dir.resolve() != Path(expected["capture_dir"]).resolve()
        or file_hash(manifest_path)
        != expected["capture_manifest_file_sha256"]
        or file_hash(execution_path)
        != expected["capture_execution_file_sha256"]
        ...
        != float(expected["total_capture_wall_seconds"])
        or file_hash(artifact) != expected["krc_runtime_sha256"]
        or file_hash(inputs_path) != expected["evaluation_inputs_sha256"]
        or manifest.get("roundtrip", {}).get("passes") is not True
    ):
        raise ValueError("invalid KRC selected-system source capture")
    ...
    joblib.dump(runtime.base_runtime.clean_runtime, pairwise_artifact, compress=3)
    pairwise = joblib.load(pairwise_artifact)
    original = runtime.base_runtime.clean_runtime.predict(views)
    reloaded = pairwise.predict(views)
    pairwise_roundtrip = {
        "prediction_array_equal": bool(
            np.array_equal(original["prediction"], reloaded["prediction"])
        ),
    ...
    for batch_size in policy["batch_sizes"]:
        ...
        for _ in range(int(policy["warmup_repetitions"])):
            ...
            pairwise.predict(batch)
        timings = {"krc_csr_caeos_v1": [], "caeos_pairwise": []}
        methods = {
            "krc_csr_caeos_v1": runtime,
            "caeos_pairwise": pairwise,
        }
        for repetition in range(int(policy["timed_repetitions"])):
            order = ["krc_csr_caeos_v1", "caeos_pairwise"]
            if repetition % 2:
                order.reverse()
            for name in order:
                timings[name].append(benchmark_once(methods[name], batch))
    ...
    peak_after = peak_rss()
    value: Dict[str, Any] = {
        "schema_version": "strict_v4_krc_selected_system_benchmark_v1",
        "state": "complete",
