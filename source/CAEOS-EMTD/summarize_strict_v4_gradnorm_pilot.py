...
METHODS = ("gradnorm", "mlp_energy", "opendetect")
...
def analyze(pilot_root, source_root, opendetect_root, gate):
    ...
    if gate.get("schema_version") != "strict_v4_mlp_gradnorm_expansion_gate_v1":
        raise ValueError("unexpected GradNorm gate schema")
    if gate.get("manifest_sha256") != canonical_hash(gate):
        raise ValueError("GradNorm gate SHA mismatch")
    if gate.get("pilot_protocol_manifest_sha256") != protocol.get("manifest_sha256"):
        raise ValueError("GradNorm gate protocol binding mismatch")
    blocks = []
            ...
            blocks.append(
                {
                    ...
                    "scenario": scenario,
                    "reports": {
                        "gradnorm": _report(payloads["new"], "gradnorm", paths["new"]),
                        "mlp_energy": _report(payloads["source"], "energy", paths["source"]),
                        "opendetect": _report(payloads["external"], "opendetect", paths["external"]),
                    },
                }
            )
    overall = []
    ...
    for metric in UNKNOWN_METRICS:
        values = []
        for block in blocks:
            candidate = block["reports"]["gradnorm"][metric]
    ...
    candidate_row = next(row for row in overall if row["method"] == "gradnorm")
    known_diff = max(
        abs(block["reports"]["gradnorm"]["known_macro_f1"] - block["reports"]["mlp_energy"]["known_macro_f1"])
        for block in blocks
    )
    checks = {
    ...
    return {
        "schema_version": "strict_v4_mlp_gradnorm_pilot_analysis_v1",
        ...
        "overall": overall,
        "gradnorm_vs_mlp_energy_oriented_mean_gains": gains,
        "gradnorm_vs_mlp_energy_four_metric_mean_gain": sum(gains.values()) / 4.0,
        "gradnorm_vs_mlp_energy_suite_mean_gains": suite_gains,
        "known_f1_max_absolute_difference": known_diff,
        "expansion_checks": checks,
        "decision": {"expand_gradnorm_to_full102": all(checks.values())},
    }
...
def render(result):
    lines = [
        ...
        "",
        "Expand GradNorm to full 102: `%s`." % ("YES" if result["decision"]["expand_gradnorm_to_full102"] else "NO"),
        "",
