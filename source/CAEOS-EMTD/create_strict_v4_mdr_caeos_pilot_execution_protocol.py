...
def create_protocol(
    ...
    *,
    ...
    if any(int(value) != 0 for value in observed_counts.values()):
        raise ValueError("MDR execution protocol requires zero pilot outputs")
    expected = {
        (suite, scenario)
    ...
    if observed != expected or len(source_records) != 14:
        raise ValueError("MDR source registry must cover the frozen 14 scenarios")
    if set(implementation) != set(implementation_sha256):
        raise ValueError("MDR implementation path/hash keys differ")
    value: Dict[str, Any] = {
        "schema_version": (
        ...
        "execution_plan": {
            ...
            * 2,
            "global_weight_selection_after_all_captures": True,
            "pilot_evaluation_count": design["pilot"][
                "expected_evaluations"
            ],
            "selected_runtime_reused_across_six_conditions": True,
            "resumable_by_canonical_capture_and_evaluation": True,
            "outer_workers": 1,
            "trainer_jobs": 8,
        },
        "resource_policy": {
            ...
            "nice_level": 19,
            "ionice_class": "idle",
            "no_parallel_mdr_training": True,
        },
        "leakage_policy": {
            "known_training_for_model_fit_only": True,
            "known_validation_for_weight_health_and_risk_calibration_only": True,
    ...
    for suite, scenarios in sorted(design["pilot"]["scenarios"].items()):
        for scenario in scenarios:
            ...
            if Path(command[1]).name != "train_hybrid_open_set.py":
                raise ValueError("MDR source must use the frozen Pairwise trainer")
            arguments = command[2:]
            if (
                option_value(arguments, "--test-corruption-kind") != "none"
                or int(option_value(arguments, "--seed")) != 137
            ):
                raise ValueError("MDR source command is not a clean seed137 run")
            ...
            records.append(
                {
                    ...
                    "fresh_training_seed_replaces_source_split_seed": int(
...
def main() -> None:
    ...
    value = create_protocol(
        ...
        source_records=build_source_records(design, comparative, project_root),
        implementation=implementation,
        implementation_sha256=implementation_sha256,
