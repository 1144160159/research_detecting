...
TASKS = (
    {"split_seed": 43, "model_seeds": [163, 167, 173]},
    {"split_seed": 47, "model_seeds": [179, 181, 191]},
)
IMPLEMENTATIONS = (
    "train_strict_v4_fhmm_stable_task_cuda.py",
    "train_strict_v4_fhmm_stable_70_10_20_task_cuda.py",
    "evaluate_strict_v4_fhmm_attack_routing_development_v3.py",
    "complete_strict_v4_fhmm_70_10_20_development.py",
    "create_strict_v4_fhmm_70_10_20_development_protocol.py",
    "audit_strict_v4_gpu_execution_resource.py",
    "scripts/run_strict_v4_fhmm_70_10_20_development_v1.sh",
)


def require_empty(path: Path) -> None:
    if path.exists() and any(path.iterdir()):
        raise ValueError(f"development target is not empty: {path}")
...
def build_protocol(
    *,
    ...
    if file_hash(sequence_dataset) != DATASET_SHA256:
        raise ValueError("70/10/20 development dataset hash drifted")
    ...
    protocol: dict[str, Any] = {
        "schema_version": "strict_v4_fhmm_70_10_20_development_protocol_v1",
        "state": "frozen_before_development_training",
        "algorithm": {
            "name": "FHMM-DS-CAEOS",
            "change_under_test": (
                "known families use 70/10/20 instead of 60/20/20"
            ),
            "unknown_family_split": "all rows remain test only",
        },
        ...
        "claim_boundary": {
            ...
            "botnet_labels_may_select_development_routing": True,
            "botnet_excluded_from_future_confirmation_claim": True,
            "split43_and_split47_not_future_confirmation": True,
            "formal_training_on_gpu_server_only": True,
            "only_known_split_ratio_changes": True,
        },
    }
