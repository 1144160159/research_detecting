...
METHOD = "optfs_vanilla"


def parse_arguments() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Evaluate official-default OptFS Ours (V) from a frozen strict MLP run")
    parser.add_argument("--source-run", type=Path, required=True)
    parser.add_argument("--output-dir", type=Path, required=True)
    parser.add_argument("--device", default="auto")
    parser.add_argument("--batch-size", type=int, default=1024)
    parser.add_argument("--num-workers", type=int, default=0)
    parser.add_argument("--quantile-epsilon", type=float, default=0.001)
    parser.add_argument("--bin-count", type=int, default=100)
    parser.add_argument("--theta-norm", type=float, default=1000.0)
...
def main() -> None:
    ...
    with np.load(source_paths["scores.npz"], allow_pickle=False) as source_scores:
        ...
        if arguments.get("dataset") != "tabular":
            raise ValueError("strict OptFS matrix only accepts tabular runs")
    ...
    calibrator = OptFSCalibrator(args.quantile_epsilon, args.bin_count, args.theta_norm)
    weight = model.classifier.weight.detach().cpu().numpy()
    bias = model.classifier.bias.detach().cpu().numpy()
    calibrator.fit(train["embedding"], train["logits"], weight, bias)
    validation_output = calibrator.evaluate(validation["embedding"], validation["logits"])
    test_output = calibrator.evaluate(test["embedding"], test["logits"])
    acceptance = float(arguments.get("known_acceptance", 0.95))
    threshold, report = report_for(
        test["labels"], test["unknown"].astype(bool), test_output["prediction"],
        validation_output["risk"], test_output["risk"], acceptance,
    )
    ...
    result: dict[str, Any] = {
        "schema_version": "strict_v4_mlp_optfs_v1", "model": "mlp_optfs_vanilla", "method": METHOD,
        "unknown_classes": unknown_classes, "seed": int(arguments["seed"]),
        "known_class_names": bundle.class_names, "sample_counts": bundle.sample_counts,
        ...
        "validation_thresholds": {METHOD: threshold}, "reports": {METHOD: report},
        "implementation": "official fixed-weight OptFS Ours (V) on frozen tabular MLP",
        "selection_evidence": {
            "protocol": "strict_frozen_checkpoint_official_default_optfs_vanilla",
        ...
        "diagnostics": {
            "validation_confidence_mean": float(validation_output["confidence"].mean()),
            "test_confidence_mean": float(test_output["confidence"].mean()),
            "validation_shaped_nonzero_fraction": float(np.count_nonzero(validation_output["shaped_feature"]) / validation_output["shaped_feature"].size),
            "test_shaped_nonzero_fraction": float(np.count_nonzero(test_output["shaped_feature"]) / test_output["shaped_feature"].size),
        },
    ...
    np.savez_compressed(
        args.output_dir / "scores.npz", validation_labels=validation["labels"], test_labels=test["labels"],
        test_unknown=test["unknown"].astype(bool), test_prediction=test_output["prediction"],
        validation_optfs_vanilla=validation_output["risk"], test_optfs_vanilla=test_output["risk"],
    )
    provenance = {
        "schema_version": "strict_v4_mlp_optfs_provenance_v1", "source_run": str(source_run),
        "source_artifact_sha256": source_sha256, "implementation_sha256": sha256_file(Path(__file__).resolve()),
        "scorer_module_sha256": sha256_file(Path(__file__).resolve().parent / "caeos" / "optfs_posthoc.py"),
        "methods": [METHOD], "split_fingerprint": bundle.split_metadata.get("split_fingerprint"),
    }
