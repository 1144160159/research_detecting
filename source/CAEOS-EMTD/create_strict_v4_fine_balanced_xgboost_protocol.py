...
IMPLEMENTATIONS = (
    ...
    "create_strict_v4_fine_balanced_xgboost_protocol.py",
...
def cache_audit(cache_csv: Path, label_column: str) -> dict[str, Any]:
    frame = pd.read_csv(
        cache_csv,
    ...
    unknown = sorted(set(labels) - set(FINE_TO_FAMILY))
    if unknown:
        raise ValueError(f"fine-balanced cache has unmapped labels: {unknown}")
    missing = sorted(set(FINE_TO_FAMILY) - set(labels))
    if missing:
        raise ValueError(f"fine-balanced cache misses labels: {missing}")
    counts = {
        str(label): int(count)
        for label, count in labels.value_counts().sort_index().items()
    }
    if any(count > 5000 for count in counts.values()):
        raise ValueError("fine-balanced cache exceeds per-class maximum")
...
def build_protocol(args: argparse.Namespace) -> dict[str, Any]:
    ...
    audit = cache_audit(cache_csv, label_column)
    if audit["rows"] != args.expected_rows:
        raise ValueError(
            f"fine-balanced cache row count is {audit['rows']}, "
    ...
    protocol: dict[str, Any] = {
        "schema_version": (
            "strict_v4_fine_balanced_xgboost_protocol_v1"
        ),
        ...
        "claim_boundary": {
            ...
            "fine_class_balancing_precedes_family_holdout": True,
            "unknown_fine_classes_are_excluded_from_train_and_validation": True,
        },
    }
...
def parse_arguments() -> argparse.Namespace:
    ...
    parser.add_argument("--expected-rows", type=int, default=49193)
