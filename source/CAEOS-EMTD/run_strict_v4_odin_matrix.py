...
def select_pilot_scenarios(coverage: dict[str, Any]) -> dict[str, list[str]]:
    ...
    for suite in sorted(registry):
        ...
        while len(indices) < 2:
            digest = hashlib.sha256(
                ("%s:odin-posthoc:%s:%d" % (coverage_sha, suite, attempt)).encode(
                    "utf-8"
                )
            ).hexdigest()
...
def valid_output(output: Path, expected_source: dict[str, str]) -> bool:
    ...
    try:
        ...
        return False
    return (
        set(metrics.get("reports", {})) == {"odin"}
        and provenance.get("source_artifact_sha256") == expected_source
        and provenance.get("methods") == ["odin"]
        and metrics.get("arguments", {}).get("temperature") == ODIN_TEMPERATURE
        and metrics.get("arguments", {}).get("noise") == ODIN_NOISE
    )
...
def run_one(
    ...
    if completed.returncode:
        raise RuntimeError("ODIN run failed for %s: %s" % (source, completed.stderr[-2000:]))
    if not valid_output(output, expected_source):
        raise RuntimeError("ODIN output validation failed for %s" % source)
...
def main() -> None:
    ...
    protocol = {
        ...
        "formula": "temperature scaling plus negative input-gradient-sign perturbation",
        "temperature": ODIN_TEMPERATURE,
        "noise": ODIN_NOISE,
        "parameter_policy": (
            "fixed before results; no OOD tuning; noise is in already-standardized "
            "tabular feature coordinates"
        ),
        "classification_prediction": "unperturbed frozen MLP",
        "fit_data": "none",
    ...
    if protocol_path.is_file():
        ...
        if existing != protocol:
            raise ValueError("existing ODIN protocol differs from frozen inputs")
    else:
    ...
    if completed != len(sources):
        raise RuntimeError("ODIN coverage incomplete: %d/%d" % (completed, len(sources)))
    summary = {
        "schema_version": "strict_v4_mlp_odin_matrix_v1",
        "status": "complete",
        "mode": args.mode,
        "expected_runs": len(sources),
        "completed_runs": completed,
        "failures": 0,
        "methods_per_run": ["odin"],
    ...
    (output_root / "odin_complete").write_text(
