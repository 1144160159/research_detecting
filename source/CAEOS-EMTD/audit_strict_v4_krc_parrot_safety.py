...
from summarize_strict_v4_krc_parrot_safety import aggregate, load
...
def audit(
    ...
    for source in protocol.get("source_model_pairs", []):
        ...
        if (
            ...
            or file_hash(path)
            != registry[identity].get("metrics_file_sha256")
        ):
    ...
    checks = {
        ...
        "all_30_model_pair_metrics_bound_by_hash": (
        ...
        "candidate_model_not_refit_for_parrot": all(
            item.get("candidate_model_refit_for_parrot") is False
            for item in records
        ),
    ...
    value: Dict[str, Any] = {
        "schema_version": "strict_v4_krc_parrot_safety_audit_v1",
