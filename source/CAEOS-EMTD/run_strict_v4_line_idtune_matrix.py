...
def select_pilot_scenarios(coverage: dict[str, Any]) -> dict[str, list[str]]:
    ...
    for suite in sorted(registry):
        ...
        attempt = 0
        while len(indices) < 2:
            digest = hashlib.sha256(("%s:line-idtune:%s:%d" % (coverage_sha, suite, attempt)).encode("utf-8")).hexdigest()
...
def run_one(evaluator: Path, source: Path, output: Path, device: str, expected_source: dict[str, str]) -> dict[str, str]:
    ...
    if completed.returncode:
        raise RuntimeError("LINe-IDTune run failed for %s: %s" % (source, completed.stderr[-3000:]))
    if not valid_output(output, expected_source):
        raise RuntimeError("LINe-IDTune output validation failed for %s" % source)
...
def main() -> None:
    ...
    evaluator = base / "evaluate_mlp_line_idtune.py"
    scorer = base / "caeos" / "line_idtune_posthoc.py"
    source_hashes = {str(path.relative_to(source_root)): source_artifacts(path) for path in sources}
    protocol = {
        "schema_version": "strict_v4_mlp_line_idtune_protocol_v1",
        "status": "frozen_before_line_idtune_results", "mode": args.mode,
        "coverage_manifest_sha256": coverage["manifest_sha256"],
        "selection_rule": "all frozen scenarios" if args.mode == "full" else "two LINe-IDTune-SHA-indexed scenarios per suite independent of metric values",
        "selected_scenarios": {
        ...
        "paper": "https://openaccess.thecvf.com/content/CVPR2023/papers/Ahn_LINe_Out-of-Distribution_Detection_by_Leveraging_Important_Neurons_CVPR_2023_paper.pdf",
        "official_implementation": "https://github.com/YongHyun-Ahn/LINe-Out-of-Distribution-Detection-by-Leveraging-Important-Neurons",
        "official_implementation_commit": "465ddf584d62c8e8bbf18c014aa48f1d3c1e7532",
        "formula": "official LINe Taylor contribution, AP, WP, upper clipping, and Energy confidence",
        "pruning_candidates": PRUNING_CANDIDATES, "clip_quantiles": CLIP_QUANTILES,
        "parameter_policy": "known-training quantile thresholds and minimum known-validation NLL only; no OOD sweep",
        "claim_boundary": "LINe-IDTune paper-formula-consistent adapter, not official fixed-default reproduction",
        "fit_data": "known_training_features_labels_and_known_validation_only",
    ...
    if protocol_path.is_file():
        if json.loads(protocol_path.read_text(encoding="utf-8")) != protocol:
            raise ValueError("existing LINe-IDTune protocol differs from frozen inputs")
    else:
    ...
    if completed != len(sources):
        raise RuntimeError("LINe-IDTune coverage incomplete: %d/%d" % (completed, len(sources)))
    summary = {
        "schema_version": "strict_v4_mlp_line_idtune_matrix_v1", "status": "complete", "mode": args.mode,
    ...
    (output_root / "line_idtune_complete").write_text(protocol["manifest_sha256"] + "\n", encoding="ascii")
