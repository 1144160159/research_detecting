...
METHODS = ("klm", "mlp_energy", "opendetect")
...
def analyze(
    ...
    if gate.get("schema_version") != "strict_v4_mlp_klm_expansion_gate_v1":
        raise ValueError("unexpected KLM expansion gate schema")
    if gate.get("manifest_sha256") != canonical_hash(gate):
        raise ValueError("KLM expansion gate SHA mismatch")
    if gate.get("pilot_protocol_manifest_sha256") != protocol.get("manifest_sha256"):
        raise ValueError("KLM expansion gate protocol binding mismatch")
    ...
    for metric in UNKNOWN_METRICS:
        values = []
        for block in blocks:
            candidate = block["reports"]["klm"][metric]
    ...
    klm_row = next(row for row in overall if row["method"] == "klm")
    known_f1_max_difference = max(
        abs(
            block["reports"]["klm"]["known_macro_f1"]
    ...
    checks = {
        ...
        "top_two_rank": klm_row["mean_unknown_metric_rank"] <= 2.0,
    ...
    return {
        "schema_version": "strict_v4_mlp_klm_pilot_analysis_v1",
        ...
        "overall": overall,
        "klm_vs_mlp_energy_oriented_mean_gains": oriented_gains,
        "klm_vs_mlp_energy_four_metric_mean_gain": sum(oriented_gains.values())
        / len(oriented_gains),
        "klm_vs_mlp_energy_suite_mean_gains": suite_gains,
        "known_f1_max_absolute_difference": known_f1_max_difference,
        "expansion_checks": checks,
        "decision": {"expand_klm_to_full102": all(checks.values())},
    }
...
def render(result: dict[str, Any]) -> str:
    lines = [
        ...
        "",
        "Expand KLM to full 102: `%s`."
        % ("YES" if result["decision"]["expand_klm_to_full102"] else "NO"),
        "",
