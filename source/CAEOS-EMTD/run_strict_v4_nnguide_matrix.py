...
def select_pilot_scenarios(coverage: dict[str, Any]) -> dict[str, list[str]]:
    ...
    for suite in sorted(registry):
        ...
        while len(indices) < 2:
            digest = hashlib.sha256(
                ("%s:nnguide-energy:%s:%d" % (coverage_sha, suite, attempt)).encode("utf-8")
            ).hexdigest()
...
def valid_output(output: Path, expected_source: dict[str, str]) -> bool:
    ...
    return (
        ...
        and arguments.get("bank_ratio") == BANK_RATIO
        and arguments.get("neighbor_count") == NEIGHBOR_COUNT
        and arguments.get("bank_seed") == BANK_SEED
    )
...
def run_one(evaluator: Path, source: Path, output: Path, device: str, expected_source: dict[str, str]) -> dict[str, str]:
    ...
    command = [
        sys.executable, str(evaluator), "--source-run", str(source), "--output-dir", str(output),
        "--device", device, "--bank-ratio", str(BANK_RATIO), "--neighbor-count", str(NEIGHBOR_COUNT),
        "--bank-seed", str(BANK_SEED),
    ]
    completed = subprocess.run(command, text=True, capture_output=True)
    (output / "run.log").write_text(completed.stdout + completed.stderr, encoding="utf-8")
    if completed.returncode:
        raise RuntimeError("NNGuide run failed for %s: %s" % (source, completed.stderr[-3000:]))
    if not valid_output(output, expected_source):
        raise RuntimeError("NNGuide output validation failed for %s" % source)
...
def main() -> None:
    ...
    evaluator = base / "evaluate_mlp_nnguide.py"
    scorer = base / "caeos" / "nnguide_posthoc.py"
    source_hashes = {str(path.relative_to(source_root)): source_artifacts(path) for path in sources}
    protocol = {
        "schema_version": "strict_v4_mlp_nnguide_protocol_v1",
        "status": "frozen_before_nnguide_results", "mode": args.mode,
        "coverage_manifest_sha256": coverage["manifest_sha256"],
        "selection_rule": "all frozen scenarios" if args.mode == "full" else "two NNGuide-SHA-indexed scenarios per suite independent of metric values",
        "selected_scenarios": {
        ...
        "paper": "https://openaccess.thecvf.com/content/ICCV2023/papers/Park_Nearest_Neighbor_Guidance_for_Out-of-Distribution_Detection_ICCV_2023_paper.pdf",
        "official_implementation": "https://github.com/roomo7time/nnguide",
        "official_implementation_commit": "c123cac961b17a6c4f11adefd9ad861298be1469",
        "formula": "official NNGuide with Energy base confidence",
        "bank_ratio": BANK_RATIO, "neighbor_count": NEIGHBOR_COUNT, "bank_seed": BANK_SEED,
        "parameter_policy": "paper and official source defaults; no OOD sweep",
        "fit_data": "known_training_features_and_logits_only",
        ...
        "small_bank_adapter": "max(k,floor(alpha*N)); required only if official ratio yields fewer than k",
    ...
    if protocol_path.is_file():
        if json.loads(protocol_path.read_text(encoding="utf-8")) != protocol:
            raise ValueError("existing NNGuide protocol differs from frozen inputs")
    else:
    ...
    if completed != len(sources):
        raise RuntimeError("NNGuide coverage incomplete: %d/%d" % (completed, len(sources)))
    summary = {
        "schema_version": "strict_v4_mlp_nnguide_matrix_v1", "status": "complete", "mode": args.mode,
    ...
    (output_root / "nnguide_complete").write_text(protocol["manifest_sha256"] + "\n", encoding="ascii")
