...
def parse_arguments() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Evaluate fixed frozen-encoder DOC on a strict MLP run")
...
def main() -> None:
    ...
    with np.load(source_paths["scores.npz"], allow_pickle=False) as source_scores:
        ...
        if arguments.get("dataset") != "tabular":
            raise ValueError("strict DOC matrix only accepts tabular runs")
        ...
        initial_weight = model.classifier.weight.detach().cpu().numpy()
        initial_bias = model.classifier.bias.detach().cpu().numpy()

    calibrator = DOCFixedCalibrator(alpha=args.alpha, max_iter=args.max_iter).fit(
        train["embedding"], train["labels"], initial_weight, initial_bias,
    )
    ...
    result: dict[str, Any] = {
        "schema_version": "strict_v4_mlp_doc_fixed_v1", "model": "mlp_doc_fixed", "method": METHOD,
        ...
        "implementation": "frozen MLP encoder plus DOC one-vs-rest sigmoid head and per-class Gaussian thresholds",
        "selection_evidence": {
            "protocol": "strict_frozen_encoder_doc_fixed_known_only",
            "source_run": str(source_run), "source_artifact_sha256": source_sha256,
            "doc": calibrator.evidence(),
            "risk": "negative maximum classwise sigmoid-minus-Gaussian-threshold margin",
        ...
        "diagnostics": {
            "validation_risk_std": float(validation_output.risk.std()),
            "test_risk_std": float(test_output.risk.std()),
            "validation_native_reject_fraction": float(validation_output.native_reject.mean()),
            "test_native_reject_fraction": float(test_output.native_reject.mean()),
        },
    ...
    np.savez_compressed(
        ...
        validation_doc_fixed=validation_output.risk, test_doc_fixed=test_output.risk,
    )
    provenance = {
        "schema_version": "strict_v4_mlp_doc_fixed_provenance_v1", "source_run": str(source_run),
        "source_artifact_sha256": source_sha256, "implementation_sha256": sha256_file(Path(__file__).resolve()),
        "scorer_module_sha256": sha256_file(Path(__file__).resolve().parent / "caeos" / "doc_fixed.py"),
