...
def create_protocol(
    ...
    *,
    ...
    if pilot.get("schema_version") != "mal_tls_counterfactual_conflict_gate_protocol_v1":
        raise ValueError("unexpected counterfactual pilot protocol schema")
    if pilot.get("manifest_sha256") != canonical_hash(pilot):
        raise ValueError("counterfactual pilot protocol SHA mismatch")
    if analysis.get("schema_version") != "mal_tls_counterfactual_conflict_gate_analysis_v1":
        raise ValueError("unexpected counterfactual pilot analysis schema")
    if analysis.get("protocol_manifest_sha256") != pilot["manifest_sha256"]:
        raise ValueError("counterfactual pilot analysis is not bound to the protocol")
    if analysis.get("passes") is not True or analysis.get("decision") != (
        "freeze_for_reserved_seed_confirmation"
    ):
        raise ValueError("counterfactual pilot did not authorize confirmation")
    if observed_metrics != 0:
        raise ValueError("counterfactual confirmation must freeze before results")
    seeds = pilot["reserved_confirmation"]["seeds"]
    if seeds != [203, 205, 207]:
        raise ValueError("unexpected counterfactual confirmation seeds")
    protocol: dict[str, Any] = {
        "schema_version": "mal_tls_counterfactual_conflict_gate_confirmation_protocol_v1",
        ...
        "known_only_counterfactual_training": pilot[
...
def main() -> None:
    ...
    names = (
        ...
        "analyze_mal_tls_counterfactual_conflict_gate_confirmation.py",
        "scripts/run_mal_tls_counterfactual_conflict_gate_confirmation.sh",
    )
    for name in (
        "caeos/model.py",
        "caeos/training.py",
