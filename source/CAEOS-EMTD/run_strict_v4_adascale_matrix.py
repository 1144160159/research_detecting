...
def select_pilot_scenarios(coverage: dict[str, Any]) -> dict[str, list[str]]:
    ...
    for suite in sorted(registry):
        ...
        while len(indices) < 2:
            digest = hashlib.sha256(
                ("%s:adascale-a-posthoc:%s:%d" % (coverage_sha, suite, attempt)).encode(
                    "utf-8"
                )
            ).hexdigest()
...
def valid_output(output: Path, expected_source: dict[str, str]) -> bool:
    ...
    return (
        ...
        and arguments.get("p_min") == P_MIN
        and arguments.get("p_max") == P_MAX
        and arguments.get("k1_percent") == K1_PERCENT
        and arguments.get("k2_percent") == K2_PERCENT
        and arguments.get("lmbda") == LAMBDA
        and arguments.get("perturb_fraction") == PERTURB_FRACTION
        and arguments.get("epsilon") == EPSILON
        and arguments.get("temperature") == TEMPERATURE
    )
...
def run_one(
    ...
    command = [
        ...
        "--p-min", str(P_MIN),
        "--p-max", str(P_MAX),
        "--k1-percent", str(K1_PERCENT),
        "--k2-percent", str(K2_PERCENT),
        "--lmbda", str(LAMBDA),
        "--perturb-fraction", str(PERTURB_FRACTION),
        "--epsilon", str(EPSILON),
    ...
    if completed.returncode:
        raise RuntimeError(
            "AdaSCALE run failed for %s: %s" % (source, completed.stderr[-3000:])
        )
    if not valid_output(output, expected_source):
        raise RuntimeError("AdaSCALE output validation failed for %s" % source)
...
def main() -> None:
    ...
    evaluator = base / "evaluate_mlp_adascale.py"
    scorer = base / "caeos" / "adascale_posthoc.py"
    source_hashes = {
    ...
    protocol = {
        ...
        "status": "frozen_before_adascale_results",
        "mode": args.mode,
        "coverage_manifest_sha256": coverage["manifest_sha256"],
        "selection_rule": (
            "all frozen scenarios"
            if args.mode == "full"
            else "two AdaSCALE-SHA-indexed scenarios per suite independent of metric values"
        ),
        "selected_scenarios": {
            suite: [
                scenario
        ...
        "methods": [METHOD],
        "paper": "https://arxiv.org/abs/2503.08023",
        "official_implementation": "https://github.com/sudarshanregmi/AdaSCALE",
        "official_implementation_commit": "ed5f639e10520a04f6a83f30a32c060d6f012ea8",
        "formula": "AdaSCALE-A paper Algorithm 1 with k1 shift Q and k2 correction C_o",
        "p_min": P_MIN,
        "p_max": P_MAX,
        "k1_percent": K1_PERCENT,
        "k2_percent": K2_PERCENT,
        "lambda": LAMBDA,
        "perturb_fraction": PERTURB_FRACTION,
        "epsilon": EPSILON,
        "temperature": TEMPERATURE,
        "parameter_policy": (
            "official fixed defaults; p_max=85 frozen from official default config and paper sensitivity setting"
        ),
        "low_dimensional_adapter": (
            "ReLU GELU embeddings and floor percentage counts with minimum one feature"
        ),
        "input_adapter": (
            "global lowest absolute predicted-logit gradient over concatenated standardized views"
        ),
        "official_source_discrepancy": (
            "ECDF setup reverses k1 and k2; paper Algorithm 1 and inference function are used"
        ),
    ...
    if protocol_path.is_file():
        ...
        if existing != protocol:
            raise ValueError("existing AdaSCALE protocol differs from frozen inputs")
    else:
    ...
    if completed != len(sources):
        raise RuntimeError("AdaSCALE coverage incomplete: %d/%d" % (completed, len(sources)))
    summary = {
        "schema_version": "strict_v4_mlp_adascale_matrix_v1",
        "status": "complete",
        "mode": args.mode,
        "expected_runs": len(sources),
        "completed_runs": completed,
        "failures": 0,
        "methods_per_run": [METHOD],
    ...
    (output_root / "adascale_complete").write_text(
