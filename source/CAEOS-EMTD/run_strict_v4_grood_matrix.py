    ...
    for suite in sorted(registry):
        ...
        while len(indices) < 2:
            digest = hashlib.sha256(
                ("%s:grood-tmlr2025:%s:%d" % (coverage_sha, suite, attempt)).encode(
                    "utf-8"
                )
            ).hexdigest()
...
def run_one(
    ...
    if completed.returncode:
        raise RuntimeError("GROOD run failed for %s: %s" % (source, completed.stderr[-2000:]))
    if not valid_output(output, expected_source):
        raise RuntimeError("GROOD output validation failed for %s" % source)
...
def build_protocol(
    ...
    output_root: Path,
    mode: str,
) -> tuple[dict[str, Any], list[Path], dict[str, dict[str, str]]]:
    ...
    protocol = {
        ...
        "methods": [METHOD],
        "paper": "https://openreview.net/forum?id=2V7itvvMVJ",
        ...
        "adaptation": (
            "official synthetic OOD prototype and OOD-prototype-gradient 1NN score; "
            "penultimate tabular mixup replaces image intermediate-layer mixup"
        ),
    ...
    return protocol, sources, source_hashes
...
def main() -> None:
    ...
    protocol, sources, source_hashes = build_protocol(
        coverage, source_root, output_root, args.mode
    )
    ...
    if protocol_path.is_file():
        if json.loads(protocol_path.read_text(encoding="utf-8")) != protocol:
            raise ValueError("existing GROOD protocol differs from frozen inputs")
    else:
    ...
    evaluator = Path(__file__).resolve().parent / "evaluate_mlp_grood.py"
    results = []
    ...
    if completed != len(sources):
        raise RuntimeError("GROOD coverage incomplete: %d/%d" % (completed, len(sources)))
    summary = {
        "schema_version": "strict_v4_mlp_grood_matrix_v1",
        "status": "complete",
        "mode": args.mode,
        "expected_runs": len(sources),
    ...
    (output_root / "grood_complete").write_text(
