...
METHOD = "nnguide_energy"


def parse_arguments() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Evaluate official-default NNGuide-Energy from a frozen strict MLP run")
    parser.add_argument("--source-run", type=Path, required=True)
    parser.add_argument("--output-dir", type=Path, required=True)
    parser.add_argument("--device", default="auto")
    parser.add_argument("--batch-size", type=int, default=1024)
    parser.add_argument("--num-workers", type=int, default=0)
    parser.add_argument("--bank-ratio", type=float, default=0.01)
...
def main() -> None:
    ...
    with np.load(source_paths["scores.npz"], allow_pickle=False) as source_scores:
        ...
        if arguments.get("dataset") != "tabular":
            raise ValueError("strict NNGuide matrix only accepts tabular runs")
    ...
    calibrator = NNGuideCalibrator(args.bank_ratio, args.neighbor_count, args.bank_seed)
    calibrator.fit(train["embedding"], train["logits"])
    ...
    result: dict[str, Any] = {
        "schema_version": "strict_v4_mlp_nnguide_v1", "model": "mlp_nnguide_energy", "method": METHOD,
        ...
        "implementation": "official-default NNGuide-Energy on frozen tabular MLP",
        "selection_evidence": {
            "protocol": "strict_frozen_checkpoint_official_default_nnguide_energy",
        ...
        "diagnostics": {
            "validation_base_confidence_mean": float(validation_output["base_confidence"].mean()),
            "test_base_confidence_mean": float(test_output["base_confidence"].mean()),
            "validation_guidance_mean": float(validation_output["guidance"].mean()),
            "test_guidance_mean": float(test_output["guidance"].mean()),
            "validation_guided_confidence_mean": float(validation_output["guided_confidence"].mean()),
            "test_guided_confidence_mean": float(test_output["guided_confidence"].mean()),
        },
    ...
    np.savez_compressed(
        ...
        validation_nnguide_energy=validation_output["risk"], test_nnguide_energy=test_output["risk"],
    )
    provenance = {
        "schema_version": "strict_v4_mlp_nnguide_provenance_v1", "source_run": str(source_run),
        "source_artifact_sha256": source_sha256, "implementation_sha256": sha256_file(Path(__file__).resolve()),
        "scorer_module_sha256": sha256_file(Path(__file__).resolve().parent / "caeos" / "nnguide_posthoc.py"),
