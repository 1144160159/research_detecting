#!/usr/bin/env python3
"""Resumable archive-efficient all-PCAP strict audit for CICIoT2022."""
...
def aggregate(inventory: dict, audits: list[dict], selected_count: int) -> dict:
    ...
    accepted = sum(accepted_audit(item) for item in audits)
    ...
    formal = inventory_ready and full_scope and exact_source_coverage and len(audits) == expected and accepted == expected
    return {
        "schema": "caeos.ciciot2022.all_pcap_audit_summary.v2",
        ...
        "formal_gate_reason": None if formal else "every expanded official experiment PCAP/member needs an accepted complete-read audit",
    }


def make_audit(entry: dict, scan: dict, temporary_removed: bool) -> dict:
    retained = scan["retained_supported_ip_flow_count"]
    return {
        "schema": "caeos.ciciot2022.all_pcap_member_audit.v2",
        "dataset_id": "CICIoT2022",
        "authority_granularity": "capture_member_not_official_flow_label",
        ...
        **scan,
        ...
        "temporary_pcap_removed": temporary_removed,
    }


def load_resumable(entry: dict, audit_dir: Path) -> dict | None:
    output = audit_dir / f"{source_token(entry['source_id'])}.json"
    if not output.is_file():
        return None
    audit = json.loads(output.read_text())
    return audit if audit.get("source_id") == entry["source_id"] and accepted_audit(audit) else None


def run(dataset_root: Path, inventory_path: Path, run_root: Path, temp_root: Path, include_regex: str | None = None) -> dict:
    inventory = json.loads(inventory_path.read_text())
    if inventory.get("schema") != "caeos.ciciot2022.all_pcap_inventory.v1":
    ...
    files = [entry for entry in entries if entry["source_type"] == "file"]
    archives = defaultdict(list)
    for entry in entries:
        if entry["source_type"] == "archive_member":
            archives[entry["archive"]].append(entry)

    for entry in files:
        prior = load_resumable(entry, audit_dir)
        if prior:
            audits.append(prior)
            continue
        ...
        audit = make_audit(entry, scan, True)
        atomic_json(audit_dir / f"{source_token(entry['source_id'])}.json", audit)
    ...
    for archive_name in sorted(archives):
        pending = {entry["member"]: entry for entry in archives[archive_name] if load_resumable(entry, audit_dir) is None}
        for entry in archives[archive_name]:
            prior = load_resumable(entry, audit_dir)
            if prior:
                audits.append(prior)
        if not pending:
            continue
        with tarfile.open(dataset_root / archive_name, "r:gz") as archive:
            for member in archive:
                entry = pending.get(member.name)
                if entry is None:
                    continue
                ...
                if source is None:
                    raise ValueError(f"cannot extract regular member: {entry['source_id']}")
                temporary_path = None
                try:
                    with tempfile.NamedTemporaryFile(prefix="caeos-ciciot2022-all-", suffix=".pcap", dir=temp_root, delete=False) as target:
                        ...
                        while True:
                            ...
                            if not block:
                                break
                            target.write(block)
                    scan = inspect_pcap(temporary_path, 2_147_483_647)
                finally:
                ...
                atomic_json(summary_path, aggregate(inventory, audits, len(entries)))
        if pending:
            raise ValueError(f"archive inventory members not encountered in {archive_name}: {sorted(pending)[:5]}")
...
def main() -> int:
    ...
    summary = run(args.dataset_root, args.inventory, args.run_root, args.temp_root, args.include_regex)
