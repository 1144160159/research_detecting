...
TSHARK_TSV_FIELD_LIMIT_BYTES = 64 * 1024 * 1024
csv.field_size_limit(max(csv.field_size_limit(), TSHARK_TSV_FIELD_LIMIT_BYTES))
...
def path_label(dataset: dict[str, Any], source_member: str) -> LabelRecord:
    ...
    if policy == "fivegad_documented_capture_taxonomy":
        if "Normal-2UE" in parts:
            return LabelRecord("capture_path", "Benign", "Benign", 0)
        try:
            attack_index = parts.index("Attacks")
            fine = parts[attack_index + 1]
        except (ValueError, IndexError):
            return LabelRecord("capture_path_unresolved", "", "", -1)
        family = FIVE_GAD_FAMILIES.get(fine)
        if family is None:
            return LabelRecord("capture_path_unresolved", "", "", -1)
        return LabelRecord("capture_path", fine, family, 1)
...
def build_processing_policy(
    *,
    ...
    return {
        ...
        "sample_id_contract": "flow_identity_plus_piece_row_ordinal_v2",
                        ...
                        for offset, segment in enumerate(split["segments"]):
                            ...
                            writer.writerow(
                                flow_row(
                                    ...
                                    task["source_sha256"],
                                    source_member,
                                    f"{task.get('piece_index', 0)}:{counters['rows']}",
                                    int(task["payload_prefix_bytes"]),
                                    int(task["sanitized_l4_prefix_bytes"]),
                                    split_label_source,
                                )
...
@contextmanager
def csv_lines_with_optional_sha256(
...
def validate_csv_row(
    ...
    for column in CSV_SEQUENCE_COLUMNS:
        values = row[column].split(";") if row[column] else []
        if len(values) != packet_count:
            raise ValueError(f"{column} length mismatch at row {row_number}")
    ...
    if require_formal_labels:
        ...
        for label_column in (
            ...
            if not row[label_column] or row[label_column] == "Pending":
                raise ValueError(f"missing formal {label_column} at row {row_number}")
...
def verification_from_counters(
    counters: Counter[str], path: Path, expected_rows: int
) -> dict[str, Any]:
...
def verify_csv_row_batch(
    ...
    counters: Counter[str] = Counter()
    text = encoded_rows.decode("utf-8")
    reader = csv.reader(io.StringIO(text, newline=""))
    for offset, values in enumerate(reader):
        row_number = first_row_number + offset
        if len(values) != len(columns):
            raise ValueError(f"CSV field count mismatch at row {row_number}")
        counters.update(
            validate_csv_row(dict(zip(columns, values)), row_number, require_formal_labels)
        )
    return counters
...
def verify_csv(
    ...
    sha256_digest: Any | None = None,
) -> dict[str, Any]:
    counters: Counter[str] = Counter()
    with csv_lines_with_optional_sha256(path, sha256_digest) as handle:
        ...
        for row_number, row in enumerate(reader, start=2):
            ...
            if any(value is None for value in row.values()):
                raise ValueError(f"missing CSV fields at row {row_number}: {path}")
            counters.update(validate_csv_row(row, row_number, require_formal_labels))
    return verification_from_counters(counters, path, expected_rows)
...
def verify_csv_and_sha256(
    ...
    return verification, digest.hexdigest()
...
def verify_csv_and_sha256_parallel(
    ...
    workers: int = 2,
    batch_rows: int = 2048,
    maximum_inflight_batches: int | None = None,
) -> tuple[dict[str, Any], str]:
    if workers < 1:
        raise ValueError("CSV validation workers must be positive")
    if batch_rows < 1:
        raise ValueError("CSV validation batch rows must be positive")
    if workers == 1:
    ...
    inflight_limit = maximum_inflight_batches or workers * 2
    if inflight_limit < workers:
        raise ValueError("maximum in-flight CSV batches must cover all workers")
    ...
    def collect_oldest() -> None:
        counters.update(pending.pop(0).result())

    with path.open("rb") as handle, ProcessPoolExecutor(max_workers=workers) as executor:
        ...
        try:
            ...
            raise ValueError(f"invalid CSV header: {path}") from exc
        if header != columns:
            raise ValueError(f"CSV header differs from frozen schema: {path}")
        ...
        for raw_line in handle:
            ...
            pending.append(
                executor.submit(
                    verify_csv_row_batch,
                    bytes(encoded_batch),
                    columns,
                    batch_first_row,
                    require_formal_labels,
                )
            )
            encoded_batch.clear()
            batch_count = 0
            batch_first_row = next_row_number
    ...
    verification = verification_from_counters(counters, path, expected_rows)
    verification.update(
        {
            "row_validation_parallelism": workers,
            "row_validation_batch_rows": batch_rows,
            "single_sequential_file_read": True,
        }
    )
    return verification, digest.hexdigest()
...
def merge_parts(
    ...
    verification, materialized_sha256 = verify_csv_and_sha256(
        final_path,
        columns,
        rows,
