...
def parse_arguments() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Evaluate no-leak NECO-ID90 from a frozen strict MLP run")
...
def main() -> None:
    ...
    with np.load(source_paths["scores.npz"], allow_pickle=False) as source_scores:
        ...
        if arguments.get("dataset") != "tabular":
            raise ValueError("strict NECO matrix only accepts tabular runs")
    ...
    calibrator.fit(train["embedding"])
    ...
    result: dict[str, Any] = {
        "schema_version": "strict_v4_mlp_neco_id90_v1", "model": "mlp_neco_id90", "method": METHOD,
        ...
        "implementation": "NECO Algorithm-1 score with paper-recommended known-training 90% PCA dimension",
        "selection_evidence": {
            "protocol": "strict_frozen_checkpoint_neco_id90_no_ood_tuning",
    ...
    np.savez_compressed(
        ...
        validation_neco_id90=validation_output["risk"], test_neco_id90=test_output["risk"],
    )
    provenance = {
        "schema_version": "strict_v4_mlp_neco_id90_provenance_v1", "source_run": str(source_run),
        "source_artifact_sha256": source_sha256, "implementation_sha256": sha256_file(Path(__file__).resolve()),
        "scorer_module_sha256": sha256_file(Path(__file__).resolve().parent / "caeos" / "neco_posthoc.py"),
