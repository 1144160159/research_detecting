...
METHODS = ("line_idtune", "mlp_energy", "opendetect")
...
def analyze(pilot_root: Path, source_root: Path, opendetect_root: Path, gate: dict[str, Any]) -> dict[str, Any]:
    ...
    if gate.get("schema_version") != "strict_v4_mlp_line_idtune_expansion_gate_v1":
        raise ValueError("unexpected LINe-IDTune expansion gate schema")
    if gate.get("manifest_sha256") != canonical_hash(gate):
        raise ValueError("LINe-IDTune expansion gate SHA mismatch")
    if gate.get("pilot_protocol_manifest_sha256") != protocol.get("manifest_sha256"):
        raise ValueError("LINe-IDTune expansion gate protocol binding mismatch")
    blocks = []
    ...
    for suite, scenarios in protocol["selected_scenarios"].items():
        for scenario in scenarios:
            ...
            blocks.append({
                ...
                "reports": {
                    "line_idtune": _report(candidate, "line_idtune", candidate_path),
    ...
    for metric in UNKNOWN_METRICS:
        gains = []
        for block in blocks:
            candidate = block["reports"]["line_idtune"][metric]
    ...
    candidate_row = next(row for row in overall if row["method"] == "line_idtune")
    known_f1_max_difference = max(
        abs(block["reports"]["line_idtune"]["known_macro_f1"] - block["reports"]["mlp_energy"]["known_macro_f1"])
        for block in blocks
    )
    selected_parameters = {}
    for block in blocks:
        metrics_path = pilot_root / block["suite"] / (block["scenario"] + "_seed7") / "metrics.json"
        selected = _read(metrics_path)["selection_evidence"]["postprocessor"]["selected"]
        key = "pw=%g,pa=%g,q=%g" % (selected["weight_percentile"], selected["activation_percentile"], selected["clip_quantile"])
        selected_parameters[key] = selected_parameters.get(key, 0) + 1
    checks = {
    ...
    return {
        "schema_version": "strict_v4_mlp_line_idtune_pilot_analysis_v1",
        ...
        "line_idtune_vs_mlp_energy_oriented_mean_gains": oriented_gains,
        "line_idtune_vs_mlp_energy_four_metric_mean_gain": sum(oriented_gains.values()) / len(oriented_gains),
        "line_idtune_vs_mlp_energy_suite_mean_gains": suite_gains,
        "known_f1_max_absolute_difference": known_f1_max_difference,
        "selected_parameter_counts": dict(sorted(selected_parameters.items())),
        "expansion_checks": checks, "decision": {"expand_line_idtune_to_full102": all(checks.values())},
    }
...
def render(result: dict[str, Any]) -> str:
    lines = [
        ...
        "Expand LINe-IDTune to full 102: `%s`." % ("YES" if result["decision"]["expand_line_idtune_to_full102"] else "NO"), "",
