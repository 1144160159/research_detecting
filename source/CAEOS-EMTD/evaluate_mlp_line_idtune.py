...
METHOD = "line_idtune"
PRUNING_CANDIDATES = ((10.0, 10.0), (90.0, 90.0), (90.0, 10.0))
CLIP_QUANTILES = (0.90, 0.95, 0.99)


def parse_arguments() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Evaluate no-leak LINe-IDTune from a frozen strict MLP run")
...
def main() -> None:
    ...
    with np.load(source_paths["scores.npz"], allow_pickle=False) as source_scores:
        ...
        if arguments.get("dataset") != "tabular":
            raise ValueError("strict LINe-IDTune matrix only accepts tabular runs")
    ...
    calibrator = LINeIDTuneCalibrator(PRUNING_CANDIDATES, CLIP_QUANTILES)
    calibrator.fit(
        train["embedding"], train["labels"], validation["embedding"], validation["labels"],
        validation["logits"], weights, bias,
    )
    ...
    result: dict[str, Any] = {
        "schema_version": "strict_v4_mlp_line_idtune_v1", "model": "mlp_line_idtune", "method": METHOD,
        ...
        "implementation": "LINe official equations with known-only scale-portable IDTune adapter",
        "selection_evidence": {
            "protocol": "strict_frozen_checkpoint_line_formula_id_only_selection",
    ...
    np.savez_compressed(
        ...
        validation_line_idtune=validation_output["risk"], test_line_idtune=test_output["risk"],
    )
    provenance = {
        "schema_version": "strict_v4_mlp_line_idtune_provenance_v1", "source_run": str(source_run),
        "source_artifact_sha256": source_sha256, "implementation_sha256": sha256_file(Path(__file__).resolve()),
        "scorer_module_sha256": sha256_file(Path(__file__).resolve().parent / "caeos" / "line_idtune_posthoc.py"),
