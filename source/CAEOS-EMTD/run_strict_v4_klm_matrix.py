...
def select_pilot_scenarios(coverage: dict[str, Any]) -> dict[str, list[str]]:
    ...
    for suite in sorted(registry):
        ...
        while len(indices) < 2:
            digest = hashlib.sha256(
                ("%s:klm-posthoc:%s:%d" % (coverage_sha, suite, attempt)).encode(
                    "utf-8"
                )
            ).hexdigest()
...
def valid_output(output: Path, expected_source: dict[str, str]) -> bool:
    ...
    try:
        ...
        return False
    return (
        set(metrics.get("reports", {})) == {"klm"}
        and provenance.get("source_artifact_sha256") == expected_source
        and provenance.get("methods") == ["klm"]
    )
...
def run_one(
    ...
    if completed.returncode:
        raise RuntimeError("KLM run failed for %s: %s" % (source, completed.stderr[-2000:]))
    if not valid_output(output, expected_source):
        raise RuntimeError("KLM output validation failed for %s" % source)
...
def main() -> None:
    ...
    evaluator = base / "evaluate_mlp_klm.py"
    scorer = base / "caeos" / "kl_matching.py"
    source_hashes = {
        str(path.relative_to(source_root)): source_artifacts(path) for path in sources
    }
    protocol = {
        "schema_version": "strict_v4_mlp_klm_protocol_v1",
        "status": "frozen_before_klm_results",
        "mode": args.mode,
        "coverage_manifest_sha256": coverage["manifest_sha256"],
        "selection_rule": (
            "all frozen scenarios"
            if args.mode == "full"
            else "two KLM-SHA-indexed scenarios per suite, independent of metric values"
        ),
        "selected_scenarios": {
            suite: [
                scenario
        ...
        "fit_data": "known_validation_softmax_only",
    ...
    if protocol_path.is_file():
        ...
        if existing != protocol:
            raise ValueError("existing KLM protocol differs from frozen inputs")
    else:
    ...
    if completed != len(sources):
        raise RuntimeError("KLM coverage incomplete: %d/%d" % (completed, len(sources)))
    summary = {
        "schema_version": "strict_v4_mlp_klm_matrix_v1",
        "status": "complete",
        "mode": args.mode,
        "expected_runs": len(sources),
    ...
    (output_root / "klm_complete").write_text(
