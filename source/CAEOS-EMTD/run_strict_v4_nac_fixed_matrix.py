...
def select_pilot_scenarios(coverage: dict[str, Any]) -> dict[str, list[str]]:
    ...
    for suite in sorted(registry):
        ...
        attempt = 0
        while len(indices) < 2:
            digest = hashlib.sha256(("%s:nac_ue_fixed:%s:%d" % (coverage_sha, suite, attempt)).encode("utf-8")).hexdigest()
...
def valid_output(output: Path, expected_source: dict[str, str]) -> bool:
    ...
    try:
        ...
        return False
    return set(metrics.get("reports", {})) == {METHOD} and provenance.get("source_artifact_sha256") == expected_source and provenance.get("methods") == [METHOD]
...
def run_one(evaluator: Path, source: Path, output: Path, device: str, expected_source: dict[str, str]) -> dict[str, str]:
    ...
    if completed.returncode:
        raise RuntimeError("NAC-UE-Fixed run failed for %s: %s" % (source, completed.stderr[-3000:]))
    if not valid_output(output, expected_source):
        raise RuntimeError("NAC-UE-Fixed output validation failed for %s" % source)
...
def main() -> None:
    ...
    evaluator = base / "evaluate_mlp_nac_fixed.py"
    scorer = base / "caeos" / "nac_posthoc.py"
    source_hashes = {str(path.relative_to(source_root)): source_artifacts(path) for path in sources}
    protocol = {
        "schema_version": "strict_v4_mlp_nac_ue_fixed_protocol_v1",
        "status": "frozen_before_nac_results", "mode": args.mode,
        "coverage_manifest_sha256": coverage["manifest_sha256"],
        "selection_rule": "all frozen scenarios" if args.mode == "full" else "two NAC-UE-Fixed-SHA-indexed scenarios per suite independent of metric values",
        "selected_scenarios": {
        ...
        "paper": "https://proceedings.iclr.cc/paper_files/paper/2024/hash/2b1a955952bc98518a331ad6d8cc524d-Abstract-Conference.html",
        "official_implementation": "https://github.com/BierOne/ood_coverage",
        "official_implementation_commit": "16933b0b17fe451cdcd60f77d95d8746e57da4cc",
        "formula": "official NAC-UE neuron output times uniform-KL gradient coverage score",
        "parameter_policy": "official CIFAR-10 ResNet avgpool defaults valid_num=1000 M=50 O=50 alpha=100 fixed before traffic results",
        "official_main_result_audit": "official evaluator selects hyperparameters by maximum AUROC on an OOD validation loader; that sweep is excluded",
        "fit_data": "class_balanced_known_training_features_only", "threshold_data": "known_validation_only",
        ...
        "layer_policy": "final MLP embedding as pre-classifier avgpool analogue",
        "gradient_policy": "analytic uniform-target KL gradient verified against autograd",
        "nonredundancy": "neuron state coverage differs from logit energy, feature distance, PCA projection and CAEOS conflict evidence",
    ...
    if protocol_path.is_file():
        if json.loads(protocol_path.read_text(encoding="utf-8")) != protocol:
            raise ValueError("existing NAC-UE-Fixed protocol differs from frozen inputs")
    else:
    ...
    if completed != len(sources):
        raise RuntimeError("NAC-UE-Fixed coverage incomplete: %d/%d" % (completed, len(sources)))
    summary = {
        "schema_version": "strict_v4_mlp_nac_ue_fixed_matrix_v1", "status": "complete", "mode": args.mode,
    ...
    (output_root / "nac_ue_fixed_complete").write_text(protocol["manifest_sha256"] + "\n", encoding="ascii")
