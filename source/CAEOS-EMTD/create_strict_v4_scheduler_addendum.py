...
def main() -> None:
    ...
    if not 72 <= len(metrics) < 103 or failures:
        raise ValueError(
            f"scheduler change expected 72-102 completed and zero failed: "
            f"metrics={len(metrics)} failures={len(failures)}"
        )
    payload: dict[str, Any] = {
        "schema_version": "strict_v4_full103_scheduler_addendum_v1",
        ...
        "change": {
            "suite": "cic_iot2023",
            "outer_scenario_workers_before": 2,
            ...
            "reason": "80 CPU cores and 476 GiB available; reduce wall-clock only",
        },
