...
RUN_ROOT="$PROJECT_ROOT/runs/strict_v4_external_training_pilot_seed7"
RESULT_ROOT="$PROJECT_ROOT/results/strict_v4_external_training_pilot_seed7"
MODELS="closr,cade,sieve"

cd "$PROJECT_ROOT"
mkdir -p "$RESULT_ROOT"

"$PYTHON" create_strict_v4_external_training_pilot_protocol.py \
...
run_suite() {
  ...
  nice -n 10 "$PYTHON" run_neural_baseline_matrix.py \
    ...
    --seeds 7 --workers 3 --epochs 0 --patience 10 \
...
[[ "$count" -eq 42 && "$failures" -eq 0 ]]

"$PYTHON" summarize_strict_v4_external_training_pilot.py \
  --pilot-root "$RUN_ROOT" \
