...
SEEDS_SPACE="73 79"
MAX_PER_CLASS=1000
RUN_ROOT="$PROJECT_ROOT/runs/strict_v4_pseudo_unknown_confirmation"
RESULT_ROOT="$PROJECT_ROOT/results/strict_v4_pseudo_unknown_confirmation"
MANIFEST="$PROJECT_ROOT/results/strict_v4_pseudo_unknown_development/candidate_manifest.json"
TON_CACHE="$PROJECT_ROOT/caches/strict_v4_pseudo_unknown_confirmation/cic_ton_iot/stratified"
CICIOT_RAW="$PROJECT_ROOT/caches/strict_v4_pseudo_unknown_confirmation/cic_iot2023/raw"
CICIOT_CACHE="$PROJECT_ROOT/caches/strict_v4_pseudo_unknown_confirmation/cic_iot2023/group_supported"
POLICY_NAME="strict_v4_pseudo_unknown_confirmation_v1"

cd "$PROJECT_ROOT"
mkdir -p "$RESULT_ROOT" "$TON_CACHE" "$CICIOT_RAW" "$CICIOT_CACHE"
MAX_ALPHA="$($PYTHON -c 'import json,sys; print(json.load(open(sys.argv[1]))["candidate"]["maximum_alpha"])' "$MANIFEST")"
...
"$PYTHON" run_nested_gate_matrix.py \
  --suite cic_ton_iot \
  --scenarios backdoor,ddos,dos \
  --seeds "$SEEDS_COMMA" \
  --workers 2 \
  --model-jobs 8 \
  --estimators 80 \
  --risk-selection nested_pseudo_unknown_blend \
...
"$PYTHON" run_nested_gate_matrix.py \
  --suite cic_iot2023 \
  --scenarios command_injection,mirai_greip_flood,vulnerability_scan \
  --seeds "$SEEDS_COMMA" \
  --workers 2 \
  --model-jobs 8 \
  --estimators 80 \
...
"$PYTHON" confirm_strict_v4_pseudo_unknown.py \
  --root "$RUN_ROOT" \
  --manifest "$MANIFEST" \
...
"$PYTHON" -c \
  'import json,sys; p=json.load(open(sys.argv[1])); assert p["validation"]["run_count"] == 12; assert p["validation"]["scenario_count"] == 6; assert p["validation"]["seeds"] == [73,79]; assert isinstance(p["decision"]["passes"], bool)' \
