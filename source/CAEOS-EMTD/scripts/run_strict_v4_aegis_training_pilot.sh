...
RUN_ROOT="$PROJECT_ROOT/runs/strict_v4_aegis_training_pilot_seed7"
RESULT_ROOT="$PROJECT_ROOT/results/strict_v4_aegis_training_pilot_seed7"

cd "$PROJECT_ROOT"
mkdir -p "$RESULT_ROOT"

"$PYTHON" create_strict_v4_aegis_training_pilot_protocol.py \
...
run_suite() {
  ...
  local scenarios="$2"
  shift 2
  nice -n 10 "$PYTHON" run_aegis_baseline_matrix.py \
    --suite "$suite" --scenarios "$scenarios" --models aegis_clean_adapter \
...
[[ "$count" -eq 14 && "$failures" -eq 0 ]]

"$PYTHON" summarize_strict_v4_aegis_training_pilot.py \
  --pilot-root "$RUN_ROOT" \
