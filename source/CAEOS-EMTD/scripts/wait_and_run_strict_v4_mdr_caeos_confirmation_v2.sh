...
PILOT_ROOT="$PROJECT_ROOT/results/strict_v4_mdr_caeos_pilot_v2"
DESIGN="$PROJECT_ROOT/results/strict_v4_mdr_caeos_design/design_v2.json"
PILOT_PROTOCOL="$PROJECT_ROOT/results/strict_v4_mdr_caeos_pilot_protocol_v2/protocol_manifest.json"
...
LOCK_DIR="$RESULT_ROOT/watcher_v2.lock.d"
STATE_LOG="$RESULT_ROOT/watcher_state_v2.log"
IDLE_LOG="$RESULT_ROOT/idle_observations_v2.log"

mkdir -p "$RESULT_ROOT"
if ! mkdir "$LOCK_DIR" 2>/dev/null; then
  echo "MDR-CAEOS confirmation v2 watcher already active" >&2
  exit 0
fi
...
cd "$PROJECT_ROOT"
: > "$STATE_LOG"
log_state "waiting for MDR-CAEOS pilot v2 completion"
...
valid = (
    ...
    and audit.get("schema_version") == "strict_v4_mdr_caeos_pilot_audit_v2"
...
if not valid:
    raise SystemExit("invalid MDR pilot v2 completion artifacts")
print(
    "true"
...
if [[ "$pilot_positive" != "true" ]]; then
  log_state "pilot v2 gate negative; writing canonical not-required branch"
  "$PYTHON" - \
...
not_required = {
    "schema_version": "strict_v4_mdr_caeos_confirmation_not_required_v2",
    "state": "complete",
    "reason": "frozen_mdr_pilot_v2_gate_failed",
...
selection = {
    ...
    "state": "complete_after_negative_mdr_pilot_v2",
    ...
    "selection_rule": (
        "negative MDR pilot v2 makes reserved confirmation not required "
        "and retains Pairwise"
    ),
...
PY
  ...
  log_state "negative MDR confirmation v2 branch complete"
  exit 0
fi
...
log_state "freezing MDR reserved-confirmation protocol from pilot v2"
"$PYTHON" create_strict_v4_mdr_caeos_confirmation_protocol_v2.py \
  --design "$DESIGN" \
  ...
  --implementation protocol_creator=create_strict_v4_mdr_caeos_confirmation_protocol_v2.py \
  --implementation protocol_creator_base=create_strict_v4_mdr_caeos_confirmation_protocol.py \
  --implementation pilot_protocol_helper=create_strict_v4_mdr_caeos_pilot_execution_protocol_v2.py \
  --implementation pilot_auditor=audit_mdr_caeos_pilot_v2.py \
  --implementation capture=capture_mdr_caeos_runtime_v2.py \
  ...
  --implementation watcher=scripts/wait_and_run_strict_v4_mdr_caeos_confirmation_v2.sh \
...
PY
log_state "MDR reserved-confirmation v2-input branch complete"
