...
RESULT_ROOT="$PROJECT_ROOT/results/strict_v4_mdr_caeos_pilot_v2"
...
if ! mkdir "$LOCK_DIR" 2>/dev/null; then
  echo "MDR-CAEOS pilot v2 watcher already active" >&2
  exit 0
fi
...
if value.get("schema_version") != "strict_v4_mdr_caeos_pilot_execution_protocol_v2":
    raise SystemExit("wrong MDR pilot v2 protocol schema")
if value.get("manifest_sha256") != canonical_hash(value):
    raise SystemExit("MDR pilot v2 protocol canonical hash mismatch")
if value.get("execution_admitted") is not True:
    raise SystemExit("MDR pilot v2 execution not admitted")
PY
...
log_state "running resumable MDR-CAEOS pilot v2"
ionice -c 3 nice -n 19 "$PYTHON" run_strict_v4_mdr_caeos_pilot_v2.py \
  --protocol "$PROTOCOL" \
  --run-root "$RUN_ROOT" \
...
log_state "summarizing MDR-CAEOS pilot v2"
...
log_state "independently auditing MDR-CAEOS pilot v2"
"$PYTHON" audit_mdr_caeos_pilot_v2.py \
  --protocol "$PROTOCOL" \
  --design "$DESIGN" \
  --selection "$RESULT_ROOT/weight_selection.json" \
...
if summary.get("manifest_sha256") != canonical_hash(summary):
    raise SystemExit("MDR pilot v2 summary canonical hash mismatch")
if audit.get("manifest_sha256") != canonical_hash(audit):
    raise SystemExit("MDR pilot v2 audit canonical hash mismatch")
if audit.get("passes") is not True:
    raise SystemExit("MDR pilot v2 independent audit failed")
marker = (
    "full_confirmation_design_required"
    if summary["decision"]["expand_to_full102_confirmation"]
    else "full_confirmation_not_admitted"
)
...
PY
log_state "MDR-CAEOS pilot v2 complete; full confirmation remains conditional"
