...
DATA_ROOT="${DATA_ROOT:-/opt/data/private/wangwt/ParkAttackKE/datasets/caeos_external_open_set_v2}"
AUDIT_ROOT="$PROJECT_ROOT/results/gpu_dataset_reconciled_admission_audit_v2"
RESULT_ROOT="$PROJECT_ROOT/results/gpu_external_dataset_preparation_v2"
PROTOCOL="$PROJECT_ROOT/results/gpu_external_dataset_preparation_protocol_v2/protocol.json"
...
if ! mkdir "$LOCK_DIR" 2>/dev/null; then
  echo "GPU external dataset preparation v2 is already active" >&2
  exit 0
fi
...
"$PYTHON" - "$PROTOCOL" <<'PY'
import json, sys
from create_gpu_external_preparation_protocol_v2 import verify_protocol
verify_protocol(json.load(open(sys.argv[1], encoding="utf-8")))
PY

if [[ ! -f "$DATA_ROOT/LSNM2024/preparation_complete" ]]; then
  ionice -c3 nice -n 15 "$PYTHON" prepare_gpu_external_datasets_v2.py \
    --dataset LSNM2024 \
...
"$PYTHON" - "$DATA_ROOT" "$RESULT_ROOT" "$PROTOCOL" <<'PY'
import hashlib, json, sys
from pathlib import Path

data_root, result_root, protocol_path = map(Path, sys.argv[1:])
...
for name in ("LSNM2024", "CICDDoS2019"):
    ...
    expected = 16 if name == "LSNM2024" else 18
    for entry in manifest["files"].values():
        assert len(entry["label_counts"]) == expected
    datasets[name] = {
        "manifest": str(path),
        ...
        "seed_label_counts": {
            seed: len(entry["label_counts"])
            for seed, entry in manifest["files"].items()
        },
    }
combined = {
    "schema_version": "gpu_external_dataset_preparation_summary_v2",
    "status": "complete",
