...
exec 9>"/tmp/caeos_unified_feature_extraction_v5_lane2.lock"
if ! flock -n 9; then
    echo '{"event":"launch_rejected","reason":"feature_extraction_lane2_already_running"}' >&2
    exit 73
fi

active_dataset_processes="$(
    ps -eo pid=,ppid=,comm=,args= |
        awk '$3 ~ /^python/ && $0 ~ /prepare_caeos_splitpcap_class_csv[.]py/ {
...
if (( active_dataset_processes >= 4 )); then
    printf '{"event":"launch_rejected","reason":"dataset_concurrency_limit","active":%s,"limit":4}\n' \
        "${active_dataset_processes}" >&2
    exit 74
fi

printf '%s\n' "$$" > "${control_root}/lane2.pid"
printf '%s\n' "$(date --iso-8601=seconds)" > "${control_root}/lane2.started_at"
cd "${code_root}"
...
exec "${python_bin}" -u prepare_caeos_splitpcap_class_csv.py \
    ...
    --completion-path "${control_root}/completion.${dataset_id}.json" \
    --dataset "${dataset_id}" \
