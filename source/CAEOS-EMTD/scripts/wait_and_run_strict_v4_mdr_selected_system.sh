...
ROOT="$PROJECT_ROOT/results/strict_v4_mdr_selected_system_v1"
RUN_ROOT="$PROJECT_ROOT/runs/strict_v4_mdr_selected_system_v1"
...
if ! mkdir "$LOCK" 2>/dev/null; then
  echo "MDR selected-system watcher already active" >&2
  exit 0
fi
...
value = {
    "schema_version": "strict_v4_mdr_selected_system_not_required_v1",
    "status": "not_required",
    ...
    "deployability_gate_passes": False,
...
log_state "MDR selected; waiting for confirmation and malicious-external branch"
until [[ -s "$MDR/protocol_manifest.json" \
  && -s "$MDR/summary.json" \
  && -s "$MDR/audit.json" \
  && -f "$EXTERNAL/branch_complete" ]]; do
  sleep 300
done
...
if [[ ! -s "$PROTOCOL" ]]; then
  "$PYTHON" create_strict_v4_mdr_selected_system_protocol.py \
...
log_state "running serial same-process MDR versus embedded Pairwise benchmarks"
...
log_state "summarizing three-seed scenario-block system evidence"
"$PYTHON" summarize_strict_v4_mdr_selected_system.py \
  --protocol "$PROTOCOL" \
  --run-root "$RUN_ROOT" \
  --output "$ROOT/summary.json" \
  > "$ROOT/summary.log" 2>&1

log_state "independently auditing system evidence and claim tiers"
"$PYTHON" audit_strict_v4_mdr_selected_system.py \
  --protocol "$PROTOCOL" \
...
PY
touch "$ROOT/branch_complete"
log_state "MDR selected-system branch complete"
