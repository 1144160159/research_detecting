...
SEEDS_SPACE="59 61"
MAX_PER_CLASS=1000
RUN_ROOT="$PROJECT_ROOT/runs/strict_v4_validation_router_v2_confirmation"
RESULT_ROOT="$PROJECT_ROOT/results/strict_v4_validation_router_v2_confirmation"
MANIFEST="$PROJECT_ROOT/results/strict_v4_validation_router_v2_development/candidate_manifest.json"
TON_CACHE="$PROJECT_ROOT/caches/strict_v4_validation_router_v2_confirmation/cic_ton_iot/stratified"
CICIOT_RAW="$PROJECT_ROOT/caches/strict_v4_validation_router_v2_confirmation/cic_iot2023/raw"
CICIOT_CACHE="$PROJECT_ROOT/caches/strict_v4_validation_router_v2_confirmation/cic_iot2023/group_supported"
...
verify_sha "e3aa42db4d0feaf555c7afd27fff2f2ba9692f735e754645c42d3c7fee511b7c" confirm_strict_v4_validation_router_v2.py
verify_sha "e7c19173ac62f8f9dd32e9d59c4d3888e6f6825e5b487641da7bd4e6bb737bc3" "$MANIFEST"
...
for seed in $SEEDS_SPACE; do
  ...
  if [[ ! -s "$ton_output" ]]; then
    "$PYTHON" prepare_stratified_cache.py --csv "$TON_SOURCE" \
      ...
      --chunksize 50000 --seed "$seed" --output "$ton_output"
  fi
  ciciot_raw="$CICIOT_RAW/seed${seed}_max${MAX_PER_CLASS}.csv"
  if [[ ! -s "$ciciot_raw" ]]; then
    "$PYTHON" prepare_cic_iot2023_strict.py --input-dir "$CICIOT_SOURCE" \
      --output "$ciciot_raw" --seed "$seed" --max-per-class "$MAX_PER_CLASS" \
      --group-rows 1000 --expected-source-files 309
  fi
  ciciot_output="$CICIOT_CACHE/seed${seed}_max${MAX_PER_CLASS}.csv"
  if [[ ! -s "$ciciot_output" ]]; then
    "$PYTHON" prepare_group_supported_cache.py --input "$ciciot_raw" \
      --output "$ciciot_output" --label-column Attack --group-column CaptureGroup \
      --minimum-groups 3
  fi
...
"$PYTHON" run_nested_gate_matrix.py --suite cic_iot2023 \
  --scenarios ddos_icmp_fragmentation,ddos_pshack_flood,ddos_rstfin_flood,dos_http_flood,recon_host_discovery,sql_injection \
  --seeds "$SEEDS_COMMA" --workers 1 --model-jobs 8 --estimators 80 \
  --risk-selection fixed_cauchy_modality_support_union --risk-policy-name "$POLICY_NAME" \
  --cic-iot2023-max-per-class "$MAX_PER_CLASS" --cic-iot2023-cache-dir "$CICIOT_CACHE" \
  --output-root "$RUN_ROOT" >> "$RESULT_ROOT/training.log" 2>&1

"$PYTHON" confirm_strict_v4_validation_router_v2.py --root "$RUN_ROOT" \
...
"$PYTHON" -c \
  'import json,sys; p=json.load(open(sys.argv[1])); assert p["validation"]["run_count"] == 30; assert p["validation"]["scenario_count"] == 15; assert p["validation"]["seeds"] == [59,61]; assert isinstance(p["decision"]["passes"], bool)' \
