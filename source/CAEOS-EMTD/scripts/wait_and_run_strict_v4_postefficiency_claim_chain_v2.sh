...
COMPARATIVE_PROTOCOL_V1="$COMPARATIVE_PROTOCOL_ROOT/protocol_manifest.json"
COMPARATIVE_PROTOCOL="$COMPARATIVE_PROTOCOL_ROOT/protocol_manifest_v2.json"
...
if [[ ! -s "$COMPARATIVE_PROTOCOL_V1" ]]; then
  observed=0
...
validate_manifest \
  "$COMPARATIVE_PROTOCOL_V1" \
  strict_v4_comparative_corruption_protocol_v1

if [[ ! -s "$COMPARATIVE_PROTOCOL" ]]; then
  observed=0
  if [[ -d "$COMPARATIVE_RUN_ROOT" ]]; then
    observed="$(find "$COMPARATIVE_RUN_ROOT" \
      -name paired_corruption.json -type f | wc -l)"
  fi
  if [[ "$observed" -ne 0 ]]; then
    log_state "refusing v2 schema revision after paired results exist"
    exit 1
  fi
  log_state "freezing comparative corruption v2 schema-only revision"
  "$PYTHON" create_strict_v4_comparative_corruption_protocol_v2.py \
    --v1-protocol "$COMPARATIVE_PROTOCOL_V1" \
    --evaluator evaluate_strict_v4_comparative_corruption.py \
    --runner run_strict_v4_comparative_corruption.py \
    --summarizer summarize_strict_v4_comparative_corruption.py \
    --failed-evaluation-log \
      "$COMPARATIVE_PROTOCOL_ROOT/evaluation_v1_failure.log" \
    --run-root "$COMPARATIVE_RUN_ROOT" \
    --output "$COMPARATIVE_PROTOCOL" \
    > "$COMPARATIVE_PROTOCOL_ROOT/freeze_v2.log" 2>&1
fi
validate_manifest \
