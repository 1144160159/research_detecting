...
FULL_ROOT="$PROJECT_ROOT/results/strict_v4_fisher_rao_family_full102_seed7"
...
cd "$PROJECT_ROOT"
mkdir -p "$RESULT_ROOT"
"$PYTHON" run_strict_v4_fisher_rao_matrix.py \
...
if [[ ! -s "$GATE" ]]; then
  "$PYTHON" create_strict_v4_fisher_rao_expansion_gate.py \
...
"$PYTHON" summarize_strict_v4_fisher_rao.py \
  --root "$RESULT_ROOT" --source-root "$SOURCE_ROOT" \
  --opendetect-root "$OPENDETECT_ROOT" --mode pilot --gate "$GATE" \
...
if [[ "$expand" == "true" ]]; then
  ...
  "$PYTHON" summarize_strict_v4_fisher_rao.py \
    --root "$FULL_ROOT" --source-root "$SOURCE_ROOT" \
    --opendetect-root "$OPENDETECT_ROOT" --mode full \
