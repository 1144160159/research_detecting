...
SEEDS_SPACE="97 101"
MAX_PER_CLASS=1000
RUN_ROOT="$PROJECT_ROOT/runs/strict_v4_local_rank_confirmation"
RESULT_ROOT="$PROJECT_ROOT/results/strict_v4_local_rank_confirmation"
MANIFEST="$PROJECT_ROOT/results/strict_v4_local_rank_development/candidate_manifest.json"
TON_CACHE="$PROJECT_ROOT/caches/strict_v4_local_rank_confirmation/cic_ton_iot/stratified"
CICIOT_RAW="$PROJECT_ROOT/caches/strict_v4_local_rank_confirmation/cic_iot2023/raw"
CICIOT_CACHE="$PROJECT_ROOT/caches/strict_v4_local_rank_confirmation/cic_iot2023/group_supported"
POLICY_NAME="strict_v4_local_rank_confirmation_v1"

cd "$PROJECT_ROOT"
mkdir -p "$RESULT_ROOT" "$TON_CACHE" "$CICIOT_RAW" "$CICIOT_CACHE"
read -r MAX_ALPHA MIN_FOLD_GAIN BINS BETA < <(
  "$PYTHON" -c \
    'import json,sys; c=json.load(open(sys.argv[1]))["candidate"]; print(c["maximum_alpha"], c["minimum_fold_gain"], c["local_rank_bins"], c["local_rank_beta"])' \
    "$MANIFEST"
)
...
COMMON=(
  ...
  --model-jobs 8
  --estimators 80
  --risk-selection nested_local_rank_pseudo_unknown_blend
  --pseudo-unknown-max-alpha "$MAX_ALPHA"
  --pseudo-unknown-min-fold-gain "$MIN_FOLD_GAIN"
  --pseudo-unknown-local-rank-bins "$BINS"
  --pseudo-unknown-local-rank-beta "$BETA"
...
"$PYTHON" confirm_strict_v4_local_rank.py \
  --root "$RUN_ROOT" \
  --manifest "$MANIFEST" \
...
"$PYTHON" -c \
  'import json,sys; p=json.load(open(sys.argv[1])); assert p["validation"]["run_count"] == 12; assert p["validation"]["seeds"] == [97,101]; assert isinstance(p["decision"]["passes"], bool)' \
