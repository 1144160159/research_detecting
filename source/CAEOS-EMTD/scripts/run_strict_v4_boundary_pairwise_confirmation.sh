...
SEEDS_SPACE="127 131"
MAX_PER_CLASS=1000
RUN_ROOT="$PROJECT_ROOT/runs/strict_v4_boundary_pairwise_confirmation"
RESULT_ROOT="$PROJECT_ROOT/results/strict_v4_boundary_pairwise_confirmation"
MANIFEST="$PROJECT_ROOT/results/strict_v4_boundary_pairwise_development/candidate_manifest.json"
TON_CACHE="$PROJECT_ROOT/caches/strict_v4_boundary_pairwise_confirmation/cic_ton_iot/stratified"
CICIOT_RAW="$PROJECT_ROOT/caches/strict_v4_boundary_pairwise_confirmation/cic_iot2023/raw"
...
"$PYTHON" confirm_strict_v4_boundary_pairwise.py \
  --root "$RUN_ROOT" \
  --manifest "$MANIFEST" \
...
"$PYTHON" -c \
  'import json,sys; p=json.load(open(sys.argv[1])); assert p["validation"]["run_count"] == 12; assert p["validation"]["seeds"] == [127,131]; assert isinstance(p["decision"]["passes"], bool)' \
