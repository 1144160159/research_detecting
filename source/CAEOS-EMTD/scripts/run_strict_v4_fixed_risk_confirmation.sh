...
SEEDS_SPACE="11 19"
MAX_PER_CLASS=1000
RUN_ROOT="$PROJECT_ROOT/runs/strict_v4_fixed_risk_confirmation"
RESULT_ROOT="$PROJECT_ROOT/results/strict_v4_fixed_risk_confirmation"
MANIFEST="$PROJECT_ROOT/results/strict_v4_risk_screening/candidate_manifest.json"
TON_CACHE="$PROJECT_ROOT/caches/strict_v4_confirmation/cic_ton_iot/stratified"
CICIOT_RAW="$PROJECT_ROOT/caches/strict_v4_confirmation/cic_iot2023/raw"
CICIOT_CACHE="$PROJECT_ROOT/caches/strict_v4_confirmation/cic_iot2023/group_supported"
...
verify_sha "abfa033322fd59f3ff796fbe051e27b73a6e77a5a1f564f3beb11e7f19571007" "$MANIFEST"
...
"$PYTHON" run_nested_gate_matrix.py \
  --suite cic_ton_iot \
  --scenarios password,backdoor,ddos \
  --seeds "$SEEDS_COMMA" \
  --workers 1 \
  --model-jobs 8 \
  --estimators 80 \
...
"$PYTHON" run_nested_gate_matrix.py \
  --suite cic_iot2023 \
  --scenarios browser_hijacking,dns_spoofing,recon_port_scan \
  --seeds "$SEEDS_COMMA" \
  --workers 1 \
  --model-jobs 8 \
  --estimators 80 \
...
"$PYTHON" -c \
  'import json,sys; p=json.load(open(sys.argv[1])); assert p["validation"]["run_count"] == 12; assert p["validation"]["scenario_count"] == 6; assert p["validation"]["seeds"] == [11,19]; assert isinstance(p["decision"]["passes"], bool)' \
