...
RUN_ROOT="$PROJECT_ROOT/runs/mal_tls_geometry_preserving_adapter_seed195"
...
mkdir -p "$RESULT_ROOT"
if [[ ! -s "$PROTOCOL" ]]; then
  "$PYTHON" create_mal_tls_geometry_preserving_adapter_protocol.py \
...
for scenario in caphaw cobalt panda qakbot scanners tor; do
  mapfile -t unknowns < <("$PYTHON" -c \
    ...
    "$PROTOCOL" "$scenario")
  reference="$RUN_ROOT/uniform_mlp/${scenario}_seed195"
  if [[ ! -s "$reference/metrics.json" ]]; then
    "$PYTHON" train.py \
      ...
      --seed 195 --device cuda --output-dir "$reference" \
      >> "$RESULT_ROOT/training.log" 2>&1
  fi
  candidate="$RUN_ROOT/mal_tls_geometry_preserving_adapter/${scenario}_seed195"
  if [[ ! -s "$candidate/metrics.json" ]]; then
    "$PYTHON" train.py \
      ...
      --seed 195 --device cuda --output-dir "$candidate" \
      >> "$RESULT_ROOT/training.log" 2>&1
  fi
...
"$PYTHON" analyze_mal_tls_geometry_preserving_adapter.py \
