...
SEEDS_SPACE="107 109"
MAX_PER_CLASS=1000
RUN_ROOT="$PROJECT_ROOT/runs/strict_v4_boundary_pseudo_unknown_confirmation"
RESULT_ROOT="$PROJECT_ROOT/results/strict_v4_boundary_pseudo_unknown_confirmation"
MANIFEST="$PROJECT_ROOT/results/strict_v4_boundary_pseudo_unknown_development/candidate_manifest.json"
TON_CACHE="$PROJECT_ROOT/caches/strict_v4_boundary_pseudo_unknown_confirmation/cic_ton_iot/stratified"
CICIOT_RAW="$PROJECT_ROOT/caches/strict_v4_boundary_pseudo_unknown_confirmation/cic_iot2023/raw"
CICIOT_CACHE="$PROJECT_ROOT/caches/strict_v4_boundary_pseudo_unknown_confirmation/cic_iot2023/group_supported"
POLICY_NAME="strict_v4_boundary_pseudo_unknown_confirmation_v1"

cd "$PROJECT_ROOT"
mkdir -p "$RESULT_ROOT" "$TON_CACHE" "$CICIOT_RAW" "$CICIOT_CACHE"
read -r MAX_ALPHA MIN_FOLD HARD_FRACTION INTERPOLATION MAX_TASK < <(
  "$PYTHON" -c \
    'import json,sys; c=json.load(open(sys.argv[1]))["candidate"]; print(c["maximum_alpha"], c["minimum_fold_gain"], c["hard_pseudo_fraction"], c["interpolation"], c["max_per_task"])' \
    "$MANIFEST"
)
...
COMMON=(
  ...
  --model-jobs 8
  --estimators 80
  --risk-selection nested_boundary_pseudo_unknown_blend
...
"$PYTHON" run_nested_gate_matrix.py \
  --suite cic_iot2023 \
  --scenarios backdoor_malware,ddos_udp_flood,recon_os_scan \
  "${COMMON[@]}" \
  --cic-iot2023-max-per-class "$MAX_PER_CLASS" \
  --cic-iot2023-cache-dir "$CICIOT_CACHE" \
  >> "$RESULT_ROOT/training.log" 2>&1

"$PYTHON" confirm_strict_v4_boundary_pseudo_unknown.py \
  --root "$RUN_ROOT" \
  --manifest "$MANIFEST" \
...
"$PYTHON" -c \
  'import json,sys; p=json.load(open(sys.argv[1])); assert p["validation"]["run_count"] == 12; assert p["validation"]["seeds"] == [107,109]; assert isinstance(p["decision"]["passes"], bool)' \
