...
FULL_ROOT="$PROJECT_ROOT/results/strict_v4_actsub_scale_fixed_full102_seed7"
...
cd "$PROJECT_ROOT"
mkdir -p "$RESULT_ROOT"
"$PYTHON" run_strict_v4_actsub_matrix.py \
...
if [[ ! -s "$GATE" ]]; then
  "$PYTHON" create_strict_v4_actsub_expansion_gate.py \
...
"$PYTHON" summarize_strict_v4_actsub_pilot.py \
...
expand="$("$PYTHON" -c 'import json,sys; print(str(json.load(open(sys.argv[1]))["decision"]["expand_actsub_to_full102"]).lower())' "$RESULT_ROOT/analysis.json")"
if [[ "$expand" == "true" ]]; then
  mkdir -p "$FULL_ROOT"
  "$PYTHON" run_strict_v4_actsub_matrix.py \
    --source-root "$SOURCE_ROOT" --coverage "$COVERAGE" \
