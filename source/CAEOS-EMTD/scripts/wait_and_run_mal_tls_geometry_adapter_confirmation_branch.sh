...
PILOT_ROOT="$PROJECT_ROOT/results/mal_tls_geometry_preserving_adapter_seed195"
...
BRANCH_ROOT="$PROJECT_ROOT/results/mal_tls_geometry_adapter_confirmation_branch"
LOCK_DIR="$BRANCH_ROOT/watcher.lock.d"

mkdir -p "$BRANCH_ROOT"
if ! mkdir "$LOCK_DIR" 2>/dev/null; then
  echo "geometry confirmation branch watcher already active" >&2
  exit 0
fi
...
if [[ "$decision" == "freeze_for_reserved_seed_confirmation" ]]; then
  ...
  if [[ ! -s "$RESULT_ROOT/protocol_manifest.json" ]]; then
    "$PYTHON" create_mal_tls_geometry_adapter_confirmation_protocol.py \
  ...
  bash scripts/run_mal_tls_geometry_adapter_confirmation.sh \
