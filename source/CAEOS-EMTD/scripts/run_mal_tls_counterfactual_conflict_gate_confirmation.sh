...
for seed in 203 205 207; do
  for scenario in caphaw cobalt panda qakbot scanners tor; do
    ...
    candidate="$RUN_ROOT/mal_tls_counterfactual_conflict_gate/${scenario}_seed${seed}"
    if [[ ! -s "$candidate/metrics.json" ]]; then
      "$PYTHON" train.py \
        ...
        --evidence-temperature-calibration --seed "$seed" --device cuda \
        --output-dir "$candidate" >> "$RESULT_ROOT/training.log" 2>&1
    fi
...
"$PYTHON" analyze_mal_tls_counterfactual_conflict_gate_confirmation.py \
