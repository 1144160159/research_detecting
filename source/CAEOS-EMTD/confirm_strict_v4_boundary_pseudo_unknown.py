...
EXPECTED_POLICY = "strict_v4_boundary_pseudo_unknown_confirmation_v1"


def load_manifest(path: Path, project_root: Path) -> dict[str, Any]:
    payload = json.loads(path.read_text(encoding="utf-8"))
    if payload.get("schema_version") != "strict_v4_boundary_pseudo_unknown_candidate_v1":
        raise ValueError("unexpected boundary pseudo-unknown manifest schema")
    if payload.get("status") != "frozen_unconfirmed":
        raise ValueError("boundary pseudo-unknown candidate is not frozen")
    if payload.get("manifest_sha256") != canonical_hash(payload):
        raise ValueError("boundary pseudo-unknown manifest SHA mismatch")
    ...
    if confirmation.get("scenario_disjoint_from_boundary_development") is not True:
...
def validate_runtime(
    ...
    if learned.get("pseudo_unknown_source") != (
        ...
        raise ValueError(f"boundary source mismatch for {label}")
    distribution = learned.get("training_distribution", {})
    if distribution.get("enabled") is not True:
        raise ValueError(f"boundary training distribution missing for {label}")
    if distribution.get("unknown_or_test_labels_used") is not False:
        raise ValueError(f"boundary training leakage for {label}")
    ...
    checks = (
        ...
        (
            float(distribution.get("hard_pseudo_fraction", float("nan"))),
            float(candidate["hard_pseudo_fraction"]),
            "hard pseudo fraction",
        ),
        (
            float(distribution.get("interpolation", float("nan"))),
            float(candidate["interpolation"]),
            "interpolation",
        ),
        (
            float(distribution.get("max_per_task", float("nan"))),
            float(candidate["max_per_task"]),
            "max per task",
        ),
    )
    ...
    return {
        ...
        "synthetic_boundary_samples": int(
            sum(row["synthetic_boundary_samples"] for row in distribution["tasks"])
        ),
    }
...
def build_rows(
    ...
    for suite, scenarios in confirmation["scenarios"].items():
        for scenario in scenarios:
            for seed in confirmation["seeds"]:
                ...
                candidate_report = metric_report(payload["reports"][selected], "candidate")
                reference_report = metric_report(payload["reports"][REFERENCE], "reference")
                if metric_report(payload["selected_report"], "selected") != candidate_report:
    ...
    if len(rows) != int(confirmation["expected_run_count"]):
        raise ValueError("boundary confirmation run count mismatch")
    return rows, {
        "passes": True,
        "run_count": len(rows),
...
def make_decision(
    ...
    result = {
        "frozen_gate": "strict_v4_boundary_pseudo_unknown_confirmation_v1",
        ...
        "boundary_endpoint_exercised": endpoints[CANDIDATE] > 0,
    ...
    result["passes"] = all(
        (
            ...
            result["boundary_endpoint_exercised"],
...
def main() -> None:
    ...
    result = {
        "schema_version": "strict_v4_boundary_pseudo_unknown_confirmation_v1",
