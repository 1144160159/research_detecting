...
# -*- encoding: utf-8 -*-
'''
@File    :   1_aecontra_seqsta_kmeans_ids_all.py
@Contact :   hanxueying@iie.ac.cn
@License :   (C)Copyright 2017-2018, Liugroup-NLPR-CASIA

@Modify Time      @Author    @Version    @Desciption
------------      -------    --------    -----------
2024/7/14 16:01   xueying      1.0       all表示的是正常数据和攻击数据都预先进行了聚类
'''
...
class InitTrainConfig:
    def __init__(self, used_config, k=1, cdr=0.05, prefix_name='kmeans0708', seq_margin=10, sta_margin=10,
        ...
        # 文件存储路径
        self.train_normal_root = '/home/hxy/driftDetect/0_data/1_cicids2018/3_nfstream_data/nfstream_pickle/normal'
        self.train_attack_root = '/home/hxy/driftDetect/0_data/1_cicids2018/3_nfstream_data/nfstream_pickle/reorganized_attack'
        self.test_normal_root = self.train_normal_root
        self.test_attack_root = self.train_attack_root

        self.train_coarse_root_normal = f'/home/hxy/driftDetect/0_data/1_cicids2018/4_ids_all_run_data/' \
        ...
        self.test_save_data_root_p2new = f'/home/hxy/driftDetect/newcode0625/{execute_folder_name}/save_data/part1/' \
                                      f'{self.prefix_name}_{self.used_config}_k{self.k}_dm{self.class_drop_ratio}_ak{self.a_k}_adm{self.a_class_drop_ratio}' \
        ...
        self.test_save_data_root_p2valnew = f'/home/hxy/driftDetect/newcode0625/{execute_folder_name}/save_data/part1/' \
                                         f'{self.prefix_name}_{self.used_config}_k{self.k}_dm{self.class_drop_ratio}_ak{self.a_k}_adm{self.a_class_drop_ratio}' \
        ...
        self.test_save_data_root_p4new = f'/home/hxy/driftDetect/newcode0625/{execute_folder_name}/save_data/part1/' \
                                      f'{self.prefix_name}_{self.used_config}_k{self.k}_dm{self.class_drop_ratio}_ak{self.a_k}_adm{self.a_class_drop_ratio}' \
        ...
        self.test_save_data_root_p4valnew = f'/home/hxy/driftDetect/newcode0625/{execute_folder_name}/save_data/part1/' \
                                         f'{self.prefix_name}_{self.used_config}_k{self.k}_dm{self.class_drop_ratio}_ak{self.a_k}_adm{self.a_class_drop_ratio}' \
        ...
        # if not os.path.exists(self.test_save_data_root_p2):
        #     os.makedirs(self.test_save_data_root_p2)
        # if not os.path.exists(self.test_save_data_root_p2val):
        #     os.makedirs(self.test_save_data_root_p2val)
        ...
        # if not os.path.exists(self.test_save_data_root_p3):
        #     os.makedirs(self.test_save_data_root_p3)
        # if not os.path.exists(self.test_save_data_root_p4):
        #     os.makedirs(self.test_save_data_root_p4)
        # if not os.path.exists(self.test_save_data_root_p4val):
        #     os.makedirs(self.test_save_data_root_p4val)
        ...
        #                       f'{self.prefix_name}_{self.used_config}_k{self.k}_dm{self.class_drop_ratio}_ak{self.a_k}_adm{self.a_class_drop_ratio}' \
        #                       f'_qm{self.seq_margin}tm{self.sta_margin}_l{self.sequence_length}_weighted_alq{self.alpha_seq_contra}_{self.alpha_seq_recon}_alt{self.alpha_sta_contra}_{self.alpha_sta_recon}_mq{seq_model_num}t{sta_model_num}/p2val'
        self.res_root_p2new = f'/home/hxy/driftDetect/newcode0625/{execute_folder_name}/result/part1/' \
                           f'{self.prefix_name}_{self.used_config}_k{self.k}_dm{self.class_drop_ratio}_ak{self.a_k}_adm{self.a_class_drop_ratio}' \
                           f'_qm{self.seq_margin}tm{self.sta_margin}_l{self.sequence_length}_weighted_alq{self.alpha_seq_contra}_{self.alpha_seq_recon}_alt{self.alpha_sta_contra}_{self.alpha_sta_recon}_mq{seq_model_num}t{sta_model_num}/p2new'
        self.res_root_p2valnew = f'/home/hxy/driftDetect/newcode0625/{execute_folder_name}/result/part1/' \
                              f'{self.prefix_name}_{self.used_config}_k{self.k}_dm{self.class_drop_ratio}_ak{self.a_k}_adm{self.a_class_drop_ratio}' \
                              f'_qm{self.seq_margin}tm{self.sta_margin}_l{self.sequence_length}_weighted_alq{self.alpha_seq_contra}_{self.alpha_seq_recon}_alt{self.alpha_sta_contra}_{self.alpha_sta_recon}_mq{seq_model_num}t{sta_model_num}/p2valnew'
        ...
        #                    f'{self.prefix_name}_{self.used_config}_k{self.k}_dm{self.class_drop_ratio}_ak{self.a_k}_adm{self.a_class_drop_ratio}' \
        #                    f'_qm{self.seq_margin}tm{self.sta_margin}_l{self.sequence_length}_weighted_alq{self.alpha_seq_contra}_{self.alpha_seq_recon}_alt{self.alpha_sta_contra}_{self.alpha_sta_recon}_mq{seq_model_num}t{sta_model_num}/p4val'
        self.res_root_p4new = f'/home/hxy/driftDetect/newcode0625/{execute_folder_name}/result/part1/' \
                           f'{self.prefix_name}_{self.used_config}_k{self.k}_dm{self.class_drop_ratio}_ak{self.a_k}_adm{self.a_class_drop_ratio}' \
                           f'_qm{self.seq_margin}tm{self.sta_margin}_l{self.sequence_length}_weighted_alq{self.alpha_seq_contra}_{self.alpha_seq_recon}_alt{self.alpha_sta_contra}_{self.alpha_sta_recon}_mq{seq_model_num}t{sta_model_num}/p4new'
        self.res_root_p4valnew = f'/home/hxy/driftDetect/newcode0625/{execute_folder_name}/result/part1/' \
                              f'{self.prefix_name}_{self.used_config}_k{self.k}_dm{self.class_drop_ratio}_ak{self.a_k}_adm{self.a_class_drop_ratio}' \
                              f'_qm{self.seq_margin}tm{self.sta_margin}_l{self.sequence_length}_weighted_alq{self.alpha_seq_contra}_{self.alpha_seq_recon}_alt{self.alpha_sta_contra}_{self.alpha_sta_recon}_mq{seq_model_num}t{sta_model_num}/p4valnew'

        if not os.path.exists(self.res_root_p1val):
            os.makedirs(self.res_root_p1val)
        # if not os.path.exists(self.res_root_p2):
        #     os.makedirs(self.res_root_p2)
        # if not os.path.exists(self.res_root_p2val):
        #     os.makedirs(self.res_root_p2val)
        ...
        # if not os.path.exists(self.res_root_p3):
        #     os.makedirs(self.res_root_p3)
        # if not os.path.exists(self.res_root_p4):
        #     os.makedirs(self.res_root_p4)
        # if not os.path.exists(self.res_root_p4val):
        #     os.makedirs(self.res_root_p4val)
        ...
        if self.relabel_attack:
            ...
            # label_cluster_attack_ids(True, self.train_attack_files, self.train_attack_root,
            #                          self.train_coarse_root_attack, self.detail_class_dict)
            # label_cluster_attack_ids(False, self.test_attack_files_p1val, self.test_attack_root,
            #                          self.test_coarse_root_attack, self.detail_class_dict)
            # label_cluster_attack_ids(False, self.test_attack_files_p2, self.test_attack_root,
            #                          self.test_coarse_root_attack, self.detail_class_dict)
            # label_cluster_attack_ids(False, self.test_attack_files_p2val, self.test_attack_root,
            #                          self.test_coarse_root_attack, self.detail_class_dict)
            # label_cluster_attack_ids(False, self.test_attack_files_p3, self.test_attack_root,
            #                          self.test_coarse_root_attack, self.detail_class_dict)
            # label_cluster_attack_ids(False, self.test_attack_files_p4, self.test_attack_root,
            #                          self.test_coarse_root_attack, self.detail_class_dict)
            ...
            # label_cluster_attack_for_test(self.test_attack_files_p2, self.test_attack_root, self.test_coarse_root_attack)
            # label_cluster_attack_for_test(self.test_attack_files_p2val, self.test_attack_root, self.test_coarse_root_attack)
            ...
            label_cluster_attack_for_test(self.test_attack_files_p2valnew, self.test_attack_root,
                                          self.test_coarse_root_attack)
            # label_cluster_attack_for_test(self.test_attack_files_p3, self.test_attack_root, self.test_coarse_root_attack)
            # label_cluster_attack_for_test(self.test_attack_files_p4, self.test_attack_root, self.test_coarse_root_attack)
            # label_cluster_attack_for_test(self.test_attack_files_p4val, self.test_attack_root, self.test_coarse_root_attack)
            ...
            # label_cluster_normal_for_test(self.test_normal_files_p2, self.test_normal_root, self.test_coarse_root_normal)
            # label_cluster_normal_for_test(self.test_normal_files_p2val, self.test_normal_root, self.test_coarse_root_normal)
            ...
            label_cluster_normal_for_test(self.test_normal_files_p2valnew, self.test_normal_root,
                                          self.test_coarse_root_normal)
            # label_cluster_normal_for_test(self.test_normal_files_p3, self.test_normal_root, self.test_coarse_root_normal)
            # label_cluster_normal_for_test(self.test_normal_files_p4, self.test_normal_root, self.test_coarse_root_normal)
            # label_cluster_normal_for_test(self.test_normal_files_p4val, self.test_normal_root, self.test_coarse_root_normal)
        ...
        # self.test_normal_paths_p4val = [os.path.join(self.test_coarse_root_normal, i) for i in self.test_normal_files_p4val]
        # self.test_attack_paths_p4val = [os.path.join(self.test_coarse_root_attack, i) for i in self.test_attack_files_p4val]
