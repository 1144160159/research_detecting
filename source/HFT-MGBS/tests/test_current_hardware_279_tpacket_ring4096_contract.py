...
RUNNER = ROOT / "scripts" / "run_current_hardware_279_tpacket_ring4096_diagnostic.sh"
CONFIG = ROOT / "configs" / "current_hardware_2_79_tpacket_ring4096_diagnostic.json"
...
class CurrentHardware279TpacketRunnerContractTests(unittest.TestCase):
    ...
    def test_config_freezes_b2_traffic_and_qm_runtime_affinity_full_pipeline(self):
        self.assertEqual(self.config["interfaces"]["capture"], "ens8f0")
        self.assertEqual(
            self.config["candidate_id"],
            "TPACKET_QM_RING4096_GEOMETRY_FULL_PIPELINE_DIAGNOSTIC",
        )
    ...
    def test_ring4096_is_the_only_geometry_variable_and_preserves_16_mib_per_worker(self):
        pipeline = self.config["pipeline"]
        self.assertEqual(pipeline["block_size"], 4096)
        self.assertEqual(pipeline["block_count"], 4096)
        self.assertEqual(pipeline["block_size"] * pipeline["block_count"], 16 * 1024 * 1024)
        self.assertEqual(pipeline["frame_size"], 256)
        self.assertEqual(pipeline["retire_block_timeout_ms"], 1)
        self.assertIn("--block-size 4096 --block-count 4096 --frame-size 256", self.script)
        self.assertIn("--retire-block-timeout-ms 1", self.script)
        self.assertNotIn("--block-size 65536 --block-count 256", self.script)
        for frozen in (
            'readonly formal_binary_sha=499b0b8e9abc14877d85fa0009489ffdf2e7f8d9527986e6cd1993008c2589fe',
            '--fanout-mode qm', '--worker-cpus 36 37 38 39 40 41 42 43',
            '--batch-size 8', '--active-timeout-s 1', '--duration-s 21',
        ):
            self.assertIn(frozen, self.script)

    def test_deterministic_multiflow_v2_is_frozen_and_read_back(self):
        traffic = self.config["traffic"]
        self.assertEqual(traffic["profile_id"], "deterministic_multiflow_v2")
        self.assertEqual(traffic["flows_per_queue"], 144)
        self.assertEqual(traffic["flowlen_packets"], 36)
