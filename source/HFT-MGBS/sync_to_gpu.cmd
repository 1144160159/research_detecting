...
tar -czf - --exclude=__pycache__ --exclude=*.pyc --exclude=.pytest_cache --exclude=target hft_mgbs configs deploy rust scripts tests *.md *.toml *.txt .gitignore sync_to_gpu.cmd ^
  | ssh -o BatchMode=yes -o ConnectTimeout=120 -o ClearAllForwardings=yes -o IdentitiesOnly=yes -p %REMOTE_PORT% -i "%SSH_KEY%" %REMOTE_HOST% ^
