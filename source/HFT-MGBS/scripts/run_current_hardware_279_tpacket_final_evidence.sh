#!/usr/bin/env bash
# One authorized, fail-closed TPACKET_V3 final-evidence candidate on the
# current BCM57810 host. Qualification is derived only from its sealed raw
# evidence and every hard gate; the script never assumes qualification.
set -Eeuo pipefail
umask 077

readonly evidence_root=/home/wangwt/task/datasets/replay
readonly default_config=/home/wangwt/phase_2/code/HFT-MGBS/configs/current_hardware_2_79_tpacket_final_evidence_contract.json
readonly default_binary=/home/wangwt/phase_2/code/HFT-MGBS/rust/hft-capture/target/release/tpacket_v3_full_pipeline
...
readonly formal_binary_sha=9c4e6cfab251b1d595dc9366f77752f8c333f9e2b2fd1092f2f1ebc7aa557255
...
expected = {
    ("schema_version",): 1,
    ("scope",): "current_hardware_2_79_tpacket_final_evidence_contract",
    ("candidate_id",): "TPACKET_QM_FIXED64_GROUP512_A09_FINAL_EVIDENCE_CONTRACT",
    ...
    ("traffic", "profile_id"): "deterministic_multiflow_v3_flowlen1_clone64_burst8_group512",
    ("traffic", "flows_per_queue"): 145,
    ("traffic", "flowlen_packets"): 1,
    ("traffic", "flow_sequence_flag"): "FLOW_SEQ",
    ("traffic", "unique_destination_addresses_per_queue"): 145,
    ("traffic", "destination_range_semantics"): "inclusive_on_observed_linux_5_10_pktgen",
    ("traffic", "expected_global_distinct_flows"): 1160,
    ("traffic", "generator_flow_rebuild_packets"): 512,
    ("traffic", "estimated_flow_materializations_per_second_at_2_8_mpps"): 5469,
    ("traffic", "estimate_is_not_a_qualification_claim"): True,
    ("traffic", "udp_destination_port"): 53,
    ("traffic", "clone_skb"): 64,
    ("traffic", "burst"): 8,
    ("traffic", "observed_group_packets"): 512,
    ("traffic", "sequence_step"): 512,
    ("traffic", "sequence_residue"): 1,
    ("traffic", "rx_usecs"): 24,
    ("traffic", "rx_ring"): 4078,
    ("traffic", "generator_duration_seconds"): 19,
    ("pipeline", "binary_sha256"): "9c4e6cfab251b1d595dc9366f77752f8c333f9e2b2fd1092f2f1ebc7aa557255",
    ("pipeline", "parser_profile_id"): "deterministic_multiflow_v2_fixed64_ipv4_udp_strict_v1",
    ...
    ("runtime_hard_gates", "minimum_full_epoch_mpps"): 2.79,
    ("runtime_hard_gates", "packet_socket_drops_max"): 0,
    ("runtime_hard_gates", "packet_socket_freeze_queue_count_max"): 0,
    ("runtime_hard_gates", "parse_rejected_max"): 0,
    ("runtime_hard_gates", "fixed_profile_general_fallback_max"): 0,
    ("runtime_hard_gates", "feature_queue_drops_max"): 0,
    ("runtime_hard_gates", "key_feature_queue_drops_max"): 0,
    ("runtime_hard_gates", "gpu_queue_full_max"): 0,
    ("runtime_hard_gates", "gpu_batches_failed_max"): 0,
    ("runtime_hard_gates", "gpu_backend_identity_failures_max"): 0,
    ("runtime_hard_gates", "gpu_worker_join_failures_max"): 0,
    ("runtime_hard_gates", "key_flows_enqueue_failed_max"): 0,
    ("runtime_hard_gates", "key_flows_inference_failed_max"): 0,
    ("runtime_hard_gates", "key_flows_terminal_unresolved_max"): 0,
    ("runtime_hard_gates", "key_flows_recovery_pending_max"): 0,
    ("runtime_hard_gates", "flow_completion_receipts_truncated_max"): 0,
    ("runtime_hard_gates", "require_flow_completion_conservation"): True,
    ("runtime_hard_gates", "per_full_window_minimums", "packet_continuity_observed_valid_packets"): 1,
    ("runtime_hard_gates", "per_full_window_minimums", "flow_completion_receipts"): 1000,
    ("runtime_hard_gates", "per_full_window_minimums", "gpu_batch_evidence"): 100,
    ("runtime_hard_gates", "require_raw_source_ids_unique"): True,
    ("runtime_hard_gates", "require_raw_receipts_not_truncated"): True,
    ("continuity_hard_gate", "profile_id"): "linux_pktgen_ipv4_udp_be_header_clone64_burst8_group512_v2",
    ("continuity_hard_gate", "clone_skb"): 64,
    ("continuity_hard_gate", "burst"): 8,
    ("continuity_hard_gate", "observed_group_packets"): 512,
    ("continuity_hard_gate", "sequence_step"): 512,
    ("execution", "network_run_performed_by_this_change"): False,
    ("execution", "default_without_exact_authorization"): "fail_before_evidence_or_host_mutation",
    ("execution", "flow_completion_density_precondition_proved"): False,
    ("execution", "static_block_before_evidence_or_host_mutation"): True,
    ("execution", "static_block_exit_code"): 86,
    ("qualification", "prequalified"): False,
}
...
# The evidence path is complete but intentionally non-executable until a
# separate, versioned flow-churn capacity proof exists. With clone_skb=64 and
# burst=8, one rebuilt flow contributes about 512 packets. At 2.8 Mpps and
# 1160 flow identities, a flow is revisited about every 0.21 seconds. Rust
# active expiry uses last_ts-first_ts >= 1 second, while idle expiry needs one
# second without a revisit. Therefore neither the required >=1000 completion
# receipts/window nor the corresponding GPU capacity is proved. Exact host
# authorization cannot override this evidence precondition.
echo "final-evidence blocked before evidence or host mutation: flow completion density and GPU capacity are not proved for clone64/burst8 group512" >&2
exit 86
...
value = {
    "schema_version": 1,
    "scope": "current_hardware_2_79_tpacket_final_evidence_raw_receipt",
    "candidate_id": "TPACKET_QM_FIXED64_GROUP512_A09_FINAL_EVIDENCE_CONTRACT",
...
ethtool -C "${capture_nic}" rx-usecs 24
modprobe pktgen
...
for index in $(seq 0 7); do
  ...
  # This host's Linux 5.10 pktgen was proven by QM runtime evidence to include
  # both endpoints: .1 through .145 yielded exactly 145 flows per queue.
...
for index in range(8):
    ...
    checks = {
        ...
        "flows": r"\bflows:\s*145\b",
        "flowlen": r"\bflowlen:\s*1\b",
        ...
        "udp_src": rf"\budp_src_min:\s*{10000 + index}\s+udp_src_max:\s*{10000 + index}\b",
        "src_identity": rf"\bsrc_min:\s*10\.{index}\.0\.1\s+src_max:\s*10\.{index}\.0\.1\b",
    ...
    current = re.search(r"^Current:\s*$([\s\S]*?)^Result:", text, re.MULTILINE)
    if current is None: failed.append("current_section")
    else:
        current_checks = {
            "current_src": rf"\bcur_saddr:\s*10\.{index}\.0\.1\b",
            "current_dst_inclusive_max": rf"\bcur_daddr:\s*11\.{index}\.0\.145\b",
            "current_udp": rf"\bcur_udp_dst:\s*53\s+cur_udp_src:\s*{10000 + index}\b",
            "current_queue": rf"\bcur_queue_map:\s*{index}\b",
            "current_not_started": r"\bpkts-sofar:\s*0\s+errors:\s*0\b",
        }
        failed.extend(name for name, pattern in current_checks.items()
                      if re.search(pattern, current.group(1)) is None)
    if failed: raise SystemExit(f"pktgen Params/Current mismatch queue {index}: {failed}")
(root / "pktgen_params_validation.txt").write_text(
    "profile=deterministic_multiflow_v3_flowlen1\nrange_semantics=inclusive\n"
    "flows_per_queue=145\nqueues_validated=8\nstatus=passed\n",
    encoding="ascii")
PY
...
setsid "${frozen_binary}" \
  ...
  --expected-gpu-candidate A09 --expected-gpu-schema 1 \
...
root = pathlib.Path(sys.argv[1])
for index in range(8):
    text = (root / f"pktgen_device_{index}.txt").read_text(encoding="utf-8")
    current = re.search(r"^Current:\s*$([\s\S]*?)^Result:", text, re.MULTILINE)
    if current is None: raise SystemExit(f"pktgen Current missing queue {index}")
    body = current.group(1)
    def one(pattern, label):
        match = re.search(pattern, body)
        if match is None: raise SystemExit(f"pktgen Current {label} missing queue {index}")
        return match.group(1)
    packets = int(one(r"\bpkts-sofar:\s*([0-9]+)\s+errors:\s*0\b", "packet/error"))
    source = one(r"\bcur_saddr:\s*([^\s]+)", "source")
    destination = one(r"\bcur_daddr:\s*([^\s]+)", "destination")
    udp_dst = int(one(r"\bcur_udp_dst:\s*([0-9]+)", "UDP destination"))
    udp_src = int(one(r"\bcur_udp_src:\s*([0-9]+)", "UDP source"))
    queue = int(one(r"\bcur_queue_map:\s*([0-9]+)", "queue map"))
    lower = ipaddress.ip_address(f"11.{index}.0.1")
    upper = ipaddress.ip_address(f"11.{index}.0.145")
    if not (packets > 0 and source == f"10.{index}.0.1"
            and lower <= ipaddress.ip_address(destination) <= upper
            and udp_dst == 53 and udp_src == 10000 + index and queue == index):
        raise SystemExit(f"pktgen Current identity/range mismatch queue {index}")
(root / "pktgen_current_validation.txt").write_text(
    "profile=deterministic_multiflow_v3_flowlen1\nrange_semantics=inclusive\n"
    "queues_validated=8\npackets_positive=true\nerrors_zero=true\nstatus=passed\n",
    encoding="ascii")
PY
wait "${capture_pid}"
capture_pid=
...
python3 - "${evidence_dir}/pipeline_raw.json" "${evidence_dir}/final_evidence_gate.json" <<'PY'
import collections, json, os, pathlib, sys, tempfile
...
required = {
    ...
    "expected_gpu_identity": {
        "candidate_id": "A09",
        "schema_version": 1,
        "model_sha256": "fa9d29858bb7a20f9a66be2105a6182368e4b3029a59ead5fd77f6228b0eb5d2",
...
if not isinstance(v.get("min_full_epoch_mpps"), (int, float)) or v["min_full_epoch_mpps"] < 2.79:
    raise SystemExit("minimum complete-window throughput is below 2.79 Mpps")
...
if not isinstance(epoch_counts, dict) or len(epoch_counts) < 3:
    raise SystemExit("epoch-second evidence cannot identify complete windows")
epoch_seconds = sorted(int(key) for key in epoch_counts)
full_window_ids = epoch_seconds[1:-1]
if len(full_window_ids) != windows:
    raise SystemExit("full-window identities disagree with full_epoch_windows")
metrics = v.get("pipeline_metrics")
if not isinstance(metrics, dict): raise SystemExit("pipeline metrics missing")
zero_top = ("parse_rejected", "fixed_profile_general_fallback", "feature_queue_drops",
            "key_feature_queue_drops", "packet_socket_drops", "packet_socket_freeze_queue_count")
zero_metrics = ("gpu_queue_full", "gpu_batches_failed", "gpu_backend_identity_failures",
                "gpu_worker_join_failures", "key_flows_enqueue_failed",
                "key_flows_inference_failed", "key_flows_terminal_unresolved",
                "key_flows_recovery_pending")
for name in zero_top:
    if v.get(name) != 0: raise SystemExit(f"nonzero loss/fallback counter: {name}")
for name in zero_metrics:
    if metrics.get(name) != 0: raise SystemExit(f"nonzero GPU/capacity counter: {name}")
if metrics.get("flow_completion_receipts_truncated") != 0:
    raise SystemExit("flow completion receipts were truncated")
conservation = metrics.get("flow_completion_conservation")
if not isinstance(conservation, dict) or conservation.get("remote_scored_equals_receipts_plus_truncated") is not True \
        or conservation.get("absolute_delta") != 0:
    raise SystemExit("flow completion conservation failed")
continuity = v.get("packet_continuity")
if not isinstance(continuity, dict): raise SystemExit("packet continuity raw evidence missing")
continuity_expected = {
    "supported": True,
    "profile_id": "linux_pktgen_ipv4_udp_be_header_clone64_burst8_group512_v2",
    "clone_skb": 64, "burst": 8, "observed_group_packets": 512, "sequence_step": 512,
    "packet_gap": 0, "duplicate_packets": 0, "reordered_group_packets": 0,
    "invalid_packets": 0, "unsupported_transitions": 0, "queue_owner_conflicts": 0,
    "input_conservation_ok": True, "ownership_merge_conservation_ok": True,
}
for name, expected in continuity_expected.items():
    if continuity.get(name) != expected: raise SystemExit(f"packet continuity mismatch: {name}")
continuity_windows = v.get("packet_continuity_windows")
if not isinstance(continuity_windows, list): raise SystemExit("packet continuity windows missing")
continuity_by_window = {item.get("epoch_second"): item for item in continuity_windows if isinstance(item, dict)}
completion = metrics.get("flow_completion_receipts")
batches = metrics.get("gpu_batch_evidence")
if not isinstance(completion, list) or not isinstance(batches, list):
    raise SystemExit("GPU completion or batch raw evidence missing")
completion_ids = [item.get("source_id") for item in completion if isinstance(item, dict)]
if len(completion_ids) != len(completion) or any(not isinstance(item, str) or not item for item in completion_ids) \
        or len(set(completion_ids)) != len(completion_ids):
    raise SystemExit("flow completion source identities missing or not unique")
if any(item.get("backend_identity") != "A09/schema_v1/ordered_v1" for item in completion):
    raise SystemExit("flow completion backend identity mismatch")
batch_ids = [item.get("source_id") for item in batches if isinstance(item, dict)]
if len(batch_ids) != len(batches) or any(not isinstance(item, str) or not item for item in batch_ids) \
        or len(set(batch_ids)) != len(batch_ids):
    raise SystemExit("GPU batch source identities missing or not unique")
minimums = {"packet_continuity_observed_valid_packets": 1,
            "flow_completion_receipts": 1000, "gpu_batch_evidence": 100}
counts = {metric: collections.Counter() for metric in minimums}
for window, item in continuity_by_window.items():
    if isinstance(window, int): counts["packet_continuity_observed_valid_packets"][window] = item.get("observed_valid_packets", 0)
for item in completion:
    if isinstance(item.get("window_id"), int): counts["flow_completion_receipts"][item["window_id"]] += 1
for item in batches:
    if isinstance(item.get("window_id"), int): counts["gpu_batch_evidence"][item["window_id"]] += 1
per_window = {
    str(window): {metric: counts[metric][window] for metric in minimums}
    for window in full_window_ids
}
failures = [
    {"window_id": window, "metric": metric, "observed": counts[metric][window], "required": required}
    for window in full_window_ids for metric, required in minimums.items()
    if counts[metric][window] < required
]
gate = {
    "schema_version": 1,
    "source_pipeline_raw": source.name,
    "full_window_ids": full_window_ids,
    "minimums": minimums,
    "per_window_counts": per_window,
    "source_ids_unique": len(set(completion_ids)) == len(completion_ids) and len(set(batch_ids)) == len(batch_ids),
    "receipts_truncated": metrics.get("flow_completion_receipts_truncated"),
    "expected_gpu_identity": required["expected_gpu_identity"],
    "packet_continuity_profile": continuity.get("profile_id"),
    "minimum_full_epoch_mpps": v.get("min_full_epoch_mpps"),
    "synthetic_or_duplicated_receipts_allowed": False,
    "failures": failures,
    "passed": not failures,
}
handle=tempfile.NamedTemporaryFile("w", encoding="utf-8", newline="\n",
    dir=str(gate_path.parent), prefix=gate_path.name+".", suffix=".tmp", delete=False)
with handle:
    json.dump(gate, handle, indent=2, sort_keys=True, allow_nan=False)
    handle.write("\n"); handle.flush(); os.fsync(handle.fileno())
pathlib.Path(handle.name).replace(gate_path)
if failures:
    raise SystemExit("per-full-window continuity/GPU capacity receipt minimum failed")
# The raw object deliberately keeps its embedded qualification flags false.
# This independent derived gate is the only final-evidence candidate receipt.
PY
