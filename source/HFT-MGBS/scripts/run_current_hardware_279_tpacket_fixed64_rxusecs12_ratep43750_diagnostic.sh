...
readonly default_config=/home/wangwt/phase_2/code/HFT-MGBS/configs/current_hardware_2_79_tpacket_fixed64_rxusecs12_ratep43750_diagnostic.json
...
expected = {
    ...
    ("traffic", "burst"): 8,
    ("traffic", "ratep_per_burst_call"): 43750,
    ("traffic", "expected_delay_ns"): 22857,
    ("traffic", "expected_effective_pps_per_queue_min"): 345000,
    ("traffic", "expected_effective_pps_per_queue_max"): 355000,
    ("traffic", "expected_effective_pps_aggregate_min"): 2800000,
    ("traffic", "rx_usecs"): 12,
    ...
    ("runtime_hard_gates", "require_capture_lossless"): True,
    ("runtime_hard_gates", "require_internal_delivery_lossless"): True,
    ("runtime_hard_gates", "require_all_workers_error_free"): True,
...
for index in range(8):
    ...
    for name, pattern in {
        ...
        "errors": r"\berrors:\s*0\s*$",
    }.items():
        if re.search(pattern, text, re.MULTILINE) is None:
            raise SystemExit(f"pktgen post-run mismatch queue {index}: {name}")
    matches = re.findall(r"^\s*([0-9]+)pps\s+.*\berrors:\s*0\s*$", text, re.MULTILINE)
    if len(matches) != 1:
        raise SystemExit(f"pktgen unique Result rate missing queue {index}")
    rate = int(matches[0])
    if not 345000 <= rate <= 355000:
        queue_failures.append({"queue": index, "pps": rate})
    rates.append(rate)
value = {
    "schema_version": 1,
    ...
    "per_queue_pps": rates,
    "aggregate_pps": sum(rates),
    "per_queue_minimum_pps": 345000,
    "per_queue_maximum_pps": 355000,
    "aggregate_minimum_pps": 2800000,
    "queues_outside_range": queue_failures,
    "passed": not queue_failures and sum(rates) >= 2800000,
}
(root / "pktgen_rate_gate.json").write_text(
    json.dumps(value, indent=2, sort_keys=True, allow_nan=False) + "\n", encoding="ascii")
if not value["passed"]:
    raise SystemExit("pktgen rate gate failed; see pktgen_rate_gate.json")
PY
...
capacity_gate = {
    "schema_version": 1,
    "scope": "current_hardware_2_79_capacity_capture_gate",
    "minimum_full_epoch_mpps": minimum,
    "minimum_required_full_epoch_mpps": 2.79,
    "full_epoch_windows": v.get("full_epoch_windows"),
    "packet_socket_drops": v.get("packet_socket_drops"),
    "packet_socket_freeze_queue_count": v.get("packet_socket_freeze_queue_count"),
    "feature_queue_drops": v.get("feature_queue_drops"),
    "key_feature_queue_drops": v.get("key_feature_queue_drops"),
    "parse_rejected": v.get("parse_rejected"),
    "capture_lossless": v.get("capture_lossless"),
    "internal_delivery_lossless": v.get("internal_delivery_lossless"),
    "all_workers_error_free": v.get("all_workers_error_free"),
}
capacity_gate["passed"] = (
    ...
    and all(capacity_gate[key] == 0 for key in (
        "packet_socket_drops", "packet_socket_freeze_queue_count",
        "feature_queue_drops", "key_feature_queue_drops", "parse_rejected"))
    and all(capacity_gate[key] is True for key in (
        "capture_lossless", "internal_delivery_lossless", "all_workers_error_free"))
)
pathlib.Path(sys.argv[1]).with_name("capacity_capture_gate.json").write_text(
    json.dumps(capacity_gate, indent=2, sort_keys=True, allow_nan=False) + "\n",
    encoding="ascii")
if not capacity_gate["passed"]:
    raise SystemExit("capacity capture gate failed")
