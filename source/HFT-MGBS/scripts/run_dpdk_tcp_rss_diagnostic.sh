...
if [[ "${HFT_DPDK_PREFLIGHT_ONLY:-}" != "YES" \
  && "${HFT_ALLOW_DISRUPTIVE_DPDK:-}" != "YES" ]]; then
...
DPDK_SOURCE_ROOT="${DPDK_SOURCE_ROOT:-${CODE_ROOT}/.deps/src/dpdk-stable-${DPDK_VERSION}}"
THRESHOLDS_FILE="$1"
DPDK_BINARY="${CODE_ROOT}/rust/hft-dpdk/target/release/hft-dpdk"
BNX2X_ETHDEV="${DPDK_SOURCE_ROOT}/drivers/net/bnx2x/bnx2x_ethdev.c"
...
for command_name in awk basename cat cp date diff ethtool find flock fuser grep \
  ...
  sleep sort rmdir sed stat sysctl tail tc timeout tr umount xargs ps; do
...
if ! jq -e '
  .schema_version == 1
  and .scope == "dpdk_stock_bnx2x_tcp_rss_q2_diagnostic"
  and .execution_mode == "one_shot_fail_closed"
  and .frozen == true
  and .diagnostic_only == true
  and .final_pareto_ingestion_allowed == false
  and .max_pipeline_drop_rate == 0
  and .traffic_profile == "ipv4_tcp_multiflow"
  and .rust_traffic_profile == "tcp-rss-diagnostic"
  and .synthetic_flow_count == 256
  and .timestamp_offset_bytes == 54
  and .queue_count == 2
  and .min_per_queue_packet_fraction == 0.40
  and .capture_port_queues == {"rx": 2, "tx": 2}
  and .replay_port_queues == {"rx": 2, "tx": 2}
  and .q4_allowed == false
  and .q2_failure_stops_branch == true
  ...
  and (.expected_validator_sha256 | type == "string"
    and test("^[0-9a-f]{64}$"))
  and (.expected_bnx2x_ethdev_sha256 | type == "string"
    and test("^[0-9a-f]{64}$"))
  ...
  and (.rx_cpus | type == "array" and length == 2
    and all(.[]; type == "number" and . >= 0 and . == floor))
  and (.tx_cpus | type == "array" and length == 2
    and all(.[]; type == "number" and . >= 0 and . == floor))
  and (([.main_cpu] + .rx_cpus + .tx_cpus | unique | length) == 5)
  ...
  and .min_run_duration_s == 15
  and .min_rate_full_windows >= 15
  and .target_load_mpps == 1.0
...
' "${THRESHOLDS_FILE}" >/dev/null; then
  echo "thresholds do not satisfy stock TCP RSS Q2 diagnostic schema" >&2
  exit 4
fi
...
expected_bnx2x_ethdev_sha256="$(jq -er '.expected_bnx2x_ethdev_sha256' \
  "${THRESHOLDS_FILE}")"
...
runner_sha256="$(sha256sum "${CODE_ROOT}/scripts/run_dpdk_tcp_rss_diagnostic.sh" \
  | awk '{print $1}')"
validator_sha256="$(sha256sum "${CODE_ROOT}/scripts/validate_dpdk_tcp_rss_diagnostic.py" \
  | awk '{print $1}')"
...
dpdk_manifest_sha256="$(sha256sum "${DPDK_ROOT}/hft-build-manifest.txt" \
  | awk '{print $1}')"
bnx2x_ethdev_sha256="$(sha256sum "${BNX2X_ETHDEV}" | awk '{print $1}')"
...
if [[ "${runner_sha256}" != "${expected_runner_sha256}" \
  ...
  || "${bnx2x_ethdev_sha256}" != "${expected_bnx2x_ethdev_sha256}" \
  ...
  echo "diagnostic scripts, DPDK manifest, or bnx2x source do not match the frozen candidate" >&2
  exit 4
fi
if ! grep -Fxq 'experimental_bnx2x_rss=NO' "${DPDK_ROOT}/hft-build-manifest.txt" \
  || ! grep -Fxq "bnx2x_ethdev_sha256=${bnx2x_ethdev_sha256}" \
    "${DPDK_ROOT}/hft-build-manifest.txt"; then
  echo "DPDK build manifest is not the frozen stock bnx2x source" >&2
  exit 4
fi
...
if [[ "${HFT_DPDK_PREFLIGHT_ONLY:-}" == "YES" ]]; then
  run_id="hft_tcp_rss_q2_preflight_$(date -u +%Y%m%dT%H%M%S%NZ)"
else
  run_id="hft_tcp_rss_q2_$(date -u +%Y%m%dT%H%M%S%NZ)"
fi
...
cp --reflink=auto --preserve=timestamps \
  ...
  "${CODE_ROOT}/scripts/run_dpdk_tcp_rss_diagnostic.sh" \
  "${CODE_ROOT}/scripts/validate_dpdk_tcp_rss_diagnostic.py" \
  "${CODE_ROOT}/scripts/preflight_dpdk_cpu_idle.py" \
...
cp --reflink=auto --preserve=timestamps \
  "${BNX2X_ETHDEV}" "${run_dir}/bnx2x_ethdev.c"
...
if (( cpu_preflight_status != 0 )); then
  blocked_id="hft_tcp_rss_q2_preflight_blocked_$(date -u +%Y%m%dT%H%M%S%NZ)"
  ...
  {
    echo "run_id=${run_id}"
    echo "scope=dpdk_stock_bnx2x_tcp_rss_q2_cpu_preflight_blocked"
...
if [[ "${HFT_DPDK_PREFLIGHT_ONLY:-}" == "YES" ]]; then
  {
    echo "run_id=${run_id}"
    echo "scope=dpdk_stock_bnx2x_tcp_rss_q2_non_mutating_preflight"
    echo "candidate_id=${candidate_id}"
    echo "frozen_thresholds_sha256=${thresholds_sha256}"
...
exact_dpdk_process_gate() {
  local proc_path executable process_name
  for proc_path in /proc/[0-9]*; do
    [[ -e "${proc_path}/exe" ]] || continue
    executable="$(readlink -f "${proc_path}/exe" 2>/dev/null || true)"
    process_name="${executable##*/}"
    case "${process_name}" in
      hft-dpdk|hft-dpdk.bin|dpdk-testpmd|testpmd|dpdk-test|dpdk-proc-info)
        echo "active DPDK executable at pre-mutation gate: ${proc_path##*/} ${executable}" \
          >&2
        return 1
        ;;
    esac
...
exact_dpdk_runtime_gate() {
  local runtime_path
  [[ ! -e "${DPDK_RUNTIME_ROOT}/${run_id}" ]] || return 1
  for runtime_path in "${DPDK_RUNTIME_ROOT}"/*/config \
    "${DPDK_RUNTIME_ROOT}"/*/mp_socket; do
    [[ -e "${runtime_path}" || -S "${runtime_path}" ]] || continue
    if fuser "${runtime_path}" >/dev/null 2>&1; then
      echo "active DPDK runtime at pre-mutation gate: ${runtime_path}" >&2
      return 1
    fi
  done
  if mountpoint -q "${HUGEPAGE_MOUNT}" \
    && find "${HUGEPAGE_MOUNT}" -maxdepth 1 -type f -print -quit | grep -q .; then
    echo "hugepage ownership changed after preflight" >&2
    return 1
  fi
  return 0
}

verify_run_dir_frozen_artifacts() {
  [[ "$(sha256sum "${THRESHOLDS_FILE}" | awk '{print $1}')" \
    == "${thresholds_sha256}" ]] || return 1
  [[ "$(sha256sum "${run_dir}/frozen_thresholds.json" | awk '{print $1}')" \
    == "${thresholds_sha256}" ]] || return 1
  [[ "$(sha256sum "${DPDK_BINARY}" | awk '{print $1}')" \
    == "${expected_binary_sha256}" ]] || return 1
  [[ "$(sha256sum "${run_dir}/hft-dpdk.bin" | awk '{print $1}')" \
    == "${expected_binary_sha256}" ]] || return 1
  [[ "$(sha256sum "${CODE_ROOT}/scripts/run_dpdk_tcp_rss_diagnostic.sh" \
    | awk '{print $1}')" == "${expected_runner_sha256}" ]] || return 1
  [[ "$(sha256sum "${run_dir}/run_dpdk_tcp_rss_diagnostic.sh" | awk '{print $1}')" \
    == "${expected_runner_sha256}" ]] || return 1
  [[ "$(sha256sum "${CODE_ROOT}/scripts/validate_dpdk_tcp_rss_diagnostic.py" \
    | awk '{print $1}')" == "${expected_validator_sha256}" ]] || return 1
  [[ "$(sha256sum "${run_dir}/validate_dpdk_tcp_rss_diagnostic.py" \
    | awk '{print $1}')" == "${expected_validator_sha256}" ]] || return 1
  [[ "$(sha256sum "${run_dir}/preflight_dpdk_cpu_idle.py" | awk '{print $1}')" \
    == "${expected_cpu_preflight_sha256}" ]] || return 1
  [[ "$(sha256sum "${run_dir}/preflight_dpdk_bnx2x.py" | awk '{print $1}')" \
    == "${expected_dpdk_preflight_sha256}" ]] || return 1
  [[ "$(sha256sum "${run_dir}/dpdk-build-manifest.txt" | awk '{print $1}')" \
    == "${expected_dpdk_manifest_sha256}" ]] || return 1
  [[ "$(sha256sum "${run_dir}/bnx2x_ethdev.c" | awk '{print $1}')" \
    == "${expected_bnx2x_ethdev_sha256}" ]] || return 1
  for relative_path in Cargo.toml build.rs csrc/hft_dpdk_shim.h \
    csrc/hft_dpdk_shim.c src/main.rs; do
    diff -q "${CODE_ROOT}/rust/hft-dpdk/${relative_path}" \
      "${run_dir}/$(basename "${relative_path}")" >/dev/null || return 1
  done
  return 0
}

write_pre_mutation_gate() {
  local process_clear="$1"
  local runtime_clear="$2"
  local artifacts_verified="$3"
  jq -n \
    --arg checked_at "$(date -u +%Y-%m-%dT%H:%M:%SZ)" \
    --arg run_id "${run_id}" \
    --arg contract_sha256 "${thresholds_sha256}" \
    --arg runner_sha256 "${expected_runner_sha256}" \
    --arg validator_sha256 "${expected_validator_sha256}" \
    --arg binary_sha256 "${expected_binary_sha256}" \
    --arg cpu_preflight_script_sha256 "${expected_cpu_preflight_sha256}" \
    --arg dpdk_preflight_script_sha256 "${expected_dpdk_preflight_sha256}" \
    --arg dpdk_build_manifest_sha256 "${expected_dpdk_manifest_sha256}" \
    --arg bnx2x_ethdev_sha256 "${expected_bnx2x_ethdev_sha256}" \
    --argjson process_clear "${process_clear}" \
    --argjson runtime_clear "${runtime_clear}" \
    --argjson artifacts_verified "${artifacts_verified}" \
    '{
      schema_version: 1,
      ...
      exact_dpdk_process_gate_clear: $process_clear,
      exact_dpdk_runtime_gate_clear: $runtime_clear,
      frozen_and_source_artifact_rehash_verified: $artifacts_verified,
      frozen_artifact_sha256: {
        contract: $contract_sha256,
        runner: $runner_sha256,
        validator: $validator_sha256,
        binary: $binary_sha256,
        cpu_preflight_script: $cpu_preflight_script_sha256,
        dpdk_preflight_script: $dpdk_preflight_script_sha256,
        dpdk_build_manifest: $dpdk_build_manifest_sha256,
        bnx2x_ethdev: $bnx2x_ethdev_sha256
      },
      passed: ($process_clear and $runtime_clear and $artifacts_verified)
    }' > "${run_dir}/pre_mutation_gate.json"
}

seal_pre_mutation_block() {
  local exit_status="$1"
  {
    echo "run_id=${run_id}"
    echo "scope=dpdk_stock_bnx2x_tcp_rss_q2_pre_mutation_blocked"
    echo "candidate_id=${candidate_id}"
    echo "frozen_thresholds_sha256=${thresholds_sha256}"
    echo "mutations_performed=false"
...
bind_bnx2x() {
  ...
  if [[ "${driver}" == "uio_pci_generic" ]]; then
    write_sysfs_value "${pci}" /sys/bus/pci/drivers/uio_pci_generic/unbind \
      || status=1
  elif [[ "${driver}" != "none" && "${driver}" != "bnx2x" ]]; then
    status=1
  fi
  if [[ "$(driver_for_pci "${pci}")" != "bnx2x" ]]; then
    write_sysfs_value bnx2x "/sys/bus/pci/devices/${pci}/driver_override" \
      || status=1
    write_sysfs_value "${pci}" /sys/bus/pci/drivers_probe || status=1
  fi
  restore_driver_override "${pci}" "${original_override}" || status=1
  [[ "$(driver_for_pci "${pci}")" == "bnx2x" ]] || status=1
  return "${status}"
}
...
restore_interface_settings() {
  ...
  local admin_up="${11}"
  local status=0
  # Every setting is attempted independently.  A first ethtool failure must not
  # suppress MTU/admin recovery or the remaining restore actions.
  ethtool -L "${interface}" combined "${combined}" || status=1
  ethtool -G "${interface}" rx "${rx_ring}" tx "${tx_ring}" || status=1
  ethtool -C "${interface}" rx-usecs "${rx_usecs}" tx-usecs "${tx_usecs}" \
    || status=1
  ethtool -K "${interface}" gro "${gro}" lro "${lro}" || status=1
  ip link set dev "${interface}" mtu "${mtu}" txqueuelen "${txqlen}" \
    || status=1
  if [[ "${admin_up}" == true ]]; then
    ip link set dev "${interface}" up || status=1
  else
    ip link set dev "${interface}" down || status=1
  fi
  return "${status}"
}
...
remove_runtime_prefix() {
  ...
  local -a active_pids=()
  mapfile -t active_pids < <(owned_process_pids)
  if (( ${#active_pids[@]} != 0 )); then
    return 1
  fi
  if [[ ! "${run_id}" =~ ^hft_tcp_rss_q2_[0-9]{8}T[0-9]{6}[0-9]+Z$ \
    || "${runtime_dir}" != /var/run/dpdk/hft_tcp_rss_q2_* ]]; then
    return 1
  fi
...
process_has_run_id() {
  ...
  tr '\0' '\n' < "/proc/${pid}/cmdline" \
    | awk -v run_id="${run_id}" '
      previous == "--file-prefix" && $0 == run_id {owned=1}
      {previous=$0}
      END {exit owned ? 0 : 1}
    '
}
...
owned_process_pids() {
  local proc_path
  for proc_path in /proc/[0-9]*; do
    process_has_run_id "${proc_path##*/}" && printf '%s\n' "${proc_path##*/}"
  done
  return 0
}
...
stop_child() {
  ...
  local group_alive=0
  local group_owned=0
  local proc_path proc_pgid owned_pid
  local -a owned_pids=()
  ...
  if (( group_alive == 1 )); then
    for proc_path in /proc/[0-9]*; do
      [[ -r "${proc_path}/stat" && -r "${proc_path}/cmdline" ]] || continue
      proc_pgid="$(ps -o pgid= -p "${proc_path##*/}" 2>/dev/null \
        | awk '{$1=$1; print; exit}')"
      [[ "${proc_pgid}" == "${child_pgid}" ]] || continue
      if process_has_run_id "${proc_path##*/}"; then
        group_owned=1
        break
      fi
    done
  ...
  # If setsid failed, the group already disappeared, or a verified group left
  # descendants, signal only PIDs whose argv still proves this exact run_id.
  mapfile -t owned_pids < <(owned_process_pids)
  for owned_pid in "${owned_pids[@]}"; do
    process_has_run_id "${owned_pid}" \
...
restore_host() {
  ...
  set +e
  # A failed child-stop verification must be recorded, but must never skip the
  # remaining PF, interface, runtime-prefix, hugepage and module restoration.
  run_restore_step child_stopped stop_child
...
build_evidence() {
  ...
  record_restore_step restored_snapshot_collection "${snapshot_status}"
  verify_host_restoration
  verify_status="$?"
  ...
  python3 "${run_dir}/validate_dpdk_tcp_rss_diagnostic.py" \
    --contract "${run_dir}/frozen_thresholds.json" \
    --result "${run_dir}/result.json" \
    --runner "${run_dir}/run_dpdk_tcp_rss_diagnostic.sh" \
    --validator "${run_dir}/validate_dpdk_tcp_rss_diagnostic.py" \
    --binary "${run_dir}/hft-dpdk.bin" \
    --dpdk-build-manifest "${run_dir}/dpdk-build-manifest.txt" \
    --bnx2x-ethdev "${run_dir}/bnx2x_ethdev.c" \
    --cpu-preflight "${run_dir}/cpu_preflight.json" \
    --dpdk-preflight "${run_dir}/preflight.json" \
    --cpu-preflight-script "${run_dir}/preflight_dpdk_cpu_idle.py" \
    --dpdk-preflight-script "${run_dir}/preflight_dpdk_bnx2x.py" \
    --pre-mutation-gate "${run_dir}/pre_mutation_gate.json" \
    --restoration-ledger "${run_dir}/restoration_ledger.json" \
  ...
  required_files=(
    ...
    main.rs run_dpdk_tcp_rss_diagnostic.sh validate_dpdk_tcp_rss_diagnostic.py
    bnx2x_ethdev.c preflight_dpdk_bnx2x.py
    preflight_dpdk_cpu_idle.py cpu_preflight.json cpu_preflight.stdout.json
    cpu_preflight.stderr.log
    dpdk-build-manifest.txt pre_mutation_gate.json result.json process_time.txt stdout.log stderr.log
    hugepages_during.json
    acceptance.json validation.stdout.json restoration_steps.tsv
...
pre_mutation_process_clear=false
pre_mutation_runtime_clear=false
pre_mutation_artifacts_verified=false
exact_dpdk_process_gate && pre_mutation_process_clear=true
exact_dpdk_runtime_gate && pre_mutation_runtime_clear=true
verify_run_dir_frozen_artifacts && pre_mutation_artifacts_verified=true
write_pre_mutation_gate "${pre_mutation_process_clear}" \
  "${pre_mutation_runtime_clear}" "${pre_mutation_artifacts_verified}"
if [[ "${pre_mutation_process_clear}" != true \
  || "${pre_mutation_runtime_clear}" != true ]]; then
  seal_pre_mutation_block 13
  exit 13
fi
if [[ "${pre_mutation_artifacts_verified}" != true ]]; then
  seal_pre_mutation_block 4
  exit 4
fi
...
{
  echo "run_id=${run_id}"
  echo "scope=dpdk_stock_bnx2x_tcp_rss_q2_diagnostic"
  echo "candidate_id=${candidate_id}"
  echo "frozen_thresholds_sha256=${thresholds_sha256}"
...
setsid timeout --signal=TERM --kill-after=5s "${run_timeout_s}s" \
  ...
  --traffic-profile tcp-rss-diagnostic \
...
observed_child_pgid=""
for _ in {1..50}; do
  observed_child_pgid="$(ps -o pgid= -p "${child_pid}" 2>/dev/null \
    | awk '{$1=$1; print; exit}')"
  [[ "${observed_child_pgid}" == "${child_pid}" ]] && break
  kill -0 "${child_pid}" 2>/dev/null || break
  sleep 0.01
done
# setsid must make the leader PID equal its PGID.  Never adopt a different
# observed PGID: it could still be this SSH shell's group before setsid().
if [[ "${observed_child_pgid}" != "${child_pid}" ]] \
  && kill -0 "${child_pid}" 2>/dev/null; then
  echo "setsid process group was not established safely" >&2
  dpdk_status=18
else
  wait "${child_pid}"
  dpdk_status="$?"
fi
# Preserve both identities through EXIT cleanup.  The timeout/session leader
# may exit before one of its descendants, and stop_child must still target and
# verify the original process group.
set -e
exit "${dpdk_status}"
