...
readonly default_config=/home/wangwt/phase_2/code/HFT-MGBS/configs/current_hardware_2_79_tpacket_traffic_v3_contract.json
...
readonly formal_binary_sha=499b0b8e9abc14877d85fa0009489ffdf2e7f8d9527986e6cd1993008c2589fe
...
expected = {
    ...
    ("candidate_id",): "TPACKET_QM_RUNTIME_AFFINITY_MULTIFLOW_V3_FLOW_CHURN_DIAGNOSTIC",
    ...
    ("traffic", "profile_id"): "deterministic_multiflow_v3_flowlen1",
    ...
    ("traffic", "expected_revisit_cycle_packets_per_queue"): 9280,
    ("traffic", "expected_active_timeout_closures_per_second"): 1160,
    ...
    ("pipeline", "binary_sha256"): "499b0b8e9abc14877d85fa0009489ffdf2e7f8d9527986e6cd1993008c2589fe",
    ...
    ("runtime_hard_gates", "per_full_window_latency_receipt_minimums", "flow_materialization_to_feature_enqueue"): 1000,
    ("runtime_hard_gates", "per_full_window_latency_receipt_minimums", "kernel_receive_to_feature_enqueue"): 1000,
    ("runtime_hard_gates", "per_full_window_latency_receipt_minimums", "flow_kernel_receive_to_remote_score_end_to_end"): 1000,
    ("runtime_hard_gates", "per_full_window_latency_receipt_minimums", "gpu_batch_round_trip"): 100,
    ("runtime_hard_gates", "require_raw_receipt_source_ids_unique"): True,
    ("runtime_hard_gates", "require_raw_receipts_not_truncated"): True,
    ("execution_blockers", "generator_tail_2_79_requalification_required"): True,
    ("execution_blockers", "per_flow_end_to_end_receipt_not_implemented_in_frozen_binary"): True,
    ("execution_blockers", "fail_before_host_mutation"): True,
...
# This frozen traffic-v3 contract is intentionally not executable yet. R3
# proved generator tail regression, and the frozen binary has no correlated
# per-flow end-to-end receipt. Releasing either blocker requires new evidence,
# a reviewed config change and new runner/config hashes; an authorization
# string cannot override this gate.
echo "traffic-v3 blocked before host mutation: requalify generator >=2.79 Mpps and implement real per-flow end-to-end receipts" >&2
exit 86
...
value = {
    ...
    "candidate_id": "TPACKET_QM_RUNTIME_AFFINITY_MULTIFLOW_V2_FULL_PIPELINE_DIAGNOSTIC",
...
python3 - "${evidence_dir}/pipeline_raw.json" "${evidence_dir}/per_window_latency_receipt_gate.json" <<'PY'
...
if not isinstance(receipts, list): raise SystemExit("raw latency receipts missing")
if metrics.get("raw_latency_sample_receipts_truncated") != 0:
    raise SystemExit("raw latency receipts were truncated")
source_ids = [receipt.get("source_id") for receipt in receipts if isinstance(receipt, dict)]
if len(source_ids) != len(receipts) or any(not isinstance(value, str) or not value for value in source_ids):
    raise SystemExit("raw latency receipt source identity missing")
if len(set(source_ids)) != len(source_ids):
    raise SystemExit("raw latency receipt source identities are not unique")
minimums = {
...
for receipt in receipts:
    ...
    if metric in counts and isinstance(window_id, int): counts[metric][window_id] += 1
per_window = {
...
if failures:
    raise SystemExit("per-full-window real latency receipt minimum failed")
# Per-window raw receipts are necessary but still do not declare qualification.
PY
