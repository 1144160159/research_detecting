    ...
    while True:
        for x in train_loader:
            ...
            if i_iter % config.log_frequency == 0:
                ...
                log_dict['train/input images'] = wandb.Image(
                    x[:config.num_images_log])
                log_dict['train/reconstructed images'] = wandb.Image(
                    rec[:config.num_images_log])
                log_dict['train/anomaly maps'] = wandb.Image(
                    (x - rec).abs()[:config.num_images_log])
