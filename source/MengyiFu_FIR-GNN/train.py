...
from utils.gnn import GNNFactory
...
class GNNTrainer:
    ...
    def __init__(self, config: dict, model: torch.nn.Module, class_names):
        self.config = config
        ...
        # 模型保存路径
        self.checkpoint_dir = os.path.join(config['train']['save_dir'], config['model']['name'])
...
def main():
    ...
    pyg_data = processor.create_pyg_data(node_data, edge_data)

    map_path = os.path.join(config["dataset"]["processed_dir"],config["dataset"]["label_level"],'label_mapping.json')
    with open(map_path, 'r') as f:
        label_mapping = json.load(f)
        class_names = list(label_mapping.keys())

    model = GNNFactory.create_model(config, pyg_data.num_node_features, len(class_names))
    total_params, trainable_params = count_parameters(model)
    print(f"总参数量：{total_params}，可训练参数量：{trainable_params}")

    trainer = GNNTrainer(config, model, class_names)
