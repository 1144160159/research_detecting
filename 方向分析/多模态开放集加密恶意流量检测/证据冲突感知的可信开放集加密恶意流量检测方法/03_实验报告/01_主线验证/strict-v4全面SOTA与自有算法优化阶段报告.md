...
### 102.173 CPU后端三fresh种子诊断与GPU正式结论撤回

服务器核心确认v2已完成`42/42`任务，seeds为`907/911/919`，评估、独立审计和完成canonical分别为`698bc422...974f`、`3378deae...f68`和`f66d0a44...7c3c`。后端复核确认该批任务虽然位于GPU服务器，但Pairwise和XGBoost训练实际使用CPU；因此本节结果统一降级为诊断，不授权GPU正式结论。Pairwise分层概率均值Alert Accuracy/Precision/Recall、Benign FPR、Known type Accuracy、Unknown alert/type Recall为`0.958889/0.996033/0.959528/0.045016/0.963626/0.844831/0.729006`。均值基本门通过，但seed919 FPR为`0.055619`，故三种子基本门失败；完整门也失败。

seed7只读开发比较180个全局配置后，Empirical-Tail Hybrid CAEOS冻结为`tail_noisy_or + 4%`双头配置。CPU后端fresh诊断没有读取或选择任何测试配置，三个种子均通过基本门；总体Alert Accuracy/Precision/Recall、FPR、Known type、Unknown alert/type Recall为`0.967798/0.996061/0.969120/0.045492/0.960278/0.886051/0.794180`，诊断canonical为`a71c51e5...0b4`。当前只允许写“该方案显示出达到基本门的可行性”，不允许写“GPU正式通过”或“未知攻击类型95%已完成”。

同拆分CPU后端XGBoost v3已完成`42/42`，总体为`0.977802/0.996120/0.979936/0.045048/0.992673/0.922353`，summary canonical为`94ece036...948`。seed919 FPR为`0.050095`，其逐种子基本门失败；且XGBoost没有Unknown标签头，只是闭集/预警锚点。该诊断提示主要瓶颈可能已从闭集表征转向未知预警和Unknown标注，不能用它替代自有算法或GPU正式目标。

GPU服务器CPU后端失效分析canonical为`9e86c973...05b5`：未知样本预警前漏检占`0.155169`，已预警但未标Unknown占`0.115825`；距离95% Unknown类型召回仍差`0.220994`。最低场景集中于Web XSS、Bot、Infiltration和PortScan。详细结果见[自有算法95%/5% CPU后端诊断、GPU纠偏与未知攻击差距](自有算法95_5正式确认与未知攻击差距_2026-07-28.md)。

### 102.174 攻击大类真实CUDA协议与异构负荷

CICIDS2017的14个恶意细分类按结果前固定映射聚合为Botnet、DDoS、DoS、BruteForce、WebAttack、Exploit和Reconnaissance七个恶意大类，Benign作为安全类另计。每大类最多5000条，原细类字段仅供审计。旧CPU开发协议seed7因缺少Pairwise `provenance.json`未生成completion，不能作为正式结果。

XGBoost CUDA能力探针确认版本2.1.4含CUDA 12.8，Booster为`cuda:0`，指定A6000 UUID为`GPU-a186...58ee`，峰值利用率92%、显存654 MiB，canonical为`2363745c...4932`；该探针不是效果结果。GPU开发v1协议`c5e1cdb4...b6682`进入CUDA后记录到最高99%利用率和1389 MiB显存，但因事后新加载Booster默认显示CPU而被审计误判为`7/7`失败，未生成completion。修复后独立冒烟证明活体Booster为`cuda:0`、峰值58%/350 MiB且`passes=true`。v2在零结果时重新冻结，开发seed17、fresh确认`953/967/971`，canonical为`00136de7...8a77`。XGBoost已知类专家必须逐任务证明CUDA设备、A6000 UUID、显存和计算进程；Pairwise冲突头仍为CPU，数据准备和指标计算允许CPU，明确不声称全链路GPU。

v2矩阵已`7/7`完成、0失败，全部GPU证据通过；completion `79e027e0...1202`记录峰值利用率99%、显存2077 MiB。开发选择的Alert Accuracy/Recall、FPR、Known family Accuracy、Unknown alert/family Recall为`0.901625/0.895768/0.041905/0.959176/0.765008/0.676494`，基本门失败。4.9%预算扩展135候选与双CUDA分类头630候选均无基本门通过者；双头最佳Accuracy/Recall也只有`0.923686/0.921224`。良性自编码器Botnet冒烟Unknown alert Recall为`0.022889`，停止扩展。由于开发门失败，`953/967/971`没有读取，fresh确认不启动。下一步转向包序列、时间窗口和训练期leave-one-known-family伪未知表征。RRC终态仍未生成，`goal_achieved=false`、基本GPU正式门、完整未知目标和全面SOTA均未授权。

### 102.175 CICIDS2017 Afternoon时间戳纠偏与PCAP序列重建

真实PCAP/CSV匹配确认Afternoon CSV的小时字段需要显式加12小时。来源审计记录`800814`条流应用`43200000000 us`调整、`2029929`条不调整，canonical为`092916c3...ecf32`。修正后包序列缓存包含`23127`条流，Benign/Botnet/BruteForce/DDoS/DoS/Exploit/Reconnaissance/WebAttack分别为`3556/1230/2726/4948/4974/35/5000/658`；NPZ SHA/canonical为`f9e30ce5...6c3b/80be732d...7e02`。Friday最终匹配到Botnet `1230`、DDoS `4948`、Reconnaissance `5000`条流，证明此前缺类主要来自时间错位。Exploit只有35条，后续必须保留小样本结论边界。

### 102.176 PSF-CAEOS包序列CUDA开发

PSF-CAEOS在seed29的7个leave-one-family任务全部完成，completion/development canonical为`2d6e363b...2895/3538d956...c623`。A6000利用率均值`79.75%`、峰值`100%`，显存峰值`20337 MiB`，CPU均值`64.49%`。开发选择的Alert Accuracy/Precision/Recall、FPR、Known type、Unknown alert/type Recall为`0.930732/0.995028/0.926705/0.036316/0.949159/0.705149/0.229644`。Known type距离95%仅差`0.000841`，但Unknown告警仍差`0.244851`，仅1/7场景通过基本门、0/7通过完整门，故不启动确认。

### 102.177 PSF-CAEOS-F与FSX-CAEOS负结果

流统计增强从官方CSV按不可变Flow ID连接80维统计，输出`23116`条流；数据SHA/canonical为`82cccd6d...7716/ba313d02...4cf8`，11个重复或冲突Flow ID按有限中位数聚合。PSF-CAEOS-F完成7/7 CUDA任务，Known type升至`0.957486`，但Alert Accuracy/Recall和Unknown alert/type Recall降为`0.882877/0.874583/0.605552/0.032952`，4/7基本门、0/7完整门；GPU均值`79.05%`。仅用良性参考尾部的945候选后验探针没有改善告警指标，不转为正式配置。

同数据上的FSX-CAEOS双XGBoost CUDA分支完成7/7任务，开发canonical为`ca87f97b...89c5`。Alert Accuracy/Recall、FPR、Known type、Unknown alert/type Recall为`0.869230/0.860034/0.035462/0.960434/0.603179/0.066782`，4/7基本门、0/7完整门；GPU均值`67.53%`、峰值`99%`。它再次说明已知分类超过95%并不代表未知预警达标。

### 102.178 FB-FSX-CAEOS细分类均衡CUDA开发

为避免按攻击大类最多5000条压缩细分类多样性，seed29缓存改为每细分类最多5000条，共`49193`条流，SHA为`a609ea6d...4ffd`。冻结协议`bf641519...f11b`使用capture-grouped划分，并同时移除未知家族的全部细分类；四类攻击概率与945种阈值/开放集配置形成3780候选。7/7 CUDA任务在`167.48`秒内完成、0失败，completion canonical为`5180d9f1...759e`；GPU均值`50.63%`、峰值`99%`、不低于50%的采样占`57.68%`。

开发选择canonical为`a49d7e50...d252`。Alert Accuracy/Precision/Recall、FPR、Known type、Unknown alert/type Recall为`0.884623/0.996654/0.880301/0.040762/0.958344/0.607385/0.092607`，3/7基本门、0/7完整门。Botnet未知告警只有`0.014242`，Reconnaissance为`0.424400`；细分类均衡改善Known type但没有学到跨家族恶意公共边界。`953/967/971`继续封存，确认不启动。

### 102.179 当前GPU自有算法Pareto前沿与下一纠偏

当前没有全面达标的单一最优自有算法。新增分支中PSF-CAEOS的Alert Accuracy与Unknown alert Recall最高；历史双CUDA XGBoost Hybrid的Unknown alert Recall更高（`0.793923`）且Known family为`0.959176`，但Alert Accuracy仍只有`0.923686`。CPU Empirical-Tail Hybrid虽在三fresh种子通过基本门，计算后端不符合正式GPU约束。下一候选不再继续同一阈值搜索，而是在包序列+流统计编码器上加入训练期leave-one-known-family伪未知、跨家族恶意对比学习和良性边界约束，并保持告警与Unknown标注两级解耦。只有开发7/7基本门与完整门同时通过，才允许启动fresh确认。轻量远端证据已同步到`results/06_自有算法95_5目标证据_20260728/gpu_cuda_development_snapshot_20260729/`，不含数据、NPZ分数或模型。

### 102.180 DMC-CAEOS Botnet资格试验

DMC-CAEOS使用包序列与80维流统计双编码、家族/恶意双度量头、监督对比损失、训练内伪未知混合和轮换留一家族余弦间隔。Botnet seed29 预注册资格门为`FPR<5%`、`Known type>=95%`、`Unknown alert Recall>=60.37%`。全统计v1得到Alert Accuracy `79.03%`、FPR `3.95%`、Known type `98.77%`、Unknown alert Recall `6.59%`，资格试验失败。

### 102.181 统计捷径消融与纯序列修正

统计 dropout 0.5 将Unknown alert Recall提升到`48.54%`，修正后的严格纯序列v3达到`50.98%`，但仍低于PSF Botnet对照`55.37%`。初始dropout 1.0仅在训练清零统计特征，验证/测试未清零，已经保留但降级为解释无效工件；修正后仅再增加`0.24`个百分点。结论是统计捷径显著存在，但静态双度量对比学习本身仍未学习到跨家族共享恶意边界。

### 102.182 PSF-DMC错误互补性

两个缓存不能以`flow_id`单键对齐，因为该字段存在重复。未生成效果输出前，协议附录冻结为`capture_id + flow_id + fine_label + family + packet/timing/mask`内容指纹，并按split内重复次数一对一配对。最终覆盖验证`4373/4377`、测试`5612/5615`条流。7个候选仅用验证已知标签选权，最终选回`dmc_only`；测试Unknown alert Recall仍为`50.98%`，简单融合路线关闭。

### 102.183 资源审计

所有DMC训练的CUDA证据均通过。并行dropout 0.5/1.0两个采样器观测整卡GPU均值`58.51%/61.95%`、中位数`68%/64%`，满足最低50%要求；单任务v1/v3均值仅`20.44%/20.91%`，不满足资源目标。后续多场景开发必须并行编排，报告均值、中位数、`>=50%`采样占比和峰值，不得用峰值替代持续负载。
